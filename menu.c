...
static int menu_select_machine_roms(libspectrum_machine machine, size_t start, size_t n);
...
MENU_CALLBACK_WITH_ACTION(menu_options_selectroms_machine_select)
{
    switch (action) {
        case  1:
            menu_select_machine_roms(LIBSPECTRUM_MACHINE_16, 0, 1);
            return;
        case  2:
            menu_select_machine_roms(LIBSPECTRUM_MACHINE_48, 1, 1);
            return;
        case  3:
            menu_select_machine_roms(LIBSPECTRUM_MACHINE_128, 2, 2);
            return;
        case  4:
            menu_select_machine_roms(LIBSPECTRUM_MACHINE_PLUS2, 4, 2);
            return;
        case  5:
            menu_select_machine_roms(LIBSPECTRUM_MACHINE_PLUS2A, 6, 4);
            return;
        case  6:
            menu_select_machine_roms(LIBSPECTRUM_MACHINE_PLUS3, 10, 4);
            return;
        case  7:
            menu_select_machine_roms(LIBSPECTRUM_MACHINE_PLUS3E, 14, 4);
            return;
        case  8:
            menu_select_machine_roms(LIBSPECTRUM_MACHINE_TC2048, 18, 1);
            return;
        case  9:
            menu_select_machine_roms(LIBSPECTRUM_MACHINE_TC2068, 19, 2);
            return;
        case 10:
            menu_select_machine_roms(LIBSPECTRUM_MACHINE_TS2068, 21, 2);
            return;
        case 11:
            menu_select_machine_roms(LIBSPECTRUM_MACHINE_PENT, 23, 3);
            return;
        case 12:
            menu_select_machine_roms(LIBSPECTRUM_MACHINE_PENT512, 26, 4);
            return;
        case 13:
            menu_select_machine_roms(LIBSPECTRUM_MACHINE_PENT1024, 30, 4);
            return;
        case 14:
            menu_select_machine_roms(LIBSPECTRUM_MACHINE_SCORP, 34, 4);
            return;
        case 15:
            menu_select_machine_roms(LIBSPECTRUM_MACHINE_SE, 38, 2);
            return;
    }
    ui_error(UI_ERROR_ERROR, "menu_options_selectroms_machine_select: unknown action %d", action);
    fuse_abort();
}
...
MENU_CALLBACK_WITH_ACTION(menu_options_selectroms_peripheral_select)
{
    switch (action) {
        case  1:
            menu_select_peripheral_roms("Interface 1", 0, 1);
            return;
        case  2:
            menu_select_peripheral_roms("Beta 128", 1, 1);
            return;
        case  3:
            menu_select_peripheral_roms("+D", 2, 1);
            return;
        case  4:
            menu_select_peripheral_roms("Didaktik 80", 3, 1);
            return;
        case  5:
            menu_select_peripheral_roms("DISCiPLE", 4, 1);
            return;
        case  6:
            menu_select_peripheral_roms("Multiface One", 5, 1);
            return;
        case  7:
            menu_select_peripheral_roms("Multiface 128", 6, 1);
            return;
        case  8:
            menu_select_peripheral_roms("Multiface 3", 7, 1);
            return;
        case  9:
            menu_select_peripheral_roms("Opus Discovery", 8, 1);
            return;
        case 10:
            menu_select_peripheral_roms("SpeccyBoot", 9, 1);
            return;
        case 11:
            menu_select_peripheral_roms("uSource", 10, 1);
            return;
    }
    ui_error(UI_ERROR_ERROR, "menu_options_selectroms_peripheral_select: unknown action %d", action);
    fuse_abort();
}
...
MENU_CALLBACK_WITH_ACTION(menu_media_ide_insert)
{
    ...
    switch (action) {
        case 1:
            simpleide_insert(filename, LIBSPECTRUM_IDE_MASTER);
            break;
        case 2:
            simpleide_insert(filename, LIBSPECTRUM_IDE_SLAVE);
            break;
        case 3:
            zxatasp_insert(filename, LIBSPECTRUM_IDE_MASTER);
            break;
        case 4:
            zxatasp_insert(filename, LIBSPECTRUM_IDE_SLAVE);
            break;
        case 5:
            zxcf_insert(filename);
            break;
        case 6:
            divide_insert(filename, LIBSPECTRUM_IDE_MASTER);
            break;
        case 7:
            divide_insert(filename, LIBSPECTRUM_IDE_SLAVE);
            break;
        case 8:
            divmmc_insert(filename);
            break;
        case 9:
            zxmmc_insert(filename);
            break;
    }
...
MENU_CALLBACK_WITH_ACTION(menu_media_ide_commit)
{
    ...
    switch (action) {
        case 1:
            simpleide_commit(LIBSPECTRUM_IDE_MASTER);
            break;
        case 2:
            simpleide_commit(LIBSPECTRUM_IDE_SLAVE);
            break;
        case 3:
            zxatasp_commit(LIBSPECTRUM_IDE_MASTER);
            break;
        case 4:
            zxatasp_commit(LIBSPECTRUM_IDE_SLAVE);
            break;
        case 5:
            zxcf_commit();
            break;
        case 6:
            divide_commit(LIBSPECTRUM_IDE_MASTER);
            break;
        case 7:
            divide_commit(LIBSPECTRUM_IDE_SLAVE);
            break;
        case 8:
            divmmc_commit();
            break;
...
MENU_CALLBACK_WITH_ACTION(menu_media_ide_eject)
{
    ...
    switch (action) {
        case 1:
            simpleide_eject(LIBSPECTRUM_IDE_MASTER);
            break;
        case 2:
            simpleide_eject(LIBSPECTRUM_IDE_SLAVE);
            break;
        case 3:
            zxatasp_eject(LIBSPECTRUM_IDE_MASTER);
            break;
        case 4:
            zxatasp_eject(LIBSPECTRUM_IDE_SLAVE);
            break;
        case 5:
            zxcf_eject();
            break;
        case 6:
            divide_eject(LIBSPECTRUM_IDE_MASTER);
            break;
        case 7:
            divide_eject(LIBSPECTRUM_IDE_SLAVE);
            break;
        case 8:
            divmmc_eject();
            break;
...
MENU_CALLBACK(menu_file_screenshot_savescreenasmlt)
{
    ...
    if (machine_current->timex && scld_last_dec.name.hires) {
        ui_error(UI_ERROR_ERROR, "MLT format not supported for Timex hi-res screen mode");
        fuse_emulation_unpause();
        return;
    }
...
MENU_CALLBACK(menu_file_movie_record_recordfromrzx)
{
    ...
    if (rzx_playback || rzx_recording || movie_recording) {
        return;
    }
...
static int menu_select_machine_roms(libspectrum_machine machine, size_t start, size_t n)
{
    return menu_select_roms_with_title(libspectrum_machine_name(machine), start, n, 0);
}


static int menu_select_peripheral_roms(const char *peripheral_name, size_t start, size_t n)
{
    return menu_select_roms_with_title(peripheral_name, start, n, 1);
}


const char *menu_machine_detail(void)
{
    return libspectrum_machine_name(machine_current->machine);
}


const char *menu_filter_detail(void)
{
    return scaler_name(current_scaler);
}


const char *menu_keyboard_joystick_detail(void)
{
    return joystick_name[settings_current.joystick_keyboard_output];
}


const char *menu_joystick_1_detail(void)
{
    return joystick_name[settings_current.joystick_1_output];
}


const char *menu_joystick_2_detail(void)
{
    return joystick_name[settings_current.joystick_2_output];
}


const char *menu_tape_detail(void)
{
    if (!tape_present()) {
        return "Not inserted";
    }
...
static const char *menu_disk_detail(fdd_t *f)
{
    int i = 0;

    if (!f->loaded) {
        return disk_detail_str[8];
    }
    if (f->wrprot) {
        i = 1;
    }
...
const char *menu_plus3a_detail(void)
{
...
const char *menu_plus3b_detail(void)
{
...
const char *menu_beta128a_detail(void)
{
...
const char *menu_beta128b_detail(void)
{
...
const char *menu_beta128c_detail(void)
{
...
const char *menu_beta128d_detail(void)
{
...
const char *menu_opus1_detail(void)
{
...
const char *menu_opus2_detail(void)
{
...
const char *menu_plusd1_detail(void)
{
...
const char *menu_plusd2_detail(void)
{
...
const char *menu_didaktik_a_detail(void)
{
...
const char *menu_didaktik_b_detail(void)
{
...
const char *menu_disciple1_detail(void)
{
...
const char *menu_disciple2_detail(void)
{
