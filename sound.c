...
static struct speaker_type_tag speaker_type[] = {{200, -37.0}, {1000, -67.0}, {0, 0.0}};
...
{
    return (machine_current->timings.processor_speed / 100) * settings_current.emulation_speed;
}
...
static int sound_init_blip(Blip_Buffer **buf, Blip_Synth **synth)
{
    *buf = new_Blip_Buffer();
    ...
    // down to 2% of speed where a single Speccy frame generates just under 1s of sound
...
void sound_init(const char *device)
{
    ...
    if (settings_current.sound && sound_lowlevel_init(device, &settings_current.sound_freq, &sound_stereo_ay)) {
        return;
    }

    if (!sound_init_blip(&left_buf, &left_beeper_synth)) {
        return;
    }
    if ((sound_stereo_ay != SOUND_STEREO_AY_NONE) && (!sound_init_blip(&right_buf, &right_beeper_synth))) {
        return;
    }
    ...
    blip_synth_set_volume(left_covox_synth, sound_get_volume(settings_current.volume_covox));
    ...
    if (sound_stereo_ay != SOUND_STEREO_AY_NONE) {
        ...
        *ay_mid_synth_r = new_Blip_Synth();
        ...
        blip_synth_set_volume(right_covox_synth, sound_get_volume(settings_current.volume_covox));
...
void sound_register_startup(void)
{
    ...
    startup_manager_register(STARTUP_MANAGER_MODULE_SOUND,
                             dependencies,
...
static inline void ay_do_tone(int level, unsigned int tone_count, int *var, int chan)
{
    *var = 0;
...
static void sound_ay_overlay(void)
{
    ...
    for (f = 0; f < machine_current->timings.tstates_per_frame; f += AY_CLOCK_DIVISOR * AY_CLOCK_RATIO) {
        // update ay registers
        ...
        if (last_chan1 != chan1) {
            ...
            if (ay_a_synth_r) {
                blip_synth_update(ay_a_synth_r, f, chan1);
            }
            last_chan1 = chan1;
        }
        if (last_chan2 != chan2) {
            ...
            if (ay_b_synth_r) {
                blip_synth_update(ay_b_synth_r, f, chan2);
            }
            last_chan2 = chan2;
        }
        if (last_chan3 != chan3) {
            ...
            if (ay_c_synth_r) {
                blip_synth_update(ay_c_synth_r, f, chan3);
            }
            last_chan3 = chan3;
        }
...
// don't make the change immediately; record it for later, to be made by sound_frame() (via sound_ay_overlay())
...
// very simple routine as the output is already a digitized waveform
