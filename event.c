...
static int event_init(void *context)
{
...
static gint event_add_cmp(gconstpointer a1, gconstpointer b1)
{
...
void event_add_with_data(libspectrum_dword event_time, int type, void *user_data)
{
    event_t *ptr;
...
int event_do_events(void)
{
    ...
    while (event_next_event <= tstates) {
        event_descriptor_t descriptor;
        ptr = event_list->data;
        descriptor = g_array_index(registered_events, event_descriptor_t, ptr->type);
        ...
        if (descriptor.fn) {
            descriptor.fn(ptr->tstates, ptr->type, ptr->user_data);
        }
...
static void event_reduce_tstates(gpointer data, gpointer user_data)
{
    event_t *ptr = data;
...
void event_frame(libspectrum_dword tstates_per_frame)
{
    ...
    event_next_event = event_list ? ((event_t*)(event_list->data))->tstates : event_no_events;
}


// Do all events that would happen between the current time and when
// the next interrupt will occur; called only when RZX playback is in effect
...
static void set_event_null_with_user_data(gpointer data, gpointer user_data)
{
    event_t *event = data;
...
static void event_free_entry(gpointer data, gpointer user_data GCC_UNUSED)
{
...
static void registered_events_free(void)
{
    int i;
    event_descriptor_t descriptor;
...
void event_register_startup(void)
{
    ...
    startup_manager_register(STARTUP_MANAGER_MODULE_EVENT,
                             dependencies,
