...
static const struct menu_item_entries menu_item_lookup[] = {
    ...
    {UI_MENU_ITEM_MEDIA_DISK_DIDAKTIK,
     "/Media/Disk/Didaktik 80"
    },

    {UI_MENU_ITEM_MEDIA_DISK_DIDAKTIK_A,
     "/Media/Disk/Didaktik 80/Drive A"
    },
...
static int print_error_to_stderr(ui_error_level severity, const char *message)
{
    // Print the error to stderr if it's more significant than just informational
    if (severity > UI_ERROR_INFO) {

        /* For the fb and svgalib UIs, we don't want to write to stderr if it's a terminal,
           as it's then likely to be what we're currently using for graphics output,
           and writing text to it isn't a good idea.
           Things are OK if we're exiting though */
#if defined(UI_FB) || defined(UI_SVGA)
        if (isatty(STDERR_FILENO) && !fuse_exiting) {
            return 1;
        }
...
libspectrum_error ui_libspectrum_error(libspectrum_error error GCC_UNUSED, const char *format, va_list ap)
{
    char new_format[257];
...
void ui_mouse_button(int button, int down)
{
    ...
    if (!ui_mouse_grabbed && !mouse_grab_suspended) {
        button = 2;
    }
    ...
    switch (button) {
        ...
        case 2:
            if (ui_mouse_present && settings_current.kempston_mouse  && !down && !mouse_grab_suspended) {
                ui_mouse_grabbed = ui_mouse_grabbed ? ui_mouse_release(1) : ui_mouse_grab(0);
            }
            break;
    }
}
...
int ui_menu_activate(ui_menu_item item, int active)
{
    ...
    for (ptr = menu_item_lookup; ptr->string1; ptr++) {
        ...
        if (item == ptr->item) {
            ...
            if (ptr->string2) {
                ui_menu_item_set_active(ptr->string2, ptr->string2_inverted ? !active : active);
            }
            if (ptr->string3) {
                ui_menu_item_set_active(ptr->string3, ptr->string3_inverted ? !active : active);
            }
            if (ptr->string4) {
                ui_menu_item_set_active(ptr->string4, ptr->string4_inverted ? !active : active);
            }
            if (ptr->string5) {
                ui_menu_item_set_active(ptr->string5, ptr->string5_inverted ? !active : active);
            }
            if (ptr->string6) {
                ui_menu_item_set_active(ptr->string6, ptr->string6_inverted ? !active : active);
            }
            if (ptr->string7) {
                ui_menu_item_set_active(ptr->string7, ptr->string7_inverted ? !active : active);
            }
            return 0;
        }
    }
...
void ui_menu_disk_update(void)
{
    ...
    if (drives_avail) {
        ...
        ui_statusbar_update(UI_STATUSBAR_ITEM_DISK, UI_STATUSBAR_STATE_NOT_AVAILABLE);
    }
