...
{
    registered_modules = g_array_new(FALSE, FALSE, sizeof(registered_module_t));
    end_functions = g_array_new(FALSE, FALSE, sizeof(startup_manager_end_fn));
}
                              ...
                              startup_manager_end_fn end_fn)
{
    ...
    registered_module.dependencies = g_array_sized_new(FALSE, FALSE, sizeof(startup_manager_module), dependency_count);
    g_array_append_vals(registered_module.dependencies, dependencies, dependency_count);
...
static void remove_dependency(startup_manager_module module)
{
    ...
    for (i = 0; i < registered_modules->len; i++) {
        registered_module_t *registered_module =  &g_array_index(registered_modules, registered_module_t, i);
        GArray *dependencies = registered_module->dependencies;

        for (j = 0; j < dependencies->len; j++) {
            startup_manager_module dependency = g_array_index(dependencies, startup_manager_module, j);
...
int startup_manager_run(void)
{
    ...
    /* Loop until we can't make any more progress; this will either be because
       we've called every function (good!) or because there's a logical error in the dependency graph (bad!) */
    do {
        i = 0;
        progress_made = 0;
        ...
        while (i < registered_modules->len) {
            ...
            if (registered_module->dependencies->len == 0) {
                ...
                if (registered_module->init_fn) {
                    error = registered_module->init_fn(registered_module->init_context);
                    if (error) {
                        return error;
                    }
                }
    ...
    if (registered_modules->len) {
        ui_error(UI_ERROR_ERROR, "%u startup modules could not be called", registered_modules->len);
        return 1;
    }
...
void startup_manager_run_end(void)
{
    ...
    for (i = end_functions->len; i-- != 0;) {
        startup_manager_end_fn end_fn = g_array_index(end_functions, startup_manager_end_fn, i);
        end_fn();
    }
