...
{
    if (!(libspectrum_machine_capabilities(machine_current->machine) & LIBSPECTRUM_MACHINE_CAPABILITY_TIMEX_DOCK)) {
...
void dck_eject(void)
{
    ...
    if (settings_current.dck_file) {
        libspectrum_free(settings_current.dck_file);
    }
...
static memory_page *dck_get_memory_page(libspectrum_dck_bank bank, size_t index)
{
    switch (bank) {
...
int dck_reset(void)
{
    ...
    error = libspectrum_dck_read2(dck, file.buffer, file.length, settings_current.dck_file);
    if (error) {
    ...
    while (dck->dck[num_block] != NULL) {
        ...
        if ((dck_bank != LIBSPECTRUM_DCK_BANK_HOME) && (dck_bank != LIBSPECTRUM_DCK_BANK_DOCK) &&
            (dck_bank != LIBSPECTRUM_DCK_BANK_EXROM)) {
            ui_error(UI_ERROR_INFO, "Sorry, bank ID %i is unsupported", dck->dck[num_block]->bank);
            libspectrum_dck_free(dck, 0);
            return 1;
        }
            ...
            switch (dck->dck[num_block]->access[i]) {
                ...
                case LIBSPECTRUM_DCK_PAGE_RAM:
                    /* Because the scr and snapshot code depends on the standard
                       memory map being in the RAM[] array,
                       we just copy RAM blocks from the HOME bank into the appropriate page;
                       in other cases, we allocate ourselves a new page to store the contents in */
                    if ((dck_bank == LIBSPECTRUM_DCK_BANK_HOME) && (i > 1)) {
                        for (j = 0; j < MEMORY_PAGES_IN_8K; j++) {
                            page = dck_get_memory_page(dck_bank, (i * MEMORY_PAGES_IN_8K) + j);
                            if (dck->dck[num_block]->access[i] == LIBSPECTRUM_DCK_PAGE_RAM) {
                                memcpy(page->page, dck->dck[num_block]->pages[i] + (j * MEMORY_PAGE_SIZE), MEMORY_PAGE_SIZE);
                            } else {
