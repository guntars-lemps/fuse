...
typedef struct if1_ula_t {
    ...
    int tx; /* (T)xD the name is very kind, because this is the read end of
                     the TxD wire of DATA machine (really RxD the view of spectrum */
    int rx; /* (R)xD the name is very kind, because this is the write end of
                     the RxD wire of DATA machine (really TxD the view of spectrum */
...
static void update_menu(enum if1_menu_item what)
{
    if ((what == UMENU_ALL) || (what == UMENU_MDRV1)) {
        ...
        ui_menu_activate(UI_MENU_ITEM_MEDIA_IF1_M1_WP_SET, (!MDR_IN(1) ? 0 : !MDR_WP(1)));
    }

    if ((what == UMENU_ALL) || (what == UMENU_MDRV2)) {
        ui_menu_activate(UI_MENU_ITEM_MEDIA_IF1_M2_EJECT, MDR_IN(2));
        ui_menu_activate(UI_MENU_ITEM_MEDIA_IF1_M2_WP_SET, (!MDR_IN(2) ? 0 : !MDR_WP(2)));
    }

    if ((what == UMENU_ALL) || (what == UMENU_MDRV3)) {
        ui_menu_activate(UI_MENU_ITEM_MEDIA_IF1_M3_EJECT, MDR_IN(3));
        ui_menu_activate(UI_MENU_ITEM_MEDIA_IF1_M3_WP_SET, (!MDR_IN(3) ? 0 : !MDR_WP(3)));
    }

    if ((what == UMENU_ALL) || (what == UMENU_MDRV4)) {
        ui_menu_activate(UI_MENU_ITEM_MEDIA_IF1_M4_EJECT, MDR_IN(4));
        ui_menu_activate(UI_MENU_ITEM_MEDIA_IF1_M4_WP_SET, (!MDR_IN(4) ? 0 : !MDR_WP(4)));
    }

    if ((what == UMENU_ALL) || (what == UMENU_MDRV5)) {
        ui_menu_activate(UI_MENU_ITEM_MEDIA_IF1_M5_EJECT, MDR_IN(5));
        ui_menu_activate(UI_MENU_ITEM_MEDIA_IF1_M5_WP_SET, (!MDR_IN(5) ? 0 : !MDR_WP(5)));
    }

    if ((what == UMENU_ALL) || (what == UMENU_MDRV6)) {
        ui_menu_activate(UI_MENU_ITEM_MEDIA_IF1_M6_EJECT, MDR_IN(6));
        ui_menu_activate(UI_MENU_ITEM_MEDIA_IF1_M6_WP_SET, (!MDR_IN(6) ? 0 : !MDR_WP(6)));
    }

    if ((what == UMENU_ALL) || (what == UMENU_MDRV7)) {
        ui_menu_activate(UI_MENU_ITEM_MEDIA_IF1_M7_EJECT, MDR_IN(7));
        ui_menu_activate(UI_MENU_ITEM_MEDIA_IF1_M7_WP_SET, (!MDR_IN(7) ? 0 : !MDR_WP(7)));
    }

    if ((what == UMENU_ALL) || (what == UMENU_MDRV8)) {
        ui_menu_activate(UI_MENU_ITEM_MEDIA_IF1_M8_EJECT, MDR_IN(8));
        ui_menu_activate(UI_MENU_ITEM_MEDIA_IF1_M8_WP_SET, (!MDR_IN(8) ? 0 : !MDR_WP(8)));
    }

    if ((what == UMENU_ALL) || (what == UMENU_RS232)) {
        ui_menu_activate(UI_MENU_ITEM_MEDIA_IF1_RS232_UNPLUG_R, ((if1_ula.fd_r > -1) ? 1 : 0));
        ui_menu_activate(UI_MENU_ITEM_MEDIA_IF1_RS232_UNPLUG_T, ((if1_ula.fd_t > -1) ? 1 : 0));
#ifdef BUILD_WITH_SNET
        ui_menu_activate(UI_MENU_ITEM_MEDIA_IF1_SNET_UNPLUG, ((if1_ula.fd_net > -1) ? 1 : 0));
#endif
    }
}
...
static void if1_end(void)
{
    ...
    for (m = 0; m < 8; m++) {
        libspectrum_error error = libspectrum_microdrive_free(microdrive[m].cartridge);
        if (error) {
            return;
        }
    }
...
void if1_register_startup(void)
{
    ...
    startup_manager_register(STARTUP_MANAGER_MODULE_IF1, dependencies, ARRAY_SIZE(dependencies), if1_init, NULL, if1_end);
}
...
static void if1_reset(int hard_reset GCC_UNUSED)
{
    ...
    if (!periph_is_active(PERIPH_TYPE_INTERFACE1)) {
        ui_statusbar_update(UI_STATUSBAR_ITEM_MICRODRIVE, UI_STATUSBAR_STATE_NOT_AVAILABLE);

        return;
    }

    // Check for an Interface 1 ROM
    if (machine_load_rom_bank(if1_memory_map_romcs,
                              0,
    ...
    update_menu(UMENU_ALL);
    ui_statusbar_update(UI_STATUSBAR_ITEM_MICRODRIVE, UI_STATUSBAR_STATE_INACTIVE);
...
static void if1_from_snapshot(libspectrum_snap *snap)
{
    ...
    if (libspectrum_snap_interface1_custom_rom(snap) &&
        ...
        machine_load_rom_bank_from_buffer(if1_memory_map_romcs,
                                          0,
                                          libspectrum_snap_interface1_rom(snap, 0),
                                          ROM_SIZE,
                                          1)) {
...
static void if1_to_snapshot(libspectrum_snap *snap)
{
    ...
    if (if1_memory_map_romcs[0].save_to_snapshot) {
        ...
        for (i = 0; i < MEMORY_PAGES_IN_8K; i++) {
            memcpy((buffer + (i * MEMORY_PAGE_SIZE)), if1_memory_map_romcs[i].page, MEMORY_PAGE_SIZE);
        }
...
static enum if1_port decode_port(libspectrum_word port)
{
    switch (port & 0x0018) {
        case 0x0000:
            return PORT_MDR;
    ...
    for (m = 0; m < 8; m++) {
        ...
        if (mdr->motor_on && mdr->inserted) {
            ...
            if (mdr->transfered < mdr->max_bytes) {
                mdr->last = libspectrum_microdrive_data(mdr->cartridge, mdr->head_pos);
                increment_head(m);
            }
...
static libspectrum_byte port_ctr_in(void)
{
    ...
    // Here we have to poll, the 'SinclairNet' busy flag but never used by software in Interface 1
    if (if1_ula.busy == 0) {
        ret &= 0xef; // %11101111
    }
...
//  return 1 if read a byte, 0 if nothing interesting...
...
static libspectrum_byte port_net_in(void)
{
    ...
    if (if1_ula.cts) { // If CTS == 1
        if (if1_ula.count_in == 0) {
            ...
            if1_ula.tx = 0; /* (n)ow send __ to if1 later we raise :-) */
...
no_rs232_in:
    ...
    if (if1_ula.s_net_mode == 0) { // if we do raw
        ...
        /* if (if1_ula.s_net_mode == 1) if we do interpreted */
        /* Here is the input routine. There are several stage in input and output.
           So first for output. if1 first do SEND-SC
           (http://www.wearmouth.demon.co.uk/if1_2.htm#L101E) to send
           a Sync-Out signal and SEND-SC do first a NET-STATE
           (http://www.wearmouth.demon.co.uk/if1_2.htm#L0FBC) to see the line activity:
           11xxxxxx times (192-255) have to get a zero (bit for network)
           plus 1 times more from SEND-SC.
           Next SEND-SC send a 0 which is a 1 on the net wire (negated output, straight input!!!)

           OK. In input first if1 call WT-SC-E to check Network activity
           (http://www.wearmouth.demon.co.uk/if1_2.htm#L0FD3). Now check
           128 times the net wire (we do two round, because to differentiate net out routines...)
        */
        ...
        } else if ((if1_ula.net_state > 0x0101) && (if1_ula.net_state < 0x010a)) { // we send the data bits...
            if1_ula.net_state++;
...
static void port_mdr_out(libspectrum_byte val)
{
    ...
    for (m = 0; m < 8; m++) {
        ...
        if (mdr->motor_on && mdr->inserted) {
            ...
            if ((mdr->transfered > 11) && (mdr->transfered < (mdr->max_bytes + 12))) {
                libspectrum_microdrive_set_data(mdr->cartridge, mdr->head_pos, val);
                increment_head(m);
                mdr->modified = 1;
            }
            mdr->transfered++;
...
static void port_ctr_out(libspectrum_byte val)
{
    ...
    if (!(val & 0x02) && (if1_ula.comms_clk)) { // ~~\__
        ...
        if (microdrive[0].motor_on || microdrive[1].motor_on ||
            ...
            if (!if1_mdr_status) {
                ui_statusbar_update(UI_STATUSBAR_ITEM_MICRODRIVE, UI_STATUSBAR_STATE_ACTIVE);
                if1_mdr_status = 1;
            }
    ...
    if (val & 0x01) { // comms_data == 1
        /* Interface 1 service manual p.:1.4 par.: 1.5.1
           ...
           being used (I do not know what it is exactly meaning.
           It is a hardware not being used, or a software should not use..?)
           This signal is routed from pin 30 to the emitter of transistor Q3 (RX DATA) and
           via resistor R4, to the base of transistor Q1 (NET).
           When COMMS_OUT is high Q3 is enabled this selecting RS232, and when it is low
           Q1 is enabled selecting the network.

           OK, the schematics offer a different interpretation, because if
           COMMS_OUT pin level high (>+3V) then Q3 is off
           (the basis cannot be more higher potential then emitter (NPN transistor),
           so whatever is on the IC1 RX DATA (pin 33) the basis of Q4 is always on +12V,
           so the collector of Q4 (PNP transistor) always on -12V.
           If COMMS_OUT pin level goes low (~0V),
           then Q3 basis (connected to IC1 RX DATA pin) can be higher level (>+3V) than emitter,
           so the basis potential of Q4 depend on IC1 RX DATA.
    ...
    val = (val & 0x10) ? 1 : 0;
    if (settings_current.rs232_handshake && (if1_ula.fd_t != -1) && (if1_ula.cts != val)) {
...
#ifdef IF1_DEBUG_NET
    fprintf(stderr,
            "Set CTS to %d, set WAIT to %d and COMMS_DATA to %d\n",
            if1_ula.cts,
            if1_ula.wait,
            if1_ula.comms_data);
#endif
...
static void port_net_out(libspectrum_byte val)
{
    ...
    if (if1_ula.comms_data == 1) { // OK, RS232
        ...
        if (if1_ula.s_net_mode == 0) { // if we out bit by bit, do it
            ...
            /* OK, examining the schematics of if1 and the disassembly of if1 ROM,
               I see that the Q1 and Q2 transistors negate the RX DATA signal,
               and the floating state of the net wire is the ~0V level, the forced is the ~3V.
...
#endif
            } else if ((if1_ula.net_state > 192) && (if1_ula.net_state < 0x0200) && ((val & 0x01) == 0)) {
...
static void if1_port_out(libspectrum_word port GCC_UNUSED, libspectrum_byte val)
{
    ...
    fprintf(stderr,
            "In if1_port_out(%%%d%d%d%d%d%d%d%d => 0x%04x).\n",
            !!(val & 128),
            !!(val & 64),
            !!(val & 32),
            !!(val & 16),
...
static void increment_head(int m)
{
    ...
    if (microdrive[m].head_pos >=
        (libspectrum_microdrive_cartridge_len(microdrive[m].cartridge) * LIBSPECTRUM_MICRODRIVE_BLOCK_LEN)) {
        microdrive[m].head_pos = 0;
    }
}
...
{
    libspectrum_microdrive_set_write_protect(microdrive[drive].cartridge, (wrprot ? 1 : 0));
...
static void if1_mdr_new(microdrive_t *mdr)
{
    ...
    if (settings_current.mdr_random_len) { // Random length
        len = 171 + ((rand() >> 2) + (rand() >> 2) + (rand() >> 2) + (rand() >> 2)) / rnd_factor;
    } else {
        len = settings_current.mdr_len = ((settings_current.mdr_len < 10) ?
                                         10 :
                                         ((settings_current.mdr_len > LIBSPECTRUM_MICRODRIVE_BLOCK_MAX) ?
    ...
    for (i = libspectrum_microdrive_cartridge_len(mdr->cartridge); i > 0; i--) {
...
int if1_mdr_insert(int which, const char *filename)
{
    ...
    if (which == -1) {
        ui_error(UI_ERROR_ERROR, "Cannot insert cartridge '%s', all Microdrives in use", filename);
        return 1;
    }
    ...
    if (libspectrum_microdrive_mdr_read(mdr->cartridge, mdr->file.buffer, mdr->file.length)) {
...
int if1_mdr_eject(int which)
{
    ...
    if (mdr->modified) {
        ...
        ui_confirm_save_t confirm = ui_confirm_save("Cartridge in Microdrive %i has been modified.\nDo you want to save it?",
                                                    which + 1);

        switch (confirm) {

            case UI_CONFIRM_SAVE_SAVE:
                if (if1_mdr_save(which, 0)) {
                    return 1; // first save
                }
                break;
...
int if1_mdr_write(int which, const char *filename)
{
    ...
    libspectrum_microdrive_mdr_write(mdr->cartridge, &mdr->file.buffer, &mdr->file.length);

    if (filename == NULL) {
        filename = mdr->filename; // Write over the original file
    }
...
void if1_plug(const char *filename, int what)
{
    ...
    switch (what) {
        case 1:
            ...
            if (fcntl(fd, F_SETFL, (O_RDONLY | O_NONBLOCK))) {
                ui_error(UI_ERROR_ERROR, "Cannot set O_RDONLY on '%s': %s", filename, strerror(errno));
            }
        ...
        case 2:
            ...
            if (fcntl(fd, F_SETFL, (O_WRONLY | O_NONBLOCK))) {
                ui_error(UI_ERROR_ERROR, "Cannot set O_WRONLY on '%s': %s", filename, strerror(errno));
            }
            break;
    ...
    if (!settings_current.rs232_handshake && (if1_ula.fd_t != -1) && (if1_ula.fd_r != -1)) {
        if1_ula.dtr = 1;
    }

    if (fd < 0) {
        ui_error(UI_ERROR_ERROR, "Error opening '%s': %s", filename, strerror(errno));
        return;
    }
