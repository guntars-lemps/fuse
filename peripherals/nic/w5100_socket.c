...
static void w5100_write_socket_mr(nic_w5100_socket_t *socket, libspectrum_byte b)
{
    ...
    switch (mode) {
        ...
        case W5100_SOCKET_MODE_TCP:
            ...
            if (flags != 0x20) {
                ui_error(UI_ERROR_WARNING,
                         "w5100: unsupported flags 0x%02x set for TCP mode on socket %d\n",
                         b & 0xf0,
                         socket->id);
                flags = 0x20;
            }
        ...
        case W5100_SOCKET_MODE_UDP:
            ...
            if (flags != 0x00) {
                ui_error(UI_ERROR_WARNING,
                         "w5100: unsupported flags 0x%02x set for UDP mode on socket %d\n",
                         b & 0xf0,
                         socket->id);
                flags = 0;
            }
...
{
    if (((socket_obj->mode == W5100_SOCKET_MODE_UDP) || (socket_obj->mode == W5100_SOCKET_MODE_TCP)) &&
...
{
    if ((socket->state == W5100_SOCKET_STATE_UDP) || (socket->state == W5100_SOCKET_STATE_ESTABLISHED)) {
...
static void w5100_write_socket_cr(nic_w5100_t *self, nic_w5100_socket_t *socket, libspectrum_byte b)
{
...
static void w5100_write_socket_port(nic_w5100_t *self, nic_w5100_socket_t *socket, int which, libspectrum_byte b)
{
...
void nic_w5100_socket_add_to_sets(nic_w5100_socket_t *socket, fd_set *readfds, fd_set *writefds, int *max_fd)
{
    ...
    if (socket->fd != compat_socket_invalid) {
        /* We can process a UDP read if we're in a UDP state and there are at least
           9 bytes free in our buffer (8 byte UDP header and 1 byte of actual data). */
...
static void w5100_socket_process_read(nic_w5100_socket_t *socket)
{
