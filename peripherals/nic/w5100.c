...
static void *w5100_io_thread(void *arg)
{
    ...
    while (!self->stop_io_thread) {
        ...
        int active;
        compat_socket_t selfpipe_socket = compat_socket_selfpipe_get_read_fd(self->selfpipe);
        ...
        /* Note that if a socket is closed between when we added it to the sets
           above and when we call select() below, it will cause the select to fail with EBADF.
           We catch this and just run around the loop again -
           the offending socket will not be added to the sets again as it's now been closed */
...
libspectrum_byte nic_w5100_read(nic_w5100_t *self, libspectrum_word reg)
{
    ...
    if (reg < 0x030) {
        ...
        b = 0xff;
        nic_w5100_error(UI_ERROR_WARNING, "w5100: reading 0x%02x from unsupported register 0x%03x\n", b, reg);
    }
...
static void w5100_write_mr(nic_w5100_t *self, libspectrum_byte b)
{
    ...
    if (b & 0x7f) {
        nic_w5100_error(UI_ERROR_WARNING, "w5100: unsupported value 0x%02x written to MR\n", b);
    }
}
...
static void w5100_write_imr(nic_w5100_t *self, libspectrum_byte b)
{
    ...
    if (b != 0xef) {
        nic_w5100_error(UI_ERROR_WARNING, "w5100: unsupported value 0x%02x written to IMR\n", b);
    }
}
...
static void w5100_write__msr(nic_w5100_t *self, libspectrum_word reg, libspectrum_byte b)
{
    ...
    if (b != 0x55) {
        nic_w5100_error(UI_ERROR_WARNING, "w5100: unsupported value 0x%02x written to %s\n", b, regname);
    }
}
...
void nic_w5100_write(nic_w5100_t *self, libspectrum_word reg, libspectrum_byte b)
{
    if (reg < 0x030) {
        ...
        nic_w5100_error(UI_ERROR_WARNING, "w5100: writing 0x%02x to unsupported register 0x%03x\n", b, reg);
    }
}
