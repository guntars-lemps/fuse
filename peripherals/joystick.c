...
/* The bit masks used by the various joysticks.
   The order is the same as the ordering of buttons in joystick.h:joystick_button (left, right, up, down, fire) */
static const libspectrum_byte kempston_mask[5] = {0x02, 0x01, 0x08, 0x04, 0x10};
static const libspectrum_byte timex_mask[5] = {0x04, 0x08, 0x01, 0x02, 0x80};

// The keys used by the Cursor joystick
static const keyboard_key_name cursor_key[5] = {KEYBOARD_5, KEYBOARD_8, KEYBOARD_7, KEYBOARD_6, KEYBOARD_0};

// The keys used by the two Sinclair joysticks
static const keyboard_key_name sinclair1_key[5] = {KEYBOARD_6, KEYBOARD_7, KEYBOARD_9, KEYBOARD_8, KEYBOARD_0};
static const keyboard_key_name sinclair2_key[5] = {KEYBOARD_1, KEYBOARD_2, KEYBOARD_4, KEYBOARD_3, KEYBOARD_5};
...
// The names of the joysticks we can emulate. Order must correspond to that of joystick.h:joystick_type_t
...
void joystick_register_startup(void)
{
    ...
    startup_manager_register(STARTUP_MANAGER_MODULE_JOYSTICK,
                             dependencies,
...
int joystick_press(int which, joystick_button button, int press)
{
    ...
    switch (which) {
        case 0:
            type = settings_current.joystick_1_output;
            break;

        case 1:
            type = settings_current.joystick_2_output;
            break;

        case JOYSTICK_KEYBOARD:
            type = settings_current.joystick_keyboard_output;
            break;
    ...
    ui_error(UI_ERROR_ERROR, "%s:joystick_press:unknown joystick type %d",  __FILE__, type);
    fuse_abort();
}
...
static void joystick_from_snapshot(libspectrum_snap *snap)
{
    ...
    for (i = 0; i < num_joysticks; i++) {
        switch (libspectrum_snap_joystick_list(snap, i)) {
            ...
            default:
                ui_error(UI_ERROR_INFO,
                         "Ignoring unsupported joystick in snapshot %s",
        ...
        if ((settings_current.joystick_keyboard_output != fuse_type) &&
            ...
            (settings_current.joystick_2_output != fuse_type) && !rzx_playback) {
        ...
        /* If the snap was configured for a Kempston joystick,
           enable our Kempston emulation in case the snap was reading from
...
static void joystick_to_snapshot(libspectrum_snap *snap)
{
    if (settings_current.joy_kempston) {
        add_joystick(snap, JOYSTICK_TYPE_KEMPSTON, LIBSPECTRUM_JOYSTICK_INPUT_NONE);
    }
    add_joystick(snap, settings_current.joystick_keyboard_output, LIBSPECTRUM_JOYSTICK_INPUT_KEYBOARD);
    add_joystick(snap, settings_current.joystick_1_output, LIBSPECTRUM_JOYSTICK_INPUT_JOYSTICK_1);
    add_joystick(snap, settings_current.joystick_2_output, LIBSPECTRUM_JOYSTICK_INPUT_JOYSTICK_2);
}
