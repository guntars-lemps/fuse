...
void upd_fdc_master_reset(upd_fdc *f)
{
    ...
    /* Caution with mirrored drives! The plus3 only use the US0 pin to select drives,
...
static int read_id(upd_fdc *f)
{
    ...
    f->status_register[1] &= ~(UPD_FDC_ST1_CRC_ERROR | UPD_FDC_ST1_MISSING_AM | UPD_FDC_ST1_NO_DATA);
...
static int seek_id(upd_fdc *f)
{
    ...
    f->status_register[2] &= ~(UPD_FDC_ST2_WRONG_CYLINDER | UPD_FDC_ST2_BAD_CYLINDER);
    r = read_id(f);
    if (r != 0) {
        return r; // not found any good id
    }
    ...
    if ((f->id_sector == f->data_register[3]) && (f->id_head == f->data_register[2])) {
...
static void cmd_result(upd_fdc *f)
{
    ...
    if (f->head_load && (f->cmd->id <= UPD_CMD_READ_ID)) {
        event_add_with_data(tstates + ((f->hut_time * machine_current->timings.processor_speed) / 1000),
                            head_event,
                            f);
    }
}
...
static void seek_step(upd_fdc *f, int start)
{
    ...
    if ((f->pcn[i] == f->ncn[i]) && (f->seek[i] == 2) && !d->tr00) { // recalibrate fail
    ...
    // There is need to seek?
    if ((f->pcn[i] == f->ncn[i]) || ((f->seek[i] == 2) && d->tr00)) { // correct position
    ...
    if (f->pcn[i] != f->ncn[i]) {    /**FIXME if d->tr00 == 1 ??? */
        fdd_step(d, ((f->pcn[i] > f->ncn[i]) ? FDD_STEP_OUT : FDD_STEP_IN));
        ...
        event_add_with_data(tstates + ((f->stp_rate * machine_current->timings.processor_speed) / 1000),
                            fdc_event,
                            f);
    }
...
static void start_read_id(upd_fdc *f)
{
    ...
    if (f->rev) {
        i = ((f->current_drive->disk.i >= f->current_drive->disk.bpt) ? 0 : f->current_drive->disk.i); // start position
        if (read_id(f) != 2) {
            f->rev = 0;
        }
        i = (f->current_drive->disk.bpt ? (((f->current_drive->disk.i - i) * 200) / f->current_drive->disk.bpt) : 200);
        if (i > 0) {
            // i * 1/20 revolution
            event_add_with_data(tstates + ((i * machine_current->timings.processor_speed) / 1000),
                                fdc_event,
                                f);
            return;
        }
    ...
    if ((f->id_mark != UPD_FDC_AM_ID) || (f->status_register[1] & UPD_FDC_ST1_CRC_ERROR)) { // not found/crc error id mark
...
static void start_read_diag(upd_fdc *f)
{
    ...
    // 2 revolution: 2 * 200 / 1000
    event_add_with_data(tstates + ((4 * machine_current->timings.processor_speed) / 10),
                        timeout_event,
                        f);
    return;
...
multi_track_next:
    ...
    if (f->first_rw || f->read_id || (f->data_register[5] > f->data_register[3])) {
        if (!f->read_id) {
            if (!f->first_rw) {
                f->data_register[3]++;
            }
        ...
        */
        ...
        if (!(f->status_register[0] & (UPD_FDC_ST0_INT_ABNORM | UPD_FDC_ST0_INT_READY))) {
            f->data_register[1]++; // next track
            f->data_register[3] = 1; // first sector
        }
...
multi_track_next:
    ...
    if (f->first_rw || f->read_id || (f->data_register[5] > f->data_register[3])) {
        ...
        d->data = ((f->del_data ? 0x00f8 : 0x00fb) | (f->mf ? 0x0000 : 0xff00)); // write data mark
        fdd_write_data(d);
        crc_add(f, d);
    } else {
        f->data_register[1]++; // next track
...
static void start_write_id(upd_fdc *f)
{
    ...
    f->main_status |= UPD_FDC_MAIN_DATAREQ | UPD_FDC_MAIN_DATA_WRITE;
    f->data_offset = 0;
    // 1/10 revolution: 1 * 200 / 1000
    event_add_with_data(tstates + ((2 * machine_current->timings.processor_speed) / 100),
                        timeout_event,
                        f);
}
...
static void head_load(upd_fdc *f)
{
    ...
    if (f->head_load) { // head already loaded
        ...
        f->head_load = 1;
        event_add_with_data(tstates + ((f->hld_time * machine_current->timings.processor_speed) / 1000),
                            fdc_event,
                            f);
    }
}
...
static void upd_fdc_event(libspectrum_dword last_tstates GCC_UNUSED, int event, void *user_data)
{
    upd_fdc *f = user_data;
...
libspectrum_byte upd_fdc_read_data(upd_fdc *f)
{
    ...
    if (!(f->main_status & UPD_FDC_MAIN_DATAREQ) || !(f->main_status & UPD_FDC_MAIN_DATA_READ)) {
        return 0xff;
    }
    ...
    if (f->state == UPD_FDC_STATE_EXE) { // READ_DATA/READ_DIAG
        ...
        if (f->speedlock > 0 && !d->do_read_weak) { // do not conflict with fdd weak reads
            ...
            } else if ((f->speedlock > 1 || f->data_offset < 64) && !(f->data_offset % 29)) {
...
void upd_fdc_write_data(upd_fdc *f, libspectrum_byte data)
{
    ...
    if ((f->main_status & UPD_FDC_MAIN_BUSY) && (f->state == UPD_FDC_STATE_EXE)) { // execution phase WRITE/FORMAT
        d = f->current_drive;
        if (f->cmd->id == UPD_CMD_WRITE_ID) { // FORMAT
            ...
            if (f->data_offset == 4) { // C, H, R, N done => format track
                ...
                if (f->mf) { // MFM
                    d->data = 0xffa1;
                    for (i = 3; i > 0; i--) { // write 3 0xa1 with clock mark
                        fdd_write_data(d); // fsdfsadfsadfdsaf
                        crc_add(f, d);
                    }
                }
    ...
    if (f->cycle == 0) { // first byte -> command
        ...
        cmd_identify(f);
        f->main_status |= UPD_FDC_MAIN_BUSY;
        /* ... A Sense Interrupt Status Command _must be sent after a Seek or Recalibrate interrupt_,
               otherwise the FDC will consider the next Command to be an invalid Command ... (i8272)
               Note: looks uPD765 should _NOT_, because The New Zealand Story does not work with this stuff
    ...
    if (f->cycle >= f->cmd->cmd_length) { // we already read all neccessery byte
        ...
        if ((f->cmd->id != UPD_CMD_SENSE_INT) && (f->cmd->id != UPD_CMD_SPECIFY) &&
            (f->cmd->id != UPD_CMD_VERSION) && (f->cmd->id != UPD_CMD_INVALID)) {
            ...
            if ((f->cmd->id == UPD_CMD_READ_DATA) || (f->cmd->id == UPD_CMD_WRITE_DATA)) {
        ...
        /* ... During the Command Phase of the SEEK operation the FDC in the FDC BUSY state,
               but during the Execution Phase it is in the NON BUSY state.
               While the FDC is in the NON BUSY state, another Seek Command may be issued,
               and in this manner paralell seek operation may be done on up to 4 Drives at once ...
           ... The ability to overlap RECALIBRATE Commands to Multiple FDDs,
               and the loss of the READY signal, as described in the SEEK Command,
               also applies to the RECALIBRATE Command ... (i8272)
        ...
        */
        if ((f->cmd->id == UPD_CMD_RECALIBRATE) || (f->cmd->id == UPD_CMD_SEEK) || (f->cmd->id == UPD_CMD_SPECIFY)) {
        ...
        if (f->cmd->id < UPD_CMD_SENSE_INT) {
            if (f->cmd->id < UPD_CMD_RECALIBRATE) { // reset status registers
                f->status_register[0] = f->status_register[1] = f->status_register[2] = 0x00;
            }
        ...
        switch (f->cmd->id) {
            ...
            case UPD_CMD_SENSE_INT:
                ...
                if ((f->seek[0] < 4) && (f->seek[1] < 4) && (f->seek[2] < 4) && (f->seek[3] < 4)) {
                    f->intrq = UPD_INTRQ_NONE; // delete INTRQ state
                }
                break;
            case UPD_CMD_RECALIBRATE:
                if (f->main_status & (1 << f->us)) {
                    break; // previous seek in progress?
                }
            ...
            case UPD_CMD_READ_DATA:
                ...
                if ((f->speedlock != -1) && !d->do_read_weak) { // do not conflict with fdd weak read
                    u = (f->data_register[2] & 0x01) + (f->data_register[1] << 1) + (f->data_register[3] << 8);
            ...
            case UPD_CMD_WRITE_ID: // FORMAT TRACK
                ...
                f->rlen = 0x80 << ((f->data_register[1] > MAX_SIZE_CODE) ? MAX_SIZE_CODE : f->data_register[1]);
                head_load(f);
                return;
            case UPD_CMD_SCAN: // & 0x0c >> 2 == 00 - equal, 10 - low, 11 - high
                f->scan = (((f->command_register & 0x0c) >> 2) == 0) ?
                           UPD_SCAN_EQ :
                           ((((f->command_register & 0x0c) >> 2) == 0x03) ? UPD_SCAN_HI : UPD_SCAN_LO);
