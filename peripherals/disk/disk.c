...
static void position_context_save(const disk_t *d, disk_position_context_t *c)
{
    c->track = d->track;
    c->clocks = d->clocks;
    c->fm = d->fm;
...
static void position_context_restore(disk_t *d, const disk_position_context_t *c)
{
    d->track = c->track;
    d->clocks = c->clocks;
    d->fm = c->fm;
...
static int id_read(disk_t *d, int *head, int *track, int *sector, int *length)
{
    ...
    while (d->i < d->bpt) {
        if ((d->track[d->i] == 0xa1) && bitmap_test(d->clocks, d->i)) { // 0xa1 with clock
            a1mark = 1;
        } else if ((d->track[d->i] == 0xfe) && (bitmap_test(d->clocks, d->i) || a1mark)) {
            // 0xfe with clock or 0xfe with 0xa1
            d->i++;
            *track = d->track[d->i++];
            *head = d->track[d->i++];
            *sector = d->track[d->i++];
...
static int datamark_read(disk_t *d, int *deleted)
{
    ...
    while (d->i < d->bpt) {
        if ((d->track[d->i] == 0xa1) && (bitmap_test(d->clocks, d->i))) { // 0xa1 with clock
            a1mark = 1;
        } else if ((d->track[d->i] >= 0xf8) && (d->track[d->i] <= 0xfe) && (bitmap_test(d->clocks, d->i) || a1mark)) {
...
static int savetrack(disk_t *d, FILE *file, int head, int track, int sector_base, int sectors, int seclen)
{
    int s;
    int del;
...
static int guess_track_geom(disk_t *d, int head, int track, int *sector_base, int *sectors, int *seclen, int *mfm)
{
    int r = 0;
    int h, t, s, sl;
    int del = 0;
...
static int check_disk_geom(disk_t *d, int *sector_base, int *sectors, int *seclen, int *mfm, int *unf)
{
...
static int gap_add(disk_t *d, int gap, int gaptype)
{
    ...
    // -------------------------------- given gap --------------------------------
    memset((d->track + d->i), g->gap,  g->len[gap]);
    d->i += g->len[gap];
    return 0;
}


static int preindex_len(disk_t *d, int gaptype) // preindex gap and index mark
{
    disk_gap_t *g = &gaps[gaptype];
...
/*
    ...
    |------------------------------------------->
        Preindex
*/
static int preindex_add(disk_t *d, int gaptype) // preindex gap and index mark
{
    disk_gap_t *g = &gaps[gaptype];
    ...
    // ------------------------------   sync    ---------------------------
    memset((d->track + d->i), g->sync, g->sync_len);
    d->i += g->sync_len;
    if (g->mark >= 0) {
        memset((d->track + d->i) , g->mark, 3);
        bitmap_set(d->clocks, d->i);
        d->i++;
        bitmap_set(d->clocks, d->i);
        d->i++;
        bitmap_set(d->clocks, d->i);
...
static int postindex_len(disk_t *d, int gaptype) // preindex gap and index mark
{
    disk_gap_t *g = &gaps[gaptype];
    return g->len[1];
}
...
static int postindex_add(disk_t *d, int gaptype) // postindex gap
{
                    ...
                    int *start_data)
{
    ...
    // ------------------------------      data      ------------------------------
    if (start_data != NULL) {
        *start_data = d->i; // record data start position
    }
    if (buffer == NULL) {
...
{
    int i, s, pos;
    int slen = calc_sectorlen(((d->density != DISK_SD) && (d->density != DISK_8_SD)), sector_length, gap);
    int idx;
...
int disk_preformat(disk_t *d)
{
    ...
    if (d->sides == 2) {
        if (trackgen(d, &buffer, 1, 0, 0xff, 1, 128, NO_PREINDEX, GAP_MINIMAL_MFM, NO_INTERLEAVE, 0xff)) {
            return DISK_GEOM;
        }

        if (trackgen(d, &buffer, 1, 2, 0xfe, 1, 128, NO_PREINDEX, GAP_MINIMAL_MFM, NO_INTERLEAVE, 0xff)) {
            return DISK_GEOM;
        }
    }

    if (trackgen(d, &buffer, 0, 0, 0xff, 1, 128, NO_PREINDEX, GAP_MINIMAL_MFM, NO_INTERLEAVE, 0xff)) {
        return DISK_GEOM;
    }

    if (trackgen(d, &buffer, 0, 2, 0xfe, 1, 128, NO_PREINDEX, GAP_MINIMAL_MFM, NO_INTERLEAVE, 0xff)) {
        return DISK_GEOM;
    }
...
// open a disk image
static int udi_read_compressed(const libspectrum_byte *buffer,
                               size_t compr_size,
                               size_t uncompr_size,
...
static int udi_write_compressed(const libspectrum_byte *buffer,
                                size_t uncompr_size,
                                size_t *compr_size,
                                libspectrum_byte **data,
                                size_t *data_size)
{
    ...
    tmp = NULL;
    error = libspectrum_zlib_compress(buffer, uncompr_size, &tmp, compr_size);
    if (error) {
        return error;
    }
...
static int udi_uncompress_tracks(disk_t *d)
{
    ...
    for (i = 0; i < (d->sides * d->cylinders); i++) {
        DISK_SET_TRACK_IDX(d, i);
        if (d->track[-1] != 0xf0) { // if not compressed
            continue;
        }
...
static int udi_compress_tracks(disk_t *d)
{
    ...
    for (i = 0; i < d->sides * d->cylinders; i++) {
        DISK_SET_TRACK_IDX(d, i);
        if (d->track[-1] == 0xf0) { // already compressed???
            continue;
        }
        tlen = UDI_TLEN(d->track[-1], d->track[-3] + (256 * d->track[-2]));
        // if fail to compress, skip ...
        if (udi_write_compressed(d->track, tlen, &clen, &data, &data_size) || (clen < 1)) {
            continue;
        }
        // if compression too large, skip...
        if ((clen > 65535) || (clen >= tlen)) {
            continue;
        }
...
static int open_udi(buffer_t *buffer, disk_t *d)
{
    ...
    if (crc != buff[eof] + (256 * buff[eof + 1]) + (65536 * buff[eof + 2]) + (16777216 * buff[eof + 3])) {
        return d->status = DISK_OPEN;
    }
    ...
    for (i = 0; buffer->index < eof; i++) {
        ...
        if (ttyp == 0x83) { // multiple read
            if (i == 0) {
                return d->status = DISK_GEOM; // cannot be first track
            }
            i--;
            bpt = 0; // not a real track
...
static int open_img_mgt_opd(buffer_t *buffer, disk_t *d)
{
    ...
    /* guess geometry of disk:
       2 * 80 * 10 * 512, 1 * 80 * 10 * 512, 1 * 40 * 10 * 512, 1 * 40 * 18 * 256, 1 * 80 * 18 * 256, 2 * 80 * 18 * 256 */
    ...
    if (d->type == DISK_IMG) { // IMG out-out
        for (j = 0; j < d->sides; j++) {
            for (i = 0; i < d->cylinders; i++) {
                if (trackgen(d, buffer, j, i, 1, sectors, seclen, NO_PREINDEX, GAP_MGT_PLUSD, NO_INTERLEAVE, NO_AUTOFILL)) {
                    return d->status = DISK_GEOM;
                }
            }
        }
        ...
        for (i = 0; i < d->cylinders; i++) {
            for (j = 0; j < d->sides; j++) {
                if (trackgen(d,
                             ...
                             NO_PREINDEX,
                             GAP_MGT_PLUSD,
                             ((d->type == DISK_MGT) ? NO_INTERLEAVE : INTERLEAVE_OPUS),
                             NO_AUTOFILL)) {
                    return d->status = DISK_GEOM;
                }
            }
...
static int open_sad(buffer_t *buffer, disk_t *d, int preindex)
{
    ...
    for (j = 0; j < d->sides; j++) {
        for (i = 0; i < d->cylinders; i++) {
            if (trackgen(d, buffer, j, i, 1, sectors, seclen, preindex, GAP_MGT_PLUSD, NO_INTERLEAVE, NO_AUTOFILL)) {
                return d->status = DISK_GEOM;
            }
        }
    }
...
static int trdos_insert_basic_file(disk_t *d, trdos_spec_t *spec, const libspectrum_byte *data, unsigned int size)
{
...
static int open_trd(buffer_t *buffer, disk_t *d)
{
    int i, j, sectors, seclen;
    ...
    if (buffer->file.length > (d->sides * d->cylinders * 16 * 256)) {
        for (i = d->cylinders + 1; i < 83; i++) {
            if ((d->sides * i * 16 * 256) >= buffer->file.length) {
                break;
            }
        }
        d->cylinders = i;
    ...
    for (i = 0; i < d->cylinders; i++) {
        for (j = 0; j < d->sides; j++) {
            if (trackgen(d, buffer, j, i, 1, sectors, seclen, NO_PREINDEX, GAP_TRDOS, INTERLEAVE_2, 0x00)) {
                return d->status = DISK_GEOM;
            }
        }
    }
...
static int open_fdi(buffer_t *buffer, disk_t *d, int preindex)
{
    ...
    for (i = 0; i < (d->cylinders * d->sides); i++) { // ALT
        ...
        bpt = postindex_len(d, GAP_MINIMAL_MFM) + (preindex ? preindex_len(d, GAP_MINIMAL_MFM) : 0) + 6; // +gap4
        bpt_fm = postindex_len(d, GAP_MINIMAL_FM) + (preindex ? preindex_len(d, GAP_MINIMAL_FM) : 0) + 3; // +gap4
...
static int open_cpc(buffer_t *buffer, disk_t *d, int preindex)
{
    ...
    for (i = 0; i < (d->sides * d->cylinders); i++) {
        ...
        if ((buffavail(buffer) < 256) || memcmp(buff, "Track-Info", 10)) { // check track header
            return d->status = DISK_OPEN;
        }
        ...
        if (i >= (d->sides * d->cylinders) || (i != ((buff[0x10] * d->sides) + buff[0x11]))) { // problem with track idx.
            return d->status = DISK_OPEN;
        }
        ...
        for (j = 0; j < buff[0x15]; j++) { // each sector
            ...
            if ((i < 84) &&( d->flag & DISK_FLAG_PLUS3_CPC)) {
                if ((j == 0) && (buff[0x1b + (8 * j)] == 6) && (seclen > 6144)) {
                ...
                } else if ((j == 1) && (plus3_fix == CPC_ISSUE_1) && (buff[0x1b + (8 * j)] == 2)) {
                    ...
                    plus3_fix = CPC_ISSUE_NONE;
                } else if ((i == 38) && (j > 0) && (plus3_fix == CPC_ISSUE_5) && (buff[0x1b + (8 * j)] != (2 - (j & 1)))) {
                    plus3_fix = CPC_ISSUE_NONE;
                }
            }
            trlen += seclen;
    ...
    for (i = 0; i < (d->sides * d->cylinders); i++) {
        ...
        for (j = 0; j < hdrb[0x15]; j++) { // each sector
            // data length in sector
            seclen = ((d->type == DISK_ECPC) ?
                      (hdrb[0x1e + (8 * j)] + (256 * hdrb[0x1f + (8 * j)])) :
                      (0x80 << hdrb[0x1b + (8 * j)]));
            ...
            if ((i < 84) && (fix[i] == CPC_ISSUE_1) && (j == 0)) { // 6144
                data_add(d,
                         ...
                         NULL,
                         seclen,
                         ((hdrb[0x1d + (8 * j)] & 0x40) ? DDAM : NO_DDAM),
                         gap,
                         (((hdrb[0x1c + (8 * j)] & 0x20) && (hdrb[0x1d + (8 * j)] & 0x20)) ? CRC_ERROR : CRC_OK),
                         0x00,
                         NULL);
                ...
                if (seclen > idlen) { // weak sector with multiple copy
                    ...
                    buffer->index += ((seclen / idlen) - 1) * idlen; // ((N * len) / len - 1) * len
                }
            }
            if (seclen % 0x100) { // every? 128/384/...byte length sector padded
                sector_pad++;
...
static int open_scl(buffer_t *buffer, disk_t *d)
{
...
static int open_td0(buffer_t *buffer, disk_t *d, int preindex)
{
    ...
    mfm_old = ((buff[5] & 0x80) ? 0 : 1); /* (t)d0notes say: may older teledisk
                                            indicate the SD on high bit of data rate */
    d->sides = buff[9]; // 1 or 2
    if ((d->sides < 1) || (d->sides > 2)) {
        return d->status = DISK_GEOM;
    }
    // skip comment block if any
    data_offset = track_offset = 12 + ((buff[7] & 0x80) ? (10 + buff[14] + (256 * buff[15])) : 0);
    ...
    while (1) {
        ...
        bpt = postindex_len(d, ((mfm_old || mfm) ? GAP_MINIMAL_FM : GAP_MINIMAL_MFM)) +
              (preindex ? preindex_len(d, ((mfm_old || mfm) ? GAP_MINIMAL_FM : GAP_MINIMAL_MFM)) : 0) +
              ((mfm_old || mfm) ? 6 : 3);
        for (s = 0; s < sectors; s++) {
            ...
            if (!(buff[4] & 0x30)) { // only if we have data
                ...
                bpt += calc_sectorlen((mfm_old || mfm),
                                      (0x80 << buff[3]),
                    ...
                    case 2: // Run Length Encoded data
                        ...
                        for (i = 0; i < seclen;) { // fill buffer
                            ...
                            if (hdrb[9] == 0) { // raw bytes
                                // too many data bytes
                                if (((i + hdrb[10]) > seclen) || (buffread((uncomp_buff + i), hdrb[10], buffer) != 1)) {
                                    libspectrum_free(uncomp_buff);
                                    return d->status = DISK_OPEN;
                                }
                                i += hdrb[10];
...
static int disk_open2(disk_t *d, const char *filename, int preindex)
{
    ...
    error = libspectrum_identify_file_raw(&type, filename, buffer.file.buffer, buffer.file.length);
    if (error) {
        return d->status = DISK_OPEN;
    }
    d->type = DISK_TYPE_NONE;
...
int disk_merge_sides(disk_t *d, disk_t *d1, disk_t *d2, int autofill)
{
    ...
    if ((d1->sides != 1) || (d2->sides != 1) || (d1->bpt != d2->bpt) ||
...
int disk_open(disk_t *d, const char *filename, int preindex, int merge_disks)
{
    char *filename2;
    char c = ' ';
    int l, g = 0, pos = 0;
    ...
    while (l) { // [Ss]ide[_][abAB12][_.]
        if ((g == 0) && ((*filename2 == '.') || (*filename2 == '_') || (*filename2 == ' '))) {
            g++;
        } else if ((g == 1) && ((*filename2 == '1') || (*filename2 == 'a') || (*filename2 == 'A'))) {
            g++;
            pos = filename2 - filename;
            c = *filename2 + 1; // 1->2, a->b, A->B
        } else if ((g == 1) && ((*filename2 == '2') || (*filename2 == 'b') || (*filename2 == 'B'))) {
            g++;
            pos = filename2 - filename;
            c = *filename2 - 1; // 2->1, b->a, B->A
        } else if ((g == 2) && ((*filename2 == '_') || (*filename2 == ' '))) {
            g++;
        } else if ((g == 3) && (l >= 5) && (!memcmp((filename2 - 3), "Side", 4) || !memcmp((filename2 - 3), "side", 4))) {
            g++;
            break;
        } else {
            g = 0;
    ...
    *(filename2 + pos) = c;
    ...
    if (settings_current.disk_ask_merge && !ui_query("Try to merge 'B' side of this disk?")) {
...
static int write_d40_d80(FILE *file, disk_t *d)
{
    ...
    if (((d->type == DISK_D40) && (cyl > 43)) || ((d->type == DISK_D80) && (cyl > 83))) {
        return d->status = DISK_GEOM;
    }
    for (i = 0; i < cyl; i++) {
        for (j = 0; j < d->sides; j++) {
...
static int write_scl(FILE *file, disk_t *d)
{
    ...
    if ((entries > 128) || (d->track[d->i + 231] != 0x10) ||
        ((d->track[d->i + 227] != 0x16) && (d->track[d->i + 227] != 0x17) &&
         (d->track[d->i + 227] != 0x18) && (d->track[d->i + 227] != 0x19)) || (d->track[d->i] != 0)) {
        return d->status = DISK_GEOM;
    }
...
static int write_log(FILE *file, disk_t *d)
{
    int i, j, k, del, rev;
    int h, t, s, b;
    char str[17];
        ...
        for (i = 0; i < d->sides; i++) {
            ...
            while (id_read(d, &h, &t, &s, &b)) {
                ...
                while (k < b) {
                    ...
                    str[k & 0x0f] = ((d->track[d->i] >= 32) && (d->track[d->i] < 127)) ? d->track[d->i] : '.';
                    k++;
                    if (!(k % 16)) {
                        fprintf(file, " | %s\n", str);
                    }
