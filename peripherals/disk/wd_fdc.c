...
static void wd_fdc_event(libspectrum_dword last_tstates, int event, void *user_data);
...
void wd_fdc_master_reset(wd_fdc *f)
{
    ...
    f->read_id = 0;
    f->hlt = 1;
    if (!(f->flags & WD_FLAG_NOHLT) && (f->hlt_time > 0)) {
        f->hlt = 0;
    }
    f->intrq = 0;
    f->datarq = 0;
...
{
    if (((f->type == WD1770) || (f->type == WD1772)) && (f->status_register & WD_FDC_SR_MOTORON)) {
        // 10 rev: 10 * 200 / 1000
        event_add_with_data(tstates + (2 * machine_current->timings.processor_speed), motor_off_event, f);
    }

    if (((f->type == WD1773) || (f->type == FD1793) || (f->type == WD2797)) && f->head_load) {
        // 15 revolution: 15 * 200 / 1000
        event_add_with_data(tstates + (3 * machine_current->timings.processor_speed), motor_off_event, f);
    }
    if (f->intrq != 1) {
        f->intrq = 1;
        if (f->set_intrq) {
...
static void wd_fdc_seek_verify_read_id(wd_fdc *f)
{
    ...
    if (f->id_mark == WD_FDC_AM_NONE) {
        while (f->rev) {
            ...
            if (i > 0) {
                // i * 1/20 revolution
                event_add_with_data(tstates + ((i * machine_current->timings.processor_speed) / 1000), fdc_event, f);
                return;
...
static void wd_fdc_seek_verify(wd_fdc *f)
{
    ...
    if ((f->type == WD1773) || (f->type == FD1793) || (f->type == WD2797)) {
        if (!f->hlt) {
            // sample every 5 ms
            event_add_with_data(tstates + ((5 * machine_current->timings.processor_speed) / 1000), fdc_event, f);
            return;
        }
        if (f->head_load) {
...
type_i_noupdate:
        ...
        if (d->tr00 && (f->direction == FDD_STEP_OUT)) {
            ...
            event_remove_type(fdc_event);
            event_add_with_data(tstates + ((f->rates[b & 0x03] * machine_current->timings.processor_speed) / 1000),
                                fdc_event,
                                f);
            return;
        }
...
type_i_verify:
    ...
    if (b & 0x04) {
        ...
        if ((f->type == WD1773) || (f->type == FD1793) || (f->type == WD2797)) {
            ...
            event_remove_type(fdc_event);
            // 15ms
            event_add_with_data(tstates + ((15 * machine_current->timings.processor_speed) / 1000), fdc_event, f);
        }
        ...
        if (((f->type == WD1770) || (f->type == WD1772)) && !(f->status_register & WD_FDC_SR_MOTORON)) {
            ...
            event_add_with_data(tstates + ((12 * machine_current->timings.processor_speed) / 10), fdc_event, f);
            return;
        }
...
static void wd_fdc_type_ii_seek(wd_fdc *f)
{
    ...
    event_add_with_data(tstates + machine_current->timings.processor_speed, timeout_event, f);
}
...
static void wd_fdc_type_iii(wd_fdc *f)
{
    ...
    event_add_with_data(tstates + ((2 * 20 * machine_current->timings.processor_speed) / 100), timeout_event, f);
}


static void wd_fdc_event(libspectrum_dword last_tstates GCC_UNUSED, int event, void *user_data)
{
    wd_fdc *f = user_data;
    fdd_t *d = f->current_drive;
    ...
    if (event == timeout_event) {
        if ((f->state == WD_FDC_STATE_READ) || (f->state == WD_FDC_STATE_WRITE) ||
            (f->state == WD_FDC_STATE_READTRACK) || (f->state == WD_FDC_STATE_WRITETRACK) ||
    ...
    if ((((f->type == WD1770) || (f->type == WD1772)) &&
         (f->status_register & WD_FDC_SR_MOTORON) && (f->status_type == WD_FDC_STATUS_TYPE1)) ||
        (((f->type == WD1773) || (f->type == FD1793) || (f->type == WD2797)) &&
         ((f->state == WD_FDC_STATE_SEEK) || (f->state == WD_FDC_STATE_SEEK_DELAY)) && f->head_load)) {
        f->status_register |= WD_FDC_SR_SPINUP;
    }
    ...
    if (f->read_id) {
        ...
        wd_fdc_type_ii(f);
    } else if (((f->state == WD_FDC_STATE_READTRACK) || (f->state == WD_FDC_STATE_READID) ||
...
static int wd_fdc_spinup(wd_fdc *f, libspectrum_byte b)
{
    ...
    if (delay) {
        event_remove_type(fdc_event);
        event_add_with_data(tstates + ((delay * machine_current->timings.processor_speed) / 1000), fdc_event, f);
        return 1;
    }
    return 0;
}
...
void wd_fdc_cr_write(wd_fdc *f, libspectrum_byte b)
{
    ...
    if (!(b & 0x80)) { // Type I
        ...
        f->status_register &= ~(WD_FDC_SR_WRPROT | WD_FDC_SR_RNF | WD_FDC_SR_IDX_DRQ| WD_FDC_SR_LOST | WD_FDC_SR_SPINUP);
        ...
        f->state = (b & 0x20) ?
                   ((b & 0x10) ? WD_FDC_STATE_WRITETRACK : WD_FDC_STATE_READTRACK) :
                   WD_FDC_STATE_READID;
        f->status_type = WD_FDC_STATUS_TYPE2;
...
libspectrum_byte wd_fdc_dr_read(wd_fdc *f)
{
        ...
        if (d->data > 0xff) { // no data
            ...
            if (f->data_offset == f->sector_length) { // read the CRC
                ...
                if ((f->crc == 0x0000) && f->data_multisector) {
                    ...
                    event_add_with_data(tstates + machine_current->timings.processor_speed, timeout_event, f);
                     // 20 ms delay
                    event_add_with_data(tstates + ((2 * machine_current->timings.processor_speed) / 100), fdc_event, f);
                } else {
                    f->status_register &= ~WD_FDC_SR_BUSY;
                    if (f->crc == 0x0000) {
                        f->status_register &= ~WD_FDC_SR_CRCERR;
    ...
    if ((f->flags & WD_FLAG_DRQ) && (f->status_register & WD_FDC_SR_BUSY)) { // we need a next datarq
        // 30 us delay
        event_add_with_data(tstates + ((30 * machine_current->timings.processor_speed) / 1000000), fdc_event, f);
    }
