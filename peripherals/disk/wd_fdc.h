...
enum {
    ...
    WD_FLAG_BETA128 = 1 << 0,
    // The Opus Discovery needs a pulse of the DRQ (data request) line for every byte transferred.
    WD_FLAG_DRQ = 1 << 1,
    // The MB-02+ provides a READY signal to FDC instead of FDD, so we use 'extra_signal' for this
    WD_FLAG_RDY = 1 << 2,
...
typedef struct wd_fdc {
    ...
    int hlt_time; /* When a logic high is found on the HLT input the head is assumed to be enganged.
                     It is typically derived from a 1 shot triggered by HLD
                     if hlt_time > 0 it means trigger time in ms,
                     if = 0 then hlt should be set with wd_fdc_set_hlt()  */
