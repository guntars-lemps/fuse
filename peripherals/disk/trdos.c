...
int trdos_read_fat(trdos_boot_info_t *info, const libspectrum_byte *sectors, unsigned int seclen)
{
    int i, j, error;
    ...
    for (i = 0; i < 8; i++) {
        ...
        /* Note: some TR-DOS versions like 5.04T have a turbo format with
           ...
           The SCL/TRD image formats can't specify a format mode and
           Fuse loads the sectors as interleaved: 1, 9, 2, 10, 3, ...
        */
        ...
        for (j = 0; j < 16; j++) {
            ...
            if ((entry.filename[0] > 0x01) && (entry.file_extension == 'B')) {
                ...
                // Boot file
                if (!info->have_boot_file && !strncmp((const char *)entry.filename, "boot    ", 8)) {
