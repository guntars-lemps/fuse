...
void fdd_register_startup(void)
{
    ...
    startup_manager_register(STARTUP_MANAGER_MODULE_FDD,
                             dependencies,
...
static void fdd_set_data(fdd_t *d, int fact)
{
    ...
    if (d->unreadable || ((d->disk.sides == 1) && (head == 1)) || (d->c_cylinder >= d->disk.cylinders)) {
        d->disk.track = NULL;
        d->disk.clocks = NULL;
        d->disk.fm = NULL;
        d->disk.weak = NULL;
    ...
    if (fact > 0) {
        /* this generate a bpt/fact +-10% triangular distribution skip in bytes i know,
           we should use the higher bits of rand(), but we not keen on _real_ (pseudo)random numbers... ;)
        */
        d->disk.i += (d->c_bpt / fact) + (((d->c_bpt * ((rand() % 10) + (rand() % 10) - 9)) / fact) / 100);
...
void fdd_motoron(fdd_t *d, int on)
{
    ...
    ui_statusbar_update(UI_STATUSBAR_ITEM_DISK,
                        ((fdd_motor > 0) ? UI_STATUSBAR_STATE_ACTIVE : UI_STATUSBAR_STATE_INACTIVE));
    /*
    TEAC FD55 Spec:
    ...
    */
    ...
    if (on) {
        ...
        event_add_with_data(tstates + ((4 * machine_current->timings.processor_speed) / 10), motor_event, d);
    }
    if (d->loaded) { // index rotating
        event_add_with_data(tstates + (((d->index_pulse ? 10 : 190) * machine_current->timings.processor_speed) / 1000),
                            index_event,
                            d);
    } else {
        // 1.5 revolution
        event_add_with_data(tstates + ((3 * machine_current->timings.processor_speed) / 10), motor_event, d);
    }
}
...
int fdd_load(fdd_t *d, int upsidedown)
{
    ...
    if ((d->disk.sides < 0) || (d->disk.sides > 2) || (d->disk.cylinders < 0) || (d->disk.cylinders > FDD_MAX_TRACK)) {
        return d->status = FDD_GEOM;
    }
...
static void fdd_event(libspectrum_dword last_tstates, int event, void *user_data)
{
    ...
    if (d->motoron & d->loaded) // keep rotating
        event_add_with_data(last_tstates + (((d->index_pulse ? 10 : 190) * machine_current->timings.processor_speed) / 1000),
                            index_event,
                            d);
}
