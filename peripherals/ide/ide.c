...
static int ide_insert_file(libspectrum_ide_channel *channel,
                           libspectrum_ide_unit unit,
                           const char *filename,
...
int ide_init(libspectrum_ide_channel *channel,
             char *master_setting,
             ui_menu_item master_menu_item,
             char *slave_setting,
             ui_menu_item slave_menu_item)
{
    int error;
    ...
    if (master_setting) {
        error = ide_insert_file(channel, LIBSPECTRUM_IDE_MASTER, master_setting, master_menu_item);
        if (error) {
            return error;
        }
    }
    ...
    if (slave_setting) {
        error = ide_insert_file(channel, LIBSPECTRUM_IDE_SLAVE, slave_setting, slave_menu_item);
        if (error) {
            return error;
        }
    }
...
int ide_master_slave_insert(libspectrum_ide_channel *channel,
                            libspectrum_ide_unit unit,
                            const char *filename,
                            char **master_setting,
                            ui_menu_item master_menu_item,
...
int ide_insert(const char *filename,
               libspectrum_ide_channel *chn,
               libspectrum_ide_unit unit,
               char **setting,
               ui_menu_item item)
{
    ...
    // Remove any currently inserted disk; abort if we want to keep the current disk
    if (*setting) {
...
int ide_master_slave_eject(libspectrum_ide_channel *channel,
                           libspectrum_ide_unit unit,
                           char **master_setting,
                           ui_menu_item master_menu_item,
                           char **slave_setting,
...
int ide_eject_mass_storage(int (*is_dirty_fn)(void *context),
...
int ide_eject(libspectrum_ide_channel *chn, libspectrum_ide_unit unit, char **setting, ui_menu_item item)
{
    struct eject_fn_context ctx = {chn, unit};
    return ide_eject_mass_storage(dirty_fn_wrapper,
                                  commit_fn_wrapper,
                                  eject_fn_wrapper,
                                  &ctx,
