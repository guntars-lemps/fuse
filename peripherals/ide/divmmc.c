...
static int divmmc_init(void *context)
{
    ...
    if (settings_current.divmmc_file) {
        ...
        error = libspectrum_mmc_insert(card, settings_current.divmmc_file);
        if (error) {
            return error;
        }
    ...
    divmmc_state = divxxx_alloc("DivMMC EPROM",
                                DIVMMC_PAGES,
                                "DivMMC RAM",
                                event_type_string,
    ...
    debugger_system_variable_register(debugger_type_string,
                                      control_register_detail_string,
...
void divmmc_register_startup(void)
{
    ...
    startup_manager_register(STARTUP_MANAGER_MODULE_DIVMMC,
                             dependencies,
...
int divmmc_insert(const char *filename)
{
    ...
    // Remove any currently inserted card; abort if we want to keep the current card
    if (settings_current.divmmc_file) {
        if (mmc_eject(card)) {
            return 0;
        }
...
static void divmmc_from_snapshot(libspectrum_snap *snap)
{
    ...
    settings_current.divmmc_wp = libspectrum_snap_divmmc_eprom_writeprotect(snap);
    divxxx_control_write_internal(divmmc_state, libspectrum_snap_divmmc_control(snap));

    if (libspectrum_snap_divmmc_eprom(snap, 0)) {
        memcpy(divxxx_get_eprom(divmmc_state), libspectrum_snap_divmmc_eprom(snap, 0), DIVMMC_PAGE_LENGTH);
    }
...
static void divmmc_to_snapshot(libspectrum_snap *snap)
{
    ...
    libspectrum_snap_set_divmmc_eprom_writeprotect(snap, settings_current.divmmc_wp);
