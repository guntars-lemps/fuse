...
// see printer_parallel_strobe_write() comment; must be less than one frame's worth.
...
static libspectrum_byte printer_parallel_read(libspectrum_word port GCC_UNUSED, libspectrum_byte *attached);
...
static int printer_zxp_open_file(void)
{
    ...
    if ((tmpf = fopen(settings_current.printer_graphics_filename,"rb")) != NULL) {
        ...
        if ((fread(buf, 1, sizeof(buf), tmpf) == sizeof(buf)) &&
            ...
            /* looks good so far, but now we need to check that the height field looks ok.
               It should be all-spaces until we get a digit, then all-digits.
            */
            for (f = 0; f < 10; f++, ptr++) {
    ...
    if ((printer_graphics_file = fopen(settings_current.printer_graphics_filename, (overwrite ? "wb" : "r+b"))) == NULL) {
        ui_error(UI_ERROR_ERROR, "Couldn't open '%s', graphics printout disabled", settings_current.printer_graphics_filename);
    ...
    if (overwrite) {
        ...
        if (fseek(printer_graphics_file, (strlen(pbmstart) + 10 + 1 + ((256 / 8) * zxpheight)), SEEK_SET) != 0) {
            ui_error(UI_ERROR_ERROR, "Couldn't seek on file, graphics printout disabled");
...
static int printer_text_open_file(void)
{
    ...
    if ((printer_text_file = fopen(settings_current.printer_text_filename,"a")) == NULL) {
        ui_error(UI_ERROR_ERROR, "Couldn't open '%s', text printout disabled", settings_current.printer_text_filename);
...
/* this is the output routine for the `proper' printers
   (those connected via serial/parallel ports), and for the text-parsing ZX Printer mode.
 */
...
static void printer_zxp_update_header(void)
{
    ...
    if (fseek(printer_graphics_file, strlen("P4\n256 "), SEEK_SET) != 0) {
        ...
        /* I originally had spaces after the image height, but that actually
           breaks the format as defined in pbm(5) (not to mention breaking when read by zgv :-)).
           So they're now before the height.
        */
        fprintf(printer_graphics_file, "%10d", zxpheight);
    }

    if (fseek(printer_graphics_file, pos, SEEK_SET) != 0) {
        ui_error(UI_ERROR_ERROR, "Couldn't re-seek on file, graphics printout disabled");
        fclose(printer_graphics_file);
        printer_graphics_file = NULL;
        printer_graphics_enabled = 0;
    }
...
/* currently just increase a frame counter we need for ZX Printer.
 ...
 */
static libspectrum_byte printer_zxp_read(libspectrum_word port GCC_UNUSED, libspectrum_byte *attached)
{
    ...
    *attached = 0xff; // TODO: check this
    ...
    if (!zxpspeed) {
        ...
        while (x > 320) {
            // if we are on another line, find out where we are
            pix = -1;
            x -= 384;
            if (sp) {
                x = (x + 64) * cpp;
...
/* called when writing to the AY port which deals with RS232 output.
   Assuming that the port gets written for every bit sent
   (which seems likely; certainly the ROMs do it)
   means we can use a bps-independent approach.
*/
...
/* The +2A/+3 ROM writes the strobe the wrong way, which doesn't work on some printers.
   But some programs (notably Ian Collier's 48K Disk BASIC)
   write it the *right* way. This means we can't treat either edge *alone*
   as a signal to print, otherwise we'll print a junk byte when 48KDB starts up.
   Realistically, we have to look for two close together.

   The Centronics spec seems to require that the strobe `goes low' for
   a minimum of 1us to give the printer time to read the data.
   But that's not even a NOP's worth on the Speccy, so we need to allow for longer;
...
void printer_parallel_strobe_write(int on)
{
    ...
    if ((old_on && !on) || (!old_on && on)) {
        ...
        if (!second_edge) {
            /* the ROM also seems to assume the printer is reading at the first edge,
               breaking the spec (sigh). So we need to save current data or we lose CRs in LPRINT/LLIST.
            */
...
void printer_parallel_write(libspectrum_word port GCC_UNUSED, libspectrum_byte b)
{
    if (!settings_current.printer) {
        return;
    }
...
void printer_register_startup(void)
{
    ...
    startup_manager_register(STARTUP_MANAGER_MODULE_PRINTER,
                             dependencies,
