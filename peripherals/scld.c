...
static int scld_init(void *context)
{
...
void scld_register_startup(void)
{
    ...
    startup_manager_register(STARTUP_MANAGER_MODULE_SCLD,
                             dependencies,
...
static libspectrum_byte scld_dec_read(libspectrum_word port GCC_UNUSED, libspectrum_byte *attached)
{
...
void scld_dec_write(libspectrum_word port GCC_UNUSED, libspectrum_byte b)
{
    ...
    /* If we changed the active screen, or change the colour in hires mode,
       update the critical region and mark the entire display file as
    ...
    if ((new_dec.mask.scrnmode != old_dec.mask.scrnmode) ||
        ...
        (new_dec.name.hires && (new_dec.mask.hirescol != old_dec.mask.hirescol))) {
...
static void scld_reset(int hard_reset GCC_UNUSED)
{
    scld_last_dec.byte = 0;
}


void scld_hsr_write(libspectrum_word port GCC_UNUSED, libspectrum_byte b)
{
    scld_last_hsr = b;
...
static libspectrum_byte scld_hsr_read(libspectrum_word port GCC_UNUSED, libspectrum_byte *attached)
{
    *attached = 0xff; // TODO: check this
...
static void scld_dock_exrom_from_snapshot(memory_page *dest, int page_num, int writable, void *source)
{
    int i;
...
static void scld_to_snapshot(libspectrum_snap *snap)
{
    ...
    if (dck_active) {
        ...
        for (i = 0; i < 8; i++) {
            ...
            memory_page *dock_base = &timex_dock[i * MEMORY_PAGES_IN_8K];
            int j;
...
void scld_home_map_16k(libspectrum_word address, memory_page source[], int page_num)
{
    int i;
...
// Set contention for SCLD, contended in home, Dock and Exrom in the 0x4000 - 0x7FFF range
