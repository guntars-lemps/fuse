...
static void do_acceleration(void)
{
    if (length_known1) {
...
static acceleration_mode_t acceleration_detector(libspectrum_word pc)
{
    int state = 0, count = 0;
    while (1) {
        libspectrum_byte b = readbyte_internal(pc);
        pc++;
        count++;
        switch (state) {
            case 0:
                switch (b) {
                    case 0x03:
                        state = 28;
                        break; // Data byte of JR NZ, ... - Alkatraz
                    case 0x04:
                        state = 1;
                        break; // INC B - Many loaders
                    default:
                        state = 13;
                        break; // Possible Digital Integration
                }
                break;
            case 1:
                switch (b) {
            ...
            case 12:
                ...
                break; // Digital Integration loader
            case 13:
                state = 14;
                break; // Possible Digital Integration
            case 14:
                switch (b) {
                    case 0x05:
                        state = 15;
                        break; // DEC B - Digital Integration
                    default:
                        return ACCELERATION_MODE_NONE;
                }
                break;
            ...
            case 29:
                state = 30;
                break; // First data byte of JP
            case 30:
                state = 31;
                break; // Second data byte of JP
            case 31:
                switch (b) {
                    case 0xdb:
                        state = 32;
            ...
            case 39:
                ...
                break; // "Variant" Alkatraz
            case 40:
                switch (b) {
                    case 0x01:
                        state = 41;
                        break; // Data byte of JR NZ
                    default:
                        return ACCELERATION_MODE_NONE;
                }
                break;
...
static void check_for_acceleration(void)
{
...
void loader_detect_loader(void)
{
    ...
    if (settings_current.accelerate_loader && tape_is_playing() && !rzx_recording) {
