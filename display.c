...
libspectrum_dword display_last_screen[DISPLAY_SCREEN_WIDTH_COLS * DISPLAY_SCREEN_HEIGHT];
...
static libspectrum_word display_dirty_ytable[DISPLAY_WIDTH_COLS * DISPLAY_HEIGHT];
static libspectrum_word display_dirty_xtable[DISPLAY_WIDTH_COLS * DISPLAY_HEIGHT];
...
static libspectrum_word display_dirty_ytable2[DISPLAY_WIDTH_COLS * DISPLAY_HEIGHT_ROWS];
static libspectrum_word display_dirty_xtable2[DISPLAY_WIDTH_COLS * DISPLAY_HEIGHT_ROWS];
...
// be redisplayed. Bit 0 corresponds to pixels 0-7, bit 39 to pixels 311-319.
...
static void display_get_attr(int x, int y, libspectrum_byte *ink, libspectrum_byte *paper);
...
static struct border_change_t *alloc_change(void)
{
    static int border_changes_size = 0;

    if (border_changes_size == border_changes_last) {
        border_changes_size += 10;
        border_changes = libspectrum_renew(struct border_change_t, border_changes, border_changes_size);
    }
    return border_changes + border_changes_last++;
}


static int add_border_sentinel(void)
{
    struct border_change_t *sentinel = alloc_change();

    sentinel->x = sentinel->y = 0;
    sentinel->colour = scld_last_dec.name.hires ? display_hires_border : display_lores_border;
...
int display_init(int *argc, char ***argv)
{
    ...
    for (i = 0; i < 3; i++) {
        for (j = 0; j < 8; j++) {
            for (k = 0; k < 8; k++) {
                  display_line_start[(64 * i) + (8 * j) + k] = 32 * ((64 * i) + j + (k * 8));
            }
        }
    }
    ...
    display_last_border = scld_last_dec.name.hires ? display_hires_border : display_lores_border;
...
void display_dirty_timex(libspectrum_word offset)
{
    switch (scld_last_dec.mask.scrnmode) {
        ...
        case HIRESATTRALTD: // strange mode using second screen
            if ((offset < 0x2000) || (offset >= 0x3b00)) {
                break;
            }
        ...
        case HIRES: // hires mode
            ...
            if ((offset >= 0x1800) && (offset < 0x2000)) {
                break;
            }
            if (offset >= 0x2000) {
                offset -= ALTDFILE_OFFSET;
            }
            display_dirty8(offset);
            break;
...
// Get the attribute byte or equivalent for the eight pixels starting at ((8 * x), y)
static inline libspectrum_byte display_get_attr_byte(int x, int y)
{
...
static void update_dirty_rects(void)
{
    int start, y;
...
void display_write_if_dirty_timex(int x, int y)
{
    ...
    last_chunk_detail = (display_flash_reversed << 24) | (mode_data << 16) | (data2 << 8) | data;
...
static inline void pentagon_16c_get_colour(libspectrum_byte data, libspectrum_byte *colour1, libspectrum_byte *colour2)
{
...
void display_write_if_dirty_pentagon_16_col(int x, int y)
{
    ...
    // We need to read the pixels from the appropriate two pages and write them out to the frame buffer
...
static void copy_critical_region_line(int y, int x, int end)
{
    ...
    while (dirty) {
        ...
        // Walk to the end of the dirty region, writing the bytes to the drawing area along the way
        do {
...
static void copy_critical_region(int beam_x, int beam_y)
{
    if (critical_region_y == beam_y) {
        copy_critical_region_line(critical_region_y, critical_region_x, beam_x);
    } else {
        copy_critical_region_line(critical_region_y++, critical_region_x, DISPLAY_WIDTH_COLS);
        for (;critical_region_y < beam_y; critical_region_y++) {
            copy_critical_region_line(critical_region_y, 0, DISPLAY_WIDTH_COLS);
        }
...
static inline void get_beam_position(int *x, int *y)
{
    ...
    *y = (tstates - machine_current->line_times[0]) / machine_current->timings.tstates_per_line;
...
void display_update_critical(int x, int y)
{
    ...
    if ((y < beam_y) || ((y == beam_y) && (x < beam_x))) {
        copy_critical_region(beam_x, beam_y);
    }
}


// Mark the 8-pixel chunk at (x,y) as maybe dirty and update the critical region as appropriate
static inline void display_dirty_chunk(int x, int y)
{
...
static void display_dirty8(libspectrum_word offset)
{
    int x, y;
...
static void display_dirty64(libspectrum_word offset)
{
    int i, x, y;
...
// Get the attributes for the eight pixels starting at ((8 * x), y)
static void display_get_attr(int x, int y, libspectrum_byte *ink, libspectrum_byte *paper)
{
...
static void push_border_change(int colour)
{
    int beam_x, beam_y;
    ...
    if (beam_y >= DISPLAY_SCREEN_HEIGHT) {
        return;
    }
    ...
    if (beam_x > DISPLAY_SCREEN_WIDTH_COLS) {
        beam_x = DISPLAY_SCREEN_WIDTH_COLS;
    }
    if (beam_y < 0) {
        beam_y = 0;
    }
...
static void check_border_change(void)
{
    if (scld_last_dec.name.hires && (display_hires_border != display_last_border)) {
        push_border_change(display_hires_border);
        display_last_border = display_hires_border;
    } else if (!scld_last_dec.name.hires && (display_lores_border != display_last_border)) {
...
static void border_change_write(int y, int start, int end, int colour)
{
    if ((y < DISPLAY_BORDER_HEIGHT) || (y >= (DISPLAY_BORDER_HEIGHT + DISPLAY_HEIGHT))) {
    ...
    if (start < DISPLAY_BORDER_WIDTH_COLS) {
        int left_end = (end > DISPLAY_BORDER_WIDTH_COLS) ? DISPLAY_BORDER_WIDTH_COLS : end;
...
static void border_change_line_part(int y, int start, int end, int colour)
{
    border_change_write(y, start, end, colour);
}


static void border_change_line(int y, int colour)
{
    border_change_write(y, 0, DISPLAY_SCREEN_WIDTH_COLS, colour);
}


static void do_border_change(struct border_change_t *first, struct border_change_t *second)
{
    if (first->x) {
        if (first->x != DISPLAY_SCREEN_WIDTH_COLS) {
            border_change_line_part(first->y, first->x, DISPLAY_SCREEN_WIDTH_COLS, first->colour);
        }
        // Don't extend region past the end of the screen
        if (first->y < (DISPLAY_SCREEN_HEIGHT - 1)) {
            first->y++;
        }
    }
...
// Take account of all the border colour changes which happened in this frame
