...
static int slt_screen_level; // The level of the screenshot. Not used for anything AFAIK
...
void slt_register_startup(void)
{
    ...
    startup_manager_register(STARTUP_MANAGER_MODULE_SLT,
                             dependencies,
...
static void slt_from_snapshot(libspectrum_snap *snap)
{
    ...
    for (i = 0; i < 256; i++) {
        ...
        if (slt_length[i]) {
            ...
            slt[i] = memory_pool_allocate(slt_length[i] * sizeof(libspectrum_byte));

            memcpy(slt[i], libspectrum_snap_slt(snap, i), libspectrum_snap_slt_length(snap, i));
        }
    }
