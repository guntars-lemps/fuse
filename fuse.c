...
static void libspectrum_register_startup(void)
{
    ...
    startup_manager_register(STARTUP_MANAGER_MODULE_LIBSPECTRUM,
                             dependencies,
                             ARRAY_SIZE(dependencies),
                             fuse_libspectrum_init,
                             NULL,
                             NULL);
}
...
static void setuid_register_startup()
{
    startup_manager_module dependencies[] = {
        STARTUP_MANAGER_MODULE_DISPLAY,
        STARTUP_MANAGER_MODULE_LIBSPECTRUM
    };
    startup_manager_register(STARTUP_MANAGER_MODULE_SETUID,
                             dependencies,
                             ARRAY_SIZE(dependencies),
                             setuid_init,
                             NULL,
...
static int fuse_init(int argc, char **argv)
{
    ...
    // Seed the bad but widely-available random number generator with the current time
    ...
    if (setup_start_files(&start_files)) {
        return 1;
    }
    if (parse_nonoption_args(argc, argv, first_arg, &start_files)) {
        return 1;
    }
    if (do_start_files(&start_files)) {
        return 1;
    }
    ...
    if (ui_mouse_present) {
        ui_mouse_grabbed = ui_mouse_grab(1);
    }
...
static int creator_init(void *context)
{
    ...
    sscanf(VERSION, "%u.%u.%u.%u", &version[0], &version[1], &version[2], &version[3]);
    ...
    if (!gcrypt_version) {
        gcrypt_version = "not available";
    }

    snprintf(custom, CUSTOM_SIZE, "gcrypt: %s\nlibspectrum: %s\nuname: %s", gcrypt_version, libspectrum_version(), osname);

    error = libspectrum_creator_set_custom(fuse_creator, (libspectrum_byte*)custom, strlen(custom));
    if (error) {
...
{
    startup_manager_module dependencies[] = {STARTUP_MANAGER_MODULE_SETUID};
    startup_manager_register(STARTUP_MANAGER_MODULE_CREATOR,
                             dependencies,
                             ARRAY_SIZE(dependencies),
                             creator_init,
                             NULL,
...
static void fuse_show_copyright(void)
{
    printf("\n");
    fuse_show_version();
    printf(FUSE_COPYRIGHT "; see the file\n"
...
int fuse_emulation_pause(void)
{
    ...
    // If we were already paused, just return. In any case, increment the pause count
    ...
    // If we had sound enabled (and hence doing the speed regulation), turn it off
    sound_pause();
...
int fuse_emulation_unpause(void)
{
    ...
    // If this doesn't start us running again, just return. In any case, decrement the pause count
    if (--fuse_emulation_paused) {
        return 0;
    }
...
static int setup_start_files(start_files_t *start_files)
{
    ...
    start_files->simpleide_master = utils_safe_strdup(settings_current.simpleide_master_file);
    start_files->simpleide_slave = utils_safe_strdup(settings_current.simpleide_slave_file);

    start_files->zxatasp_master = utils_safe_strdup(settings_current.zxatasp_master_file);
    start_files->zxatasp_slave = utils_safe_strdup(settings_current.zxatasp_slave_file);

    start_files->zxcf = utils_safe_strdup(settings_current.zxcf_pri_file);

    start_files->divide_master = utils_safe_strdup(settings_current.divide_master_file);
    start_files->divide_slave = utils_safe_strdup(settings_current.divide_slave_file);

    start_files->divmmc = utils_safe_strdup(settings_current.divmmc_file);
...
static int parse_nonoption_args(int argc, char **argv, int first_arg, start_files_t *start_files)
{
    ...
    for (i = first_arg; i < (size_t)argc; i++) {
        ...
        error = libspectrum_identify_file_with_class(&type, &class, filename, file.buffer, file.length);
        if (error) {
        ...
        switch (class) {
            ...
            case LIBSPECTRUM_CLASS_DISK_GENERIC:
                if (machine_current->capabilities & LIBSPECTRUM_MACHINE_CAPABILITY_PLUS3_DISK) {
                    start_files->disk_plus3 = filename;
                } else if (machine_current->capabilities & LIBSPECTRUM_MACHINE_CAPABILITY_TRDOS_DISK) {
                    start_files->disk_beta = filename;
                } else {
            ...
            case LIBSPECTRUM_CLASS_RECORDING:
                start_files->playback = filename;
                break;

            case LIBSPECTRUM_CLASS_SNAPSHOT:
                start_files->snapshot = filename;
                break;
            ...
            default:
                ui_error(UI_ERROR_ERROR, "parse_nonoption_args: unknown file class %d", class);
               break;
...
static int do_start_files(start_files_t *start_files)
{
    ...
    if (start_files->playback && start_files->recording) {
        ui_error(UI_ERROR_WARNING, "can't do both input playback and recording; recording disabled");
    ...
    if (start_files->disk_plus3 && start_files->disk_beta) {
        ui_error(UI_ERROR_WARNING, "can't use +3 and TR-DOS disks simultaneously; +3 disk ignored");
        start_files->disk_plus3 = NULL;
    }

    // Can't use disks and the dock simultaneously
    if ((start_files->disk_plus3 || start_files->disk_beta) && start_files->dock) {
        ui_error(UI_ERROR_WARNING, "can't use disks and the dock simultaneously; dock cartridge ignored");
        start_files->dock = NULL;
    }

    // Can't use disks and the Interface 2 simultaneously
    if ((start_files->disk_plus3 || start_files->disk_beta) && start_files->if2) {
        ui_error(UI_ERROR_WARNING, "can't use disks and the Interface 2 simultaneously; cartridge ignored");
    ...
    // snapshot loading such that the right snapshot is embedded into the file;
    // input playback being done after snapshot loading means
    // any embedded snapshot in the input recording will override any specified snapshot
    ...
    if (start_files->simpleide_master) {
        error = simpleide_insert(start_files->simpleide_master, LIBSPECTRUM_IDE_MASTER);
        simpleide_reset(0);
        if (error) {
            return error;
        }
    ...
    if (start_files->simpleide_slave) {
        error = simpleide_insert(start_files->simpleide_slave, LIBSPECTRUM_IDE_SLAVE);
        simpleide_reset(0);
        if (error) {
            return error;
        }
    ...
    if (start_files->zxatasp_master) {
        error = zxatasp_insert(start_files->zxatasp_master, LIBSPECTRUM_IDE_MASTER);
        if (error) {
            return error;
        }
    }
    ...
    if (start_files->zxatasp_slave) {
        error = zxatasp_insert(start_files->zxatasp_slave, LIBSPECTRUM_IDE_SLAVE);
        if (error) {
            return error;
        }
    }
    ...
    if (start_files->zxcf) {
        error = zxcf_insert(start_files->zxcf);
        if (error) {
            return error;
        }
    }
    ...
    if (start_files->divide_master) {
        error = divide_insert(start_files->divide_master, LIBSPECTRUM_IDE_MASTER);
        if (error) {
            return error;
        }
    }
    ...
    if (start_files->divide_slave) {
        error = divide_insert(start_files->divide_slave, LIBSPECTRUM_IDE_SLAVE);
        if (error) {
            return error;
        }
    }
    ...
    if (start_files->recording) {
        error = rzx_start_recording(start_files->recording, settings_current.embed_snapshot);
        if (error) {
            return error;
        }
    }
