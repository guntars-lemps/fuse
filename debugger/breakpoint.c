...
static int breakpoint_check(debugger_breakpoint *bp, debugger_breakpoint_type type, libspectrum_dword value);
...
// Add a breakpoint
int debugger_breakpoint_add_address(debugger_breakpoint_type type,
                                    int source,
                                    int page,
                                    ...
                                    debugger_expression *condition)
{
    ...
    switch (type) {
        ...
        default:
            ui_error(UI_ERROR_ERROR, "debugger_breakpoint_add_address given type %d", type);
            fuse_abort();
    }
                                 ...
                                 debugger_expression *condition)
{
    ...
    switch (type) {
        ...
        default:
            ui_error(UI_ERROR_ERROR, "debugger_breakpoint_add_port given type %d", type);
            fuse_abort();
    }
                                 ...
                                 debugger_expression *condition)
{
    ...
    switch (type) {
        ...
        default:
            ui_error(UI_ERROR_ERROR, "debugger_breakpoint_add_time given type %d", type);
            fuse_abort();
    }
                                  ...
                                  debugger_expression *condition)
{
    ...
    if (!debugger_event_is_registered(type_string, detail)) {
        ui_error(UI_ERROR_WARNING, "Event type %s:%s not known", type_string, detail);
        return 1;
    }
                          ...
                          debugger_expression *condition)
{
    ...
    // If this was a timed breakpoint, set an event to stop emulation at that point
...
static memory_page *get_page(debugger_breakpoint_type type, libspectrum_word address)
{
    memory_page *bank;
    ...
    switch (type) {
        ...
        default:
            ui_error(UI_ERROR_ERROR, "%s:get_page: unexpected breakpoint type %d", __FILE__, type);
            fuse_abort();
    }
...
static int breakpoint_check(debugger_breakpoint *bp, debugger_breakpoint_type type, libspectrum_dword value)
{
    ...
    switch (type) {
        ...
        case DEBUGGER_BREAKPOINT_TYPE_WRITE:
            ...
            // If source == memory_source_any, value must match exactly; otherwise, the source, page and offset must match
            if (bp->value.address.source == memory_source_any) {
                ...
                if ((bp->value.address.source != page->source) ||
                    ...
                    (bp->value.address.offset != (value & 0x3fff))) {
                    return 0;
                }
            }
            break;
...
static debugger_breakpoint *get_breakpoint_by_id(size_t id)
{
    ...
    ptr = g_slist_find_custom(debugger_breakpoints, &id, find_breakpoint_by_id);
    if (!ptr) {
        ui_error(UI_ERROR_ERROR, "Breakpoint %ld does not exist", (unsigned long)id);
        return NULL;
    }
...
static void remove_time(gpointer data, gpointer user_data)
{
    ...
    if ((event->type == debugger_breakpoint_event) && (event->tstates == remove->tstates)) {
...
int debugger_breakpoint_clear(libspectrum_word address)
{
    ...
    while (1) {
        ...
        ptr = g_slist_find_custom(debugger_breakpoints, &address, find_breakpoint_by_address);
        if (!ptr) {
            break;
        }
...
int debugger_breakpoint_remove_all(void)
{
    ...
    g_slist_free(debugger_breakpoints);
...
static void free_breakpoint(gpointer data, gpointer user_data GCC_UNUSED)
{
    ...
    if (bp->condition) {
        debugger_expression_delete(bp->condition);
    }
    if (bp->commands) {
...
// Add events corresponding to all the time breakpoints to happen during this frame
...
void debugger_breakpoint_time_fn(libspectrum_dword tstates, int type GCC_UNUSED, void *user_data GCC_UNUSED)
{
