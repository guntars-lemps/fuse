...
int utils_open_file(const char *filename, int autoload, libspectrum_id_t *type_ptr)
{
    ...
    if (libspectrum_identify_file_with_class(&type, &class, filename, file.buffer, file.length)) {
    ...
    switch (class) {
        ...
        case LIBSPECTRUM_CLASS_UNKNOWN:
            ui_error(UI_ERROR_ERROR, "utils_open_file: couldn't identify `%s'", filename);
            utils_close_file(&file);
            return 1;
        ...
        case LIBSPECTRUM_CLASS_TAPE:
            error = tape_read_buffer(file.buffer, file.length, type, filename, autoload);
            pokemem_find_pokfile(filename);
            break;
        ...
        case LIBSPECTRUM_CLASS_DISK_TRDOS:
            if (!(machine_current->capabilities & LIBSPECTRUM_MACHINE_CAPABILITY_TRDOS_DISK) &&
                ...
                error = machine_select(LIBSPECTRUM_MACHINE_SCORP);
                if (error) {
                    break;
                }
            }
        ...
        case LIBSPECTRUM_CLASS_DISK_GENERIC:
            if ((machine_current->machine == LIBSPECTRUM_MACHINE_PLUS3) ||
                ...
                error = beta_disk_insert(BETA_DRIVE_A, filename, autoload);
            } else if (periph_is_active(PERIPH_TYPE_BETA128)) {
                error = beta_disk_insert(BETA_DRIVE_A, filename, autoload);
        ...
        case LIBSPECTRUM_CLASS_CARTRIDGE_TIMEX:
            if (!(machine_current->capabilities & LIBSPECTRUM_MACHINE_CAPABILITY_TIMEX_DOCK)) {
...
// Find the auxiliary file called `filename';
// returns a fd for the file on success, -1 if it couldn't find the file
...
int utils_find_file_path(const char *filename, char *ret_path, utils_aux_type type)
{
    path_context ctx;
...
#else
        bytes_written = snprintf(ret_path, PATH_MAX, "%s" FUSE_DIR_SEP_STR "%s", ctx.path, filename);
#endif
...
int utils_write_file(const char *filename, const unsigned char *buffer, size_t length)
{
    compat_fd fd;
...
int utils_read_auxiliary_file(const char *filename, utils_file *file, utils_aux_type type)
{
    int error;
    compat_fd fd;
...
int utils_read_screen(const char *filename, utils_file *screen)
{
    ...
    if (error == -1) {
        ui_error(UI_ERROR_ERROR, "couldn't find screen picture ('%s')", filename);
        return 1;
    }
    ...
    if (screen->length != STANDARD_SCR_SIZE) {
        ...
        ui_error(UI_ERROR_ERROR, "screen picture ('%s') is not %d bytes long", filename, STANDARD_SCR_SIZE);
        return 1;
    }
...
int utils_save_binary(libspectrum_word start, libspectrum_word length, const char *filename)
{
