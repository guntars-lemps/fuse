...
static int machine_init_machines(void *context)
{
    int error;
...
static int machine_add_machine(int (*init_function)(fuse_machine_info *machine))
{
    ...
    machine_types = libspectrum_renew(fuse_machine_info *, machine_types, machine_count);

    machine_types[machine_count - 1] = libspectrum_new(fuse_machine_info, 1);
    machine = machine_types[machine_count - 1];

    error = init_function(machine);
    if (error) {
        return error;
    }
...
static int machine_select_machine(fuse_machine_info *machine)
{
    ...
    if (uidisplay_init(width, height)) {
        return 1;
    }
...
int machine_load_rom_bank_from_buffer(memory_page *bank_map,
                                      int page_num,
...
static int machine_load_rom_bank_from_file(memory_page *bank_map,
                                           int page_num,
                                           const char *filename,
                                           ...
                                           int custom)
{
    ...
    error = machine_load_rom_bank_from_buffer(bank_map, page_num, rom.buffer, rom.length, custom);
...
int machine_load_rom_bank(memory_page *bank_map,
                          int page_num,
                          const char *filename,
                          const char *fallback,
                          size_t expected_length)
{
    ...
    if (fallback) {
        custom = !!strcmp(filename, fallback);
    }

    retval = machine_load_rom_bank_from_file(bank_map, page_num, filename, expected_length, custom);
    if (retval && fallback && custom) {
        retval = machine_load_rom_bank_from_file(bank_map, page_num, fallback, expected_length, 0);
    }
    return retval;
}


int machine_load_rom(int page_num, const char *filename, const char *fallback, size_t expected_length)
{
    return machine_load_rom_bank(memory_map_rom, page_num, filename, fallback, expected_length);
}
...
int machine_reset(int hard_reset)
{
    ...
    error = machine_current->memory_map();
    if (error) {
        return error;
    }
...
static void machine_set_const_timings(fuse_machine_info *machine)
{
    // Pull timings we use repeatedly out of libspectrum and store them for ourself
    machine->timings.processor_speed = libspectrum_timings_processor_speed(machine->machine);
    machine->timings.left_border = libspectrum_timings_left_border(machine->machine);
    machine->timings.horizontal_screen = libspectrum_timings_horizontal_screen(machine->machine);
    machine->timings.right_border = libspectrum_timings_right_border(machine->machine);
    machine->timings.tstates_per_line = libspectrum_timings_tstates_per_line(machine->machine);
    machine->timings.interrupt_length = libspectrum_timings_interrupt_length(machine->machine);
    machine->timings.tstates_per_frame = libspectrum_timings_tstates_per_frame(machine->machine);
}
...
static void machine_set_variable_timings(fuse_machine_info *machine)
{
    ...
    /* Magic number alert
       ...
       after the interrupt at which the top-left pixel of the screen is displayed.
       However, what's more useful for Fuse is when the first
       pixel of the top line of the border is displayed.

       Fuse shows DISPLAY_BORDER_HEIGHT lines of top border and
       DISPLAY_BORDER_WIDTH_COLS columns of left border,
       so we subtract the appropriate offset to get when the top-left pixel of the border is displayed.
    */
...
static void machine_end(void)
{
    int i;

    for (i = 0; i < machine_count; i++) {
        if (machine_types[i]->shutdown) {
            machine_types[i]->shutdown();
        }
...
void machine_register_startup(void)
{
    ...
    startup_manager_register(STARTUP_MANAGER_MODULE_MACHINE,
                             dependencies,
