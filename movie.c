...
static char get_screentype(void)
{
...
static void movie_compress_area(int x, int y, int w, int h, int s)
{
...
static void movie_start_fmf(const char *name)
{
    if ((of = fopen(name, "wb")) == NULL) { // trunc old file ? or append ?
        ui_error(UI_ERROR_ERROR, "error opening movie file '%s': %s", name, strerror(errno));
        return;
    }
...
void movie_start(const char *name) // some init, open file (name)
{
    frame_no = slice_no = 0;
    if ((name == NULL) || (*name == '\0')) {
...
{
    if (!movie_paused && !movie_recording) {
        return;
    }
...
static inline void write_alaw(libspectrum_signed_word *buff, int len)
{
    int i = 0;
    while (len--) {
        if (*buff >= 0) {
...
static void add_sound(libspectrum_signed_word *buff, int len)
{
...
void movie_add_sound(libspectrum_signed_word *buff, int len)
{
    while (len) {
        if (stereo == 'S') {
