...
static gint find_drive(gconstpointer data, gconstpointer user_data)
{
    ...
    return !((drive->controller_index == info->controller) && (drive->drive_index == info->drive));
}
...
static int drive_disk_write(const ui_media_drive_info_t *drive, const char *filename)
{
    ...
    if (filename == NULL) {
        ...
        ui_confirm_save_t confirm = ui_confirm_save("%s already exists.\nDo you want to overwrite it?", filename1);
...
static int drive_eject(const ui_media_drive_info_t *drive)
{
    ...
    if (drive->fdd->disk.dirty) {
        ...
        ui_confirm_save_t confirm = ui_confirm_save("%s has been modified.\nDo you want to save it?", drive->name);
...
int ui_media_drive_insert(const ui_media_drive_info_t *drive, const char *filename, int autoload)
{
    ...
    if (filename) {
        error = disk_open(&drive->fdd->disk, filename, 0, DISK_TRY_MERGE(drive->fdd->fdd_heads));
        if (error != DISK_OK) {
        ...
        dt = drive->get_params();
        error = disk_new(&drive->fdd->disk, dt->heads, dt->cylinders, DISK_DENS_AUTO, DISK_UDI);
        if (error != DISK_OK) {
