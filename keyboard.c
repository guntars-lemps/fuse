...
// Bit masks for each of the eight keyboard half-rows; `AND' the selected ones of these to get the value to return
...
// Which Spectrum keys should be pressed for each key passed from the Fuse input layer
...
// When each Spectrum key is pressed, twiddle this {port, bit} pair in `keyboard_return_values'.
...
static int keyboard_init(void *context)
{
    struct key_info *ptr;
    ...
    for (ptr2 = spectrum_keys_table; ptr2->input != INPUT_KEY_NONE; ptr2++) {
        g_hash_table_insert(spectrum_keys, &(ptr2->input), &(ptr2->spectrum));
    }
...
void keyboard_register_startup(void)
{
    ...
    startup_manager_register(STARTUP_MANAGER_MODULE_KEYBOARD,
                             dependencies,
...
libspectrum_byte keyboard_simulate_keypress(libspectrum_byte porth, keyboard_key_name key)
{
