...
static int memory_init(void *context)
{
    size_t i, j;
...
static void memory_pool_free_entry(gpointer data, gpointer user_data GCC_UNUSED)
{
...
void memory_register_startup(void)
{
    ...
    startup_manager_register(STARTUP_MANAGER_MODULE_MEMORY,
                             dependencies,
...
static gint find_non_persistent(gconstpointer data, gconstpointer user_data GCC_UNUSED)
{
...
// Map 16K of memory
void memory_map_16k(libspectrum_word address, memory_page source[], int page_num)
{
...
// Map 8K of memory
void memory_map_8k(libspectrum_word address, memory_page source[], int page_num)
{
...
void memory_map_2k_read_write(libspectrum_word address, memory_page source[], int page_num, int map_read, int map_write)
{
    int i;
    for (i = 0; i < MEMORY_PAGES_IN_2K; i++) {
        ...
        if (map_write) {
            memory_map_write[page_offset] = *page;
        }
    }
}
...
{
    memory_map_read[page_num] = memory_map_write[page_num] = *source[page_num];
}
...
void memory_display_dirty_pentagon_16_col(libspectrum_word address, libspectrum_byte b)
{
    ...
    /* If this is a write to the current screen areas (and it actually changes the destination), redraw that bit.
       The trick here is that we need to check the home bank screen areas in page 5 and 4
       (if screen 1 is in use), and page 7 & 6 (if screen 2 is in use)
       and both the standard and ALTDFILE areas of those pages
     */
    if ((mapping->source == memory_source_ram) &&
        (((memory_current_screen == 5) && ((mapping->page_num == 5) || (mapping->page_num == 4))) ||
         ((memory_current_screen == 7) && ((mapping->page_num == 7) || (mapping->page_num == 6)))) &&
        ((offset2 & 0xdfff) < 0x1b00) && (memory[offset] != b)) {
...
void writebyte_internal(libspectrum_word address, libspectrum_byte b)
{
    ...
    } else if (mapping->writable || ((mapping->source != memory_source_none) && settings_current.writable_roms)) {
        libspectrum_word offset = address & MEMORY_PAGE_SIZE_MASK;
        libspectrum_byte *memory = mapping->page;
...
static void memory_from_snapshot(libspectrum_snap *snap)
{
    ...
    if (capabilities & LIBSPECTRUM_MACHINE_CAPABILITY_PENT1024_MEMORY) {
        pentagon1024_memoryport_write(0x7ffd, libspectrum_snap_out_128_memoryport(snap));
        pentagon1024_v22_memoryport_write(0xeff7, libspectrum_snap_out_plus3_memoryport(snap));
    } else {
        if (capabilities & LIBSPECTRUM_MACHINE_CAPABILITY_128_MEMORY) {
            spec128_memoryport_write(0x7ffd, libspectrum_snap_out_128_memoryport(snap));
        }

        if ((capabilities & LIBSPECTRUM_MACHINE_CAPABILITY_PLUS3_MEMORY) ||
            (capabilities & LIBSPECTRUM_MACHINE_CAPABILITY_SCORP_MEMORY)) {
            specplus3_memoryport2_write_internal(0x1ffd, libspectrum_snap_out_plus3_memoryport(snap));
       }
    }
    ...
    if (libspectrum_snap_custom_rom(snap)) {
        for (i = 0; i < libspectrum_snap_custom_rom_pages(snap) && (i < 4); i++) {
            if (libspectrum_snap_roms(snap, i)) {
                machine_load_rom_bank_from_buffer(memory_map_rom,
                                                  i,
...
static void write_rom_to_snap(libspectrum_snap *snap, int *current_rom_num, libspectrum_byte **current_rom, size_t *rom_length)
{
...
static void memory_rom_to_snapshot(libspectrum_snap *snap)
{
    ...
    for (i = 0; i < SPECTRUM_ROM_PAGES * MEMORY_PAGES_IN_16K; i++) {
        if (memory_map_rom[i].page) {
            if (current_page_num != memory_map_rom[i].page_num) {
                ...
                current_rom = libspectrum_renew(libspectrum_byte, current_rom, rom_length + MEMORY_PAGE_SIZE);

                memcpy(current_rom + rom_length, memory_map_rom[i].page, MEMORY_PAGE_SIZE);
                rom_length += MEMORY_PAGE_SIZE;
            }
        }
    }
...
static void memory_to_snapshot(libspectrum_snap *snap)
{
    ...
    libspectrum_snap_set_out_128_memoryport(snap, machine_current->ram.last_byte);
...
int trap_check_rom(trap_type type)
{
    ...
    switch (machine_current->machine) {
        ...
        case LIBSPECTRUM_MACHINE_PLUS3E:
            // OK if we're not in a 64Kb RAM configuration and we're in ROM 3
            return (!machine_current->ram.special && (machine_current->ram.current_rom == 3));

        case LIBSPECTRUM_MACHINE_128E:
            // OK if we're not in a 64Kb RAM configuration and we're in either ROM 1 or ROM 3 (which are the same)
            return (!machine_current->ram.special &&
                    ((machine_current->ram.current_rom == 1) || (machine_current->ram.current_rom == 3)));
        ...
        case LIBSPECTRUM_MACHINE_UNKNOWN: // should never happen
            ui_error(UI_ERROR_ERROR, "trap_check_rom: machine type is LIBSPECTRUM_MACHINE_UNKNOWN");
            fuse_abort();
    }

    ui_error(UI_ERROR_ERROR, "trap_check_rom: unknown machine type %d", machine_current->machine);
    fuse_abort();
}
