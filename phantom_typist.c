...
static int phantom_typist_init(void *context)
{
...
void phantom_typist_register_startup(void)
{
    startup_manager_module dependencies[] = {STARTUP_MANAGER_MODULE_SETUID};
    startup_manager_register(STARTUP_MANAGER_MODULE_PHANTOM_TYPIST,
                             dependencies,
...
static phantom_typist_highlevel_mode_t get_highlevel_mode(libspectrum_machine machine)
{
...
static void set_state_waiting(void)
{
    command_count = 0;
...
void phantom_typist_activate(libspectrum_machine machine, int needs_code)
{
    phantom_typist_highlevel_mode_t highlevel_mode;
    ...
    if (strcasecmp(setting, "Keyword") == 0) {
        ...
        highlevel_mode = PHANTOM_TYPIST_HIGHLEVEL_MODE_MENU;
    } else if (strcasecmp(setting, "Plus 2A") == 0 || strcasecmp(setting, "plus2a") == 0) {
        highlevel_mode = PHANTOM_TYPIST_HIGHLEVEL_MODE_PLUS2A;
    } else if (strcasecmp(setting, "Plus 3") == 0 || strcasecmp(setting, "plus3") == 0) {
        highlevel_mode = PHANTOM_TYPIST_HIGHLEVEL_MODE_PLUS3;
    ...
    switch (highlevel_mode) {
        ...
        default:
            phantom_typist_mode = needs_code ? PHANTOM_TYPIST_MODE_JPPI : PHANTOM_TYPIST_MODE_JPP;
            break;

        case PHANTOM_TYPIST_HIGHLEVEL_MODE_KEYSTROKE:
            phantom_typist_mode = needs_code ? PHANTOM_TYPIST_MODE_LOADPPCODE : PHANTOM_TYPIST_MODE_LOADPP;
            break;

        case PHANTOM_TYPIST_HIGHLEVEL_MODE_MENU:
            phantom_typist_mode = needs_code ? PHANTOM_TYPIST_MODE_DOWN_LOADPPCODE : PHANTOM_TYPIST_MODE_ENTER;
          break;

        case PHANTOM_TYPIST_HIGHLEVEL_MODE_PLUS2A:
            phantom_typist_mode = needs_code ?  PHANTOM_TYPIST_MODE_WAIT_DOWN_LOADPPCODE : PHANTOM_TYPIST_MODE_ENTER;
            break;

        case PHANTOM_TYPIST_HIGHLEVEL_MODE_PLUS3:
            phantom_typist_mode = needs_code ? PHANTOM_TYPIST_MODE_PLUS3_CODE_BLOCK : PHANTOM_TYPIST_MODE_ENTER;
            break;
    }
    set_state_waiting();
}
...
void phantom_typist_activate_disk(void)
{
...
void phantom_typist_deactivate(void)
{
...
static void process_waiting_state(libspectrum_byte high_byte)
{
    switch (high_byte) {
        case 0xfe:
        case 0xfd:
...
static phantom_typist_state_t t_colon_or_quote(void)
{
    ...
    if ((phantom_typist_mode == PHANTOM_TYPIST_MODE_PLUS3_CODE_BLOCK) && (command_count == 0)) {
...
static phantom_typist_state_t code_or_enter(void)
{
    ...
    switch (phantom_typist_mode) {
        ...
        case PHANTOM_TYPIST_MODE_LOADPP:
            ...
            break;
        case PHANTOM_TYPIST_MODE_PLUS3_CODE_BLOCK:
            next_state = (command_count == 0) ? PHANTOM_TYPIST_STATE_ENTER : PHANTOM_TYPIST_STATE_CODE_C;
            break;
        case PHANTOM_TYPIST_MODE_JPPI:
...
static libspectrum_byte process_state(libspectrum_byte high_byte)
{
    ...
    next_phantom_typist_state = this_state->next_state_fn ? this_state->next_state_fn() : this_state->next_state;

    return r;
}


libspectrum_byte phantom_typist_ula_read(libspectrum_word port)
{
    libspectrum_byte r = 0xff;
...
int phantom_typist_is_active(void)
{
    return phantom_typist_state != PHANTOM_TYPIST_STATE_INACTIVE;
}


void phantom_typist_frame(void)
{
