...
static OSStatus coreaudiowrite(void *inRefCon,
...
static int get_default_output_device(AudioDeviceID *device)
{
    ...
    // get the default output device for the HAL
    count = sizeof(*device);
    err = AudioObjectGetPropertyData(kAudioObjectSystemObject, &property_address, 0, NULL, &count, device);
    if ((err != kAudioHardwareNoError) && (device != kAudioObjectUnknown)) {
        ui_error(UI_ERROR_ERROR, "get kAudioHardwarePropertyDefaultOutputDevice error %ld", (long)err);
        return 1;
    }
    return 0;
}
...
static int get_default_sample_rate(AudioDeviceID device, Float64 *rate)
{
    ...
    // get the default output device for the HAL
    count = sizeof(*rate);
    err = AudioObjectGetPropertyData(device, &property_address, 0, NULL, &count, rate);
    if (err != kAudioHardwareNoError) {
        ui_error(UI_ERROR_ERROR, "get kAudioDevicePropertyNominalSampleRate error %ld", (long)err);
        return 1;
    }
    return 0;
}
...
int sound_lowlevel_init(const char *dev, int *freqptr, int *stereoptr)
{
    ...
    *freqptr = deviceFormat.mSampleRate;
    ...
    /* Amount of audio data we will accumulate before yielding back to the OS.
       Not much point having more than 100Hz playback, we probably get downgraded by the OS
       as being a hog too (unlimited Hz limits playback speed to about 2000% on my Mac,
       100Hz allows up to 5000% for me) */
    if (hz > 100.0) {
        hz = 100.0;
    }
    sound_framesiz = deviceFormat.mSampleRate / hz;

    if (error = sfifo_init(&sound_fifo,
                           ((NUM_FRAMES * deviceFormat.mBytesPerFrame * deviceFormat.mChannelsPerFrame * sound_framesiz) + 1))) {
