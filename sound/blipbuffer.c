...
{
    buff->factor_ = blip_buffer_clock_rate_factor(buff, (buff->clock_rate_ = cps));
}
...
{
    _blip_synth_volume_unit(&synth->impl, (v * (1.0 / ((BLIP_SYNTH_RANGE < 0) ? -BLIP_SYNTH_RANGE : BLIP_SYNTH_RANGE))));
}


inline void blip_synth_offset_resampled(Blip_Synth *synth, blip_resampled_time_t time, int delta, Blip_Buffer *blip_buf)
{
...
void blip_synth_update(Blip_Synth *synth, blip_time_t t, int amp)
{
    ...
    blip_synth_offset_resampled(synth, ((t * synth->impl.buf->factor_) + synth->impl.buf->offset_), delta, synth->impl.buf);
}
...
{
    synth->impulses = malloc(((BLIP_RES * (BLIP_SYNTH_QUALITY / 2)) + 1) * sizeof(imp_t) * 4);
    if (synth->impulses) {
...
void blip_buffer_clear(Blip_Buffer *buff, int entire_buffer)
{
    ...
    buff->reader_accum = 0;
    if (buff->buffer_) {
        long count = (entire_buffer ? buff->buffer_size_ : blip_buffer_samples_avail(buff));
        memset(buff->buffer_, 0, ((count + BUFFER_EXTRA) * sizeof(buf_t_)));
    }
}


blargg_err_t blip_buffer_set_sample_rate(Blip_Buffer *buff, long new_rate, int msec)
{
    ...
    if (buff->buffer_size_ != new_size) {
        void *p = realloc(buff->buffer_, ((new_size + BUFFER_EXTRA) * sizeof(buf_t_)));
        if (!p) {
...
inline void blip_buffer_remove_silence(Blip_Buffer *buff, long count)
{
    buff->offset_ -= (blip_resampled_time_t)count << BLIP_BUFFER_ACCURACY;
}


inline void blip_buffer_remove_samples(Blip_Buffer *buff, long count)
{
    long remain;
    ...
    if (count) {
        ...
        memmove(buff->buffer_, (buff->buffer_ + count), (remain * sizeof(buf_t_)));
...
// Blip_Synth_
void _blip_synth_init(Blip_Synth_ *synth_, short *p)
{
    synth_->impulses = p;
...
static void blip_eq_generate(blip_eq_t *eq, float *out, int count)
{
    // lower cutoff freq for narrow kernels with their wider transition band (8 points->1.49, 16 points->1.15)
    int i;
...
void _blip_synth_treble_eq(Blip_Synth_ *synth_, blip_eq_t *eq)
{
...
void _blip_synth_volume_unit(Blip_Synth_ *synth_, double new_unit)
{
        ...
        if (factor > 0.0) {
            ...
            if (shift) {
                ...
                for (i = _blip_synth_impulses_size(synth_); i--;) {
                    synth_->impulses[i] = (short)(((synth_->impulses[i] + offset) >> shift) - offset2);
                }
