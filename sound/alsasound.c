...
// This is necessary to prevent warnings from the calls to snd_pcm_[hs]w_params_alloca()
#ifndef NDEBUG
#define NDEBUG
#endif
...
int sound_lowlevel_init(const char *device, int *freqptr, int *stereoptr)
{
    ...
    while (option && *option) {
        tmp = '*';
        if ((err = sscanf(option, " buffer=%i %n%c", &val, &n, &tmp) > 0) && ((tmp == ',') || (strlen(option) == n))) {
            if (val < 1) {
                fprintf(stderr, "Bad value for ALSA buffer size %i, using default\n", val);
            } else {
                bsize = val;
            }
        } else if (((err = sscanf(option, " frames=%i %n%c", &val, &n, &tmp)) > 0) && ((tmp == ',') || (strlen(option) == n))) {
            if (val < 1) {
                fprintf(stderr, "Bad value for ALSA buffer size %i frames, using default (%d)\n", val, NUM_FRAMES);
            } else {
                nperiods = val;
            }
        } else if (((err = sscanf(option, " avail=%i %n%c", &val, &n, &tmp)) > 0) && ((tmp == ',') || (strlen(option) == n))) {
            if (val < 1) {
                fprintf(stderr, "Bad value for ALSA avail_min size %i frames, using default\n", val);
            } else {
                avail_min = val;
            }
        } else if (((err = sscanf(option, " verbose %n%c", &n, &tmp)) == 1) && ((tmp == ',')  || (strlen(option) == n))) {
            verb = 1;
    ...
    if (snd_pcm_open(&pcm_handle, pcm_name , stream, 0) < 0) {
        if (strcmp(pcm_name, "default") == 0) {
            ...
            if (snd_pcm_open(&pcm_handle, "plughw:0,0", stream, 0) < 0) {
                ...
                ui_error(UI_ERROR_ERROR, "couldn't open sound device 'default' and 'plughw:0,0' check ALSA configuration.");
                init_running = 0;
                return 1;
            } else {
                if (first_init) {
                    fprintf(stderr, "Couldn't open sound device 'default', using 'plughw:0,0' check ALSA configuration.\n");
                }
            }
        }
        settings_current.sound = 0;
    ...
    if (snd_pcm_hw_params_any(pcm_handle, hw_params) < 0) {
        settings_current.sound = 0;
        ui_error(UI_ERROR_ERROR, "couldn't get configuration space on sound device '%s'.", pcm_name);
        snd_pcm_close(pcm_handle);
        init_running = 0;
        return 1;
    }
    ...
    if (snd_pcm_hw_params_set_access(pcm_handle, hw_params, SND_PCM_ACCESS_RW_INTERLEAVED) < 0) {
        settings_current.sound = 0;
        ui_error(UI_ERROR_ERROR, "couldn't set access interleaved on '%s'.", pcm_name);
        snd_pcm_close(pcm_handle);
        init_running = 0;
        return 1;
    }
    ...
    if (snd_pcm_hw_params_set_channels(pcm_handle, hw_params, ch) < 0) {
        fprintf(stderr, "Couldn't set %s to '%s'.\n", pcm_name, (*stereoptr ? "stereo" : "mono"));
        ch = *stereoptr ? 1 : 2; // try with opposite
        if (snd_pcm_hw_params_set_channels(pcm_handle, hw_params, ch) < 0) {
            ui_error(UI_ERROR_ERROR, "couldn't set %s to '%s'.", pcm_name, (*stereoptr ? "stereo" : "mono"));
            settings_current.sound = 0;
            snd_pcm_close(pcm_handle);
            init_running = 0;
            return 1;
    ...
    exact_rate = *freqptr;
    if (snd_pcm_hw_params_set_rate_near(pcm_handle, hw_params, &exact_rate, NULL) < 0) {
        settings_current.sound = 0;
        ui_error(UI_ERROR_ERROR, "couldn't set rate %d on '%s'.", *freqptr, pcm_name);
        snd_pcm_close(pcm_handle);
        init_running = 0;
        return 1;
    }
    ...
    if (bsize == 0) {
        /* Adjust relative processor speed to deal with adjusting sound generation
           frequency against emulation speed (more flexible than adjusting generated sample rate) */
        ...
        /* Amount of audio data we will accumulate before yielding back to the OS.
           Not much point having more than 100Hz playback,
           we probably get downgraded by the OS as being a hog too
           (unlimited Hz limits playback speed to about 2000% on my Mac, 100Hz allows up to 5000% for me) */
        if (hz > 100.0) {
            hz = 100.0;
        }
    ...
    dir = -1;
    if (snd_pcm_hw_params_set_period_size_near(pcm_handle, hw_params, &exact_periodsize, &dir) < 0) {
        settings_current.sound = 0;
        ui_error(UI_ERROR_ERROR, "couldn't set period size %d on '%s'.", (int)sound_periodsize, pcm_name);
        snd_pcm_close(pcm_handle);
        init_running = 0;
        return 1;
    }

    if (first_init && ((exact_periodsize < (sound_periodsize / 1.5)) || (exact_periodsize > (sound_periodsize * 1.5)))) {
        fprintf(stderr,
                "The period size %d is not supported by your hardware. Using %d instead.\n",
    ...
    if (snd_pcm_hw_params_set_periods_near(pcm_handle, hw_params, &periods, NULL) < 0) {
        settings_current.sound = 0;
    ...
    if (first_init && (periods != nperiods)) {
        fprintf(stderr, "%d periods is not supported by your hardware. Using %d instead.\n", nperiods, periods);
    }
    ...
    if ((err = snd_pcm_sw_params_current(pcm_handle, sw_params)) < 0) {
        ui_error(UI_ERROR_ERROR, "couldn't get sw_params from %s: %s", pcm_name, snd_strerror (err));
        snd_pcm_close(pcm_handle);
        init_running = 0;
        return 1;
    }

    if ((err = snd_pcm_sw_params_set_start_threshold(pcm_handle, sw_params, (exact_periodsize * (nperiods - 1)))) < 0) {
        ui_error(UI_ERROR_ERROR,"couldn't set start_treshold on %s: %s", pcm_name, snd_strerror (err));
        snd_pcm_close(pcm_handle);
        init_running = 0;
        return 1;
    }
    ...
    if (snd_pcm_sw_params_set_avail_min(pcm_handle, sw_params, avail_min) < 0) {
#if SND_LIB_VERSION < 0x10010
        if ((err = snd_pcm_sw_params_set_sleep_min(pcm_handle, sw_params, 1)) < 0) {
    ...
    if ((err = snd_pcm_sw_params_set_xfer_align(pcm_handle, sw_params, 1)) < 0) {
        ui_error(UI_ERROR_ERROR,"couldn't set xfer_allign on %s: %s", pcm_name, snd_strerror (err));
        init_running = 0;
        return 1;
    }
#endif
    ...
    if ((err = snd_pcm_sw_params(pcm_handle, sw_params)) < 0) {
        ui_error(UI_ERROR_ERROR,"couldn't set sw_params on %s: %s", pcm_name, snd_strerror (err));
        init_running = 0;
        return 1;
    }
