...
int sfifo_init(sfifo_t *f, int size)
{
    ...
    /* Set sufficient power-of-2 size.
       ...
       No, there's no bug. If you need room for N bytes, the buffer must be at least N+1 bytes.
       (The fifo can't tell 'empty' from 'full' without unsafe index manipulations otherwise.)
    */
    f->size = 1;
