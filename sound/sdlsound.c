...
int sound_lowlevel_init(const char *device, int *freqptr, int *stereoptr)
{
    ...
    if (device) {
        ...
        char *command = libspectrum_new(char, (strlen(environment) + strlen(device) + 1));
        ...
        if (error) {
            ...
            ui_error(UI_ERROR_ERROR, "Couldn't set SDL_AUDIODRIVER: %s", strerror (error));
            return 1;
        }
    }
    ...
    if (SDL_OpenAudio(&requested, &received) < 0) {
        settings_current.sound = 0;
        ui_error(UI_ERROR_ERROR, "Couldn't open sound device: %s", SDL_GetError());
        return 1;
    }
    ...
    *freqptr = received.freq;
    ...
    if (received.format != AUDIO_S16SYS) {
        // close audio and then just let SDL convert to this wacky format at a supported sample rate
        SDL_CloseAudio();
    ...
    if ((error = sfifo_init(&sound_fifo, ((NUM_FRAMES * received.channels * sound_framesiz) + 1)))) {
...
void sdlwrite(void *userdata, Uint8 *stream, int len)
{
    ...
    // If we ran out of sound, do nothing else as SDL has prefilled the output buffer with silence :(
}
