...
static void driver_error(void)
{
    switch (errno) {
        case AO_ENODRIVER:
            ui_error(UI_ERROR_ERROR, "ao: no driver corresponds to driver_id.");
            break;
        case AO_ENOTLIVE:
            ui_error(UI_ERROR_ERROR, "ao: driver is not a live output device.");
            break;
        case AO_ENOTFILE:
            ui_error(UI_ERROR_ERROR, "ao: driver is not a file output driver.");
            break;
        case AO_EBADOPTION:
            ui_error(UI_ERROR_ERROR, "ao: a valid option key has an invalid value.");
            break;
        case AO_EOPENDEVICE:
            ui_error(UI_ERROR_ERROR, "ao: cannot open output device.");
            break;
        case AO_EOPENFILE:
            ui_error(UI_ERROR_ERROR, "ao: cannot open output file '%s'.", filename);
            break;
        case AO_EFILEEXISTS:
            ui_error(UI_ERROR_ERROR, "ao: output file '%s' already exists.", filename);
            break;
        case AO_EFAIL:
...
static int parse_driver_options(const char *device, int *driver_id, ao_option **options)
{
    ...
    while (option) {
        ...
        if (strcmp(key, "file") == 0) {
            ...
            ui_error(UI_ERROR_ERROR, "ignoring badly formed libao option (%s%s)", (key ? key : ""), (value ? value : ""));
        }
...
int sound_lowlevel_init(const char *device, int *freqptr, int *stereoptr)
{
    ...
    if (driver_id == -1) {
        ui_error(UI_ERROR_ERROR, "ao: driver '%s' unknown", device);
        settings_current.sound = 0;
        sound_lowlevel_init_in_progress = 0;
        return 1;
    }
    ...
    if ((driver_info->type == AO_TYPE_FILE) && (format.bits != 0)) { // OK. We not want to trunc the file :-)
        ui_error(UI_ERROR_WARNING, "ao: must truncate audio file '%s'", filename);
    }
