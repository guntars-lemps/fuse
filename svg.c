...
static int svg_attribute(const char *name, const char *value, const char *element)
{
...
static int svg_write_metadata(void)
{
...
void svg_openfile(void)
{
    ...
    // we free the writer at the start of each of these branches since this must be done before using the buffer
...
void svg_rect(int xpos, int ypos, const char *xsize, const char *ysize, const char *opacity, const char *color)
{
    ...
    if (xmlTextWriterEndElement(writer) < 0) {
        ui_error(UI_ERROR_ERROR, "error finishing a box creation in the SVG file");
        return;
    }
}


void svg_openpath(void)
{
    int err;
    char svgcolor[BUFSZ];
    ...
    snprintf(svgcolor, BUFSZ, "rgb(%u,%u,%u)", palette[svg_paper][0], palette[svg_paper][1], palette[svg_paper][2]);
    ...
    svg_ink = (readbyte_internal(0x5c8d) & 7) + ((readbyte_internal(0x5c8d) >> 3) & 8); // ATTR_P

    err = 0;
    snprintf(svgcolor, BUFSZ, "rgb(%u,%u,%u)", palette[svg_ink][0], palette[svg_ink][1], palette[svg_ink][2]);
...
void svg_closefile(void)
{
    ...
    snprintf(svg_fname, strlen(svg_fnameroot) + BUFSIZ, "%s%d.svg", svg_fnameroot, svg_filecount++);

    if ((fp = fopen(svg_fname, "w")) != NULL) {
        fprintf(fp, "%s", buffer->content);

        if (fclose(fp) != 0) {
            ui_error(UI_ERROR_ERROR, "error closing SVG file '%s': %s", svg_fname, strerror(errno));
        }
    }
...
void svg_capture_draw(void)
{
    ...
    svg_ink = (readbyte_internal(0x5c8f) & 7) + ((readbyte_internal(0x5c8f) >> 3) & 8); // ATTR_T
    snprintf(svgcolor, BUFSZ, "rgb(%u,%u,%u)", palette[svg_ink][0], palette[svg_ink][1], palette[svg_ink][2]);

    if ((svg_capture_mode == SVG_CAPTURE_LINES) && ((z80.pc.w == 0x24ba) || (z80.pc.w == 0x2813))) {
        // DRAW
        ...
        snprintf(path_element, BUFSZ, "%d", (svg_y_size - 1 - readbyte_internal(0x5c7e)) * 2);
        err += (svg_attribute("y1", path_element, "line") != 0);

        snprintf(path_element, BUFSZ, "%d", (readbyte_internal(0x5c7d) + dx) * 2);
        err += (svg_attribute("x2", path_element, "line") != 0);
        snprintf(path_element, BUFSZ, "%d", ((svg_y_size - 1 - readbyte_internal(0x5c7e) - dy) * 2));
    ...
    if (xmlTextWriterEndElement(writer) < 0) {
        ui_error(UI_ERROR_ERROR, "error finishing the creation of the SVG element");
        return;
    }
...
void svg_byte(int xpos, int ypos, int udg_byte, char *color)
{
    int i;
    char path_element[BUFSZ];
    int err = 0;
    ...
    for (i = 7; i >= 0; i--) {
        if (udg_byte & 1) {
            ...
            if (xmlTextWriterEndElement(writer) < 0) {
                ui_error(UI_ERROR_ERROR, "error finishing the creation of a dot in the UDG element");
                return;
            }
        }
        udg_byte /= 2;
...
void svg_capture_char(void)
{
    ...
    if ((z80.de.b.l == 188) || ((z80.de.b.l == 70) &&
        ...
        snprintf(svgcolor_paper, BUFSZ, "rgb(%u,%u,%u)", palette[svg_paper][0], palette[svg_paper][1], palette[svg_paper][2]);
        snprintf(svgcolor_ink, BUFSZ, "rgb(%u,%u,%u)", palette[svg_ink][0], palette[svg_ink][1], palette[svg_ink][2]);
        svg_char = z80.af.b.h;


        // -- PAPER
        if (svg_char < 165) {
            svg_rect(x_pos * 16, (y_pos - 1) * 16, "16", "16", "0.4", svgcolor_paper);
        }
        ...
        if (svg_char > 127) {
            ...
            if (svg_char < 144) {
                // GRAPHICS BLOCKS
                if (svg_char & 1)
                    svg_rect(8 + x_pos * 16, (y_pos - 1) * 16, "8", "8", "1", svgcolor_ink);

                if (svg_char & 2)
                    svg_rect(x_pos * 16, (y_pos - 1) * 16, "8", "8", "1", svgcolor_ink);

                if (svg_char & 4)
                    svg_rect(8 + x_pos * 16, 8 + (y_pos - 1) * 16, "8", "8", "1", svgcolor_ink);

                if (svg_char & 8)
                    svg_rect(x_pos * 16, 8 + (y_pos - 1) * 16, "8", "8", "1", svgcolor_ink);
            } else if (svg_char < 165) {
                // 144-164 -> UDG "A"-"U" (pointed by $5C7B/$5C7C)
                for (i = 0; i < 8; i++) {
                    udg_ptr = (readbyte_internal(0x5c7c) << 8) + readbyte_internal(0x5c7b);
                    svg_byte(x_pos * 16, (i * 2) + (y_pos - 1) * 16,
                    readbyte_internal(udg_ptr + i + 8 * (svg_char - 144)), svgcolor_ink);
                }
            }
        } else {
            // -- INK and char
            ...
            if (err) {
                ui_error(UI_ERROR_ERROR, "error setting the SVG text element coordinates");
                return;
            }
...
void svg_capture_cls(void)
{
    if (svg_flag) {
        ...
        // This trick could load empty fake paths into the SVG file when CLS happens
        svg_closepath();
        svg_openpath();
    }
    svg_flag = 0;
...
void svg_capture(void)
{
    if (trap_check_rom(CHECK_48K_ROM)) {
        ...
        if ((machine_current->machine == LIBSPECTRUM_MACHINE_TS2068) ||
            ...
            if ((svg_capture_mode == SVG_CAPTURE_LINES) && ((z80.pc.w == 0x2813 || z80.pc.w == 0x263e))) {
                svg_capture_draw();
            }
            ...
            if ((svg_capture_mode == SVG_CAPTURE_LINES) && ((z80.pc.w == 0x24ba || z80.pc.w == 0x22df))) {
                svg_capture_draw();
            }
