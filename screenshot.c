...
static int get_rgb32_data(libspectrum_byte *rgb32_data, size_t stride, size_t height, size_t width);
static int rgb32_to_rgb24(libspectrum_byte *rgb24_data,
                          size_t rgb24_stride,
                          libspectrum_byte *rgb32_data,
                          size_t rgb32_stride,
                          size_t height,
                          size_t width);

/* The biggest size screen (in units of DISPLAY_ASPECT_WIDTH x DISPLAY_SCREEN_HEIGHT ie a Timex screen is size 2)
   we will be creating via the scalers */
#define MAX_SIZE 3

/* The space used for drawing the screen image on. Out here to avoid placing these large objects on the stack */
static libspectrum_byte rgb_data1[MAX_SIZE * DISPLAY_SCREEN_HEIGHT * 3 * DISPLAY_ASPECT_WIDTH * 4];
static libspectrum_byte rgb_data2[MAX_SIZE * DISPLAY_SCREEN_HEIGHT * 3 * DISPLAY_ASPECT_WIDTH * 4];
static libspectrum_byte png_data[MAX_SIZE * DISPLAY_SCREEN_HEIGHT * 3 * DISPLAY_ASPECT_WIDTH * 3];
...
int screenshot_write(const char *filename, scaler_type scaler)
{
    ...
    size_t png_stride = MAX_SIZE * DISPLAY_ASPECT_WIDTH * 3;
    ...
    f = fopen(filename, "wb");
    if (!f) {
        ui_error(UI_ERROR_ERROR, "Couldn't open `%s': %s", filename, strerror(errno));
        return 1;
    }
    ...
    // Set up the error handling; libpng will return to here if it encounters an error
    ...
    if (fclose(f)) {
        ui_error(UI_ERROR_ERROR, "Couldn't close `%s': %s", filename, strerror(errno));
        return 1;
    }
    return 0;
}
...
static int screenshot_scr_hires_write(const char *filename)
{
    ...
    memcpy(scr_data, &RAM[memory_current_screen][display_get_addr(0,0)], MONO_BITMAP_SIZE);
    memcpy((scr_data + MONO_BITMAP_SIZE),
           &RAM[memory_current_screen][display_get_addr(0,0) + ALTDFILE_OFFSET],
           MONO_BITMAP_SIZE);
    scr_data[HIRES_ATTR] = (scld_last_dec.byte & HIRESCOLMASK) | scld_last_dec.mask.scrnmode;
...
static int scr_write(const char *filename, const int data_size, data_write_fn scr_data_write)
{
...
int screenshot_scr_write(const char *filename)
{
    if (scld_last_dec.name.hires) {
        ...
        return scr_write(filename, HICOLOUR_SCR_SIZE, &set_hicolor_pixels_and_attribute);
    } else { // ALTDFILE and default
        return scr_write(filename, STANDARD_SCR_SIZE, &set_standard_pixels_and_attribute);
    }
}
...
int screenshot_scr_read(const char *filename)
{
    ...
    switch (screen.length) {
        case STANDARD_SCR_SIZE:
            memcpy(&RAM[memory_current_screen][display_get_addr(0,0)], screen.buffer, screen.length);
        ...
        case HICOLOUR_SCR_SIZE:
            // If it is a Timex and it is not in hi colour mode, copy screen and switch mode if neccesary
            ...
            if (machine_current->timex) {
                ...
                ui_error(UI_ERROR_INFO, "The file contained a TC2048 high-colour screen, loaded as mono");
            }

            memcpy(&RAM[memory_current_screen][display_get_addr(0,0)], screen.buffer, MONO_BITMAP_SIZE);
            break;

        case HIRES_SCR_SIZE:
            // If it is a Timex and it is not in hi res mode, copy screen and switch mode if neccesary
            ...
            if (machine_current->timex) {
                ...
                for (i = 0; i < MONO_BITMAP_SIZE; i++) {
                    RAM[memory_current_screen][display_get_addr(0,0) + i] =
                        convert_hires_to_lores(*(screen.buffer + MONO_BITMAP_SIZE + i), *(screen.buffer + i));
                }

                // set attributes based on hires attribute byte
                for (i = 0; i < 768; i++) {
                    RAM[memory_current_screen][display_get_addr(0,0) + MONO_BITMAP_SIZE + i] = attr;
                }
                ui_error(UI_ERROR_INFO, "The file contained a TC2048 high-res screen, converted to lores");
            }
            break;
...
int screenshot_mlt_read(const char *filename)
{
    ...
    if (screen.length != MLT_SIZE) {
        ui_error(UI_ERROR_ERROR, "MLT picture ('%s') is not %d bytes long", filename, MLT_SIZE);
        return 1;
    }
    ...
    if (machine_current->timex) {
        ...
        ui_error(UI_ERROR_INFO, "The file contained a MLT high-colour screen, loaded as mono");
    }
    memcpy(&RAM[memory_current_screen][display_get_addr(0,0)], screen.buffer, MONO_BITMAP_SIZE);
    utils_close_file(&screen);
    display_refresh_all();
