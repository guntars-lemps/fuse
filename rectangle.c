...
// redrawn, either by extending an existing rectangle or creating a new one/
void rectangle_add(int y, int x, int w)
{
    size_t i;
    struct rectangle *ptr;

    // Check through all 'active' rectangles (those which were modified on the previous line)
    // and see if we can use this new rectangle to extend them
    for (i = 0; i < rectangle_active_count; i++) {

        if ((rectangle_active[i].x == x) && (rectangle_active[i].w == w)) {
    ...
    if (++rectangle_active_count > rectangle_active_allocated) {
        ...
        new_alloc = rectangle_active_allocated ? (2 * rectangle_active_allocated) : 8;

        ptr = libspectrum_renew(struct rectangle, rectangle_active, new_alloc);

        rectangle_active_allocated = new_alloc;
        rectangle_active = ptr;
    }
...
static inline int compare_and_merge_rectangles(struct rectangle *source)
{
    size_t z;

    // Now look to see if there is an overlapping rectangle in the inactive list.
    // These occur when frame skip is on and the same lines are covered more than once...
    for (z = 0; z < rectangle_inactive_count; z++) {
        if ((rectangle_inactive[z].x == source->x) &&
            ...
            if (((rectangle_inactive[z].y < source->y) &&
                (source->y < (rectangle_inactive[z].y + rectangle_inactive[z].h + 1))) ||
        ...
        if ((rectangle_inactive[z].y == source->y) && (rectangle_inactive[z].h == source->h)) {

            if (((rectangle_inactive[z].x < source->x) &&
                (source->x < (rectangle_inactive[z].x + rectangle_inactive[z].w + 1))) ||
        ...
        // Handle overlaps offset by both x and y? how much overlap and hence overdraw can be tolerated?
    }
    return 0;
}
...
void rectangle_end_line(int y)
{
    ...
    for (i = 0; i < rectangle_active_count; i++) {
        ...
        if ((rectangle_active[i].y + rectangle_active[i].h) == (y + 1)) {
            continue;
        }

        if ((settings_current.frame_rate > 1) && compare_and_merge_rectangles(&rectangle_active[i])) {
        ...
        if (++rectangle_inactive_count > rectangle_inactive_allocated) {
            ...
            new_alloc = rectangle_inactive_allocated ? (2 * rectangle_inactive_allocated) : 8;

            ptr = libspectrum_renew(struct rectangle, rectangle_inactive, new_alloc);

            rectangle_inactive_allocated = new_alloc;
            rectangle_inactive = ptr;
        }
