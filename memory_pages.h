...
typedef struct memory_page {
    ...
    int save_to_snapshot; // Set if this page should be saved into snapshots
                          // (set only if this page would not normally be saved; things like RAM are always saved)
...
/* A memory page will be 1 << (this many) bytes in size
   ...
   without a full review of all relevant code and changes will be required to match
 */
...
// The number of memory pages in 64K. This calculation is equivalent to 2 ^ 16 / MEMORY_PAGE_SIZE
...
// Reset any memory configuration that may have changed in the machine configuration
...
// Use a macro for performance in the main core, but a function for flexibility in the core tester
...
typedef void (*memory_display_dirty_fn)(libspectrum_word address, libspectrum_byte b);
