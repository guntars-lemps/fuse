...
int snapshot_read(const char *filename)
{
    ...
    error = libspectrum_snap_read(snap, file.buffer, file.length, LIBSPECTRUM_ID_UNKNOWN, filename);
    if (error) {
...
int snapshot_copy_from(libspectrum_snap *snap)
{
    ...
    if (machine != machine_current->machine) {
        ...
        if (error) {
            ui_error(UI_ERROR_ERROR, "Loading a %s snapshot, but that's not available", libspectrum_machine_name(machine));
        }
    } else {
        machine_reset(0);
    }
    ...
    // Need to reset memory_map_[read|write] after all modules have had a turn initialising from the snapshot
...
int snapshot_write(const char *filename)
{
    ...
    // Work out what sort of file we want from the filename; default to .szx if we couldn't guess
