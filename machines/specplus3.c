...
static int specplus3_reset(void)
{
    ...
    error = machine_load_rom(0, settings_current.rom_plus3_0, settings_default.rom_plus3_0, 0x4000);
    if (error) {
        return error;
    }

    error = machine_load_rom(1, settings_current.rom_plus3_1, settings_default.rom_plus3_1, 0x4000);
    if (error) {
        return error;
    }

    error = machine_load_rom(2, settings_current.rom_plus3_2, settings_default.rom_plus3_2, 0x4000);
    if (error) {
        return error;
    }

    error = machine_load_rom(3, settings_current.rom_plus3_3, settings_default.rom_plus3_3, 0x4000);
    if (error) {
        return error;
    }
...
static void special_memory_map(int which)
{
    switch (which) {
        case 0:
            select_special_map(0, 1, 2, 3);
            break;
        case 1:
            select_special_map(4, 5, 6, 7);
            break;
        case 2:
            select_special_map(4, 5, 6, 3);
            break;
        case 3:
            select_special_map(4, 7, 6, 3);
            break;
        default:
...
void specplus3_memoryport2_write_internal(libspectrum_word port GCC_UNUSED, libspectrum_byte b)
{
...
int specplus3_memory_map(void)
{
    ...
    rom = ((machine_current->ram.last_byte  & 0x10) >> 4) | ((machine_current->ram.last_byte2 & 0x04) >> 1);

    // If we changed the active screen, mark the entire display file as dirty so we redraw it on the next pass
...
{
    ui_media_drive_update_menus(&ui_drives[SPECPLUS3_DRIVE_A], UI_MEDIA_DRIVE_UPDATE_ALL);
    ui_media_drive_update_menus(&ui_drives[SPECPLUS3_DRIVE_B], UI_MEDIA_DRIVE_UPDATE_ALL);
}


libspectrum_byte specplus3_fdc_status(libspectrum_word port GCC_UNUSED, libspectrum_byte *attached)
{
...
libspectrum_byte specplus3_fdc_read(libspectrum_word port GCC_UNUSED, libspectrum_byte *attached)
{
    *attached = 0xff; // TODO: check this
...
int specplus3_disk_insert(specplus3_drive_number which, const char *filename, int autoload)
{
    if (which >= SPECPLUS3_NUM_DRIVES) {
        ui_error(UI_ERROR_ERROR, "specplus3_disk_insert: unknown drive %d", which);
        fuse_abort();
    }
