...
static int pentagon1024_reset(void)
{
    ...
    error = machine_load_rom(0, settings_current.rom_pentagon1024_0, settings_default.rom_pentagon1024_0, 0x4000);
    if (error) {
        return error;
    }
    error = machine_load_rom(1, settings_current.rom_pentagon1024_1, settings_default.rom_pentagon1024_1, 0x4000);
    if (error) {
        return error;
    }
    error = machine_load_rom(2, settings_current.rom_pentagon1024_3, settings_default.rom_pentagon1024_3, 0x4000);
    if (error) {
        return error;
    }
    error = machine_load_rom_bank(beta_memory_map_romcs,
                                  ...
                                  settings_default.rom_pentagon1024_2,
                                  0x4000);
    if (error) {
        return error;
    }
...
void pentagon1024_memoryport_write(libspectrum_word port GCC_UNUSED, libspectrum_byte b)
{
...
void pentagon1024_v22_memoryport_write(libspectrum_word port GCC_UNUSED, libspectrum_byte b)
{
    if (machine_current->ram.locked) {
        return;
    }
...
static int pentagon1024_memory_map(void)
{
    ...
    if (!(machine_current->ram.last_byte2 & 0x04)) {
        page += ((machine_current->ram.last_byte & 0xC0) >> 3) + (machine_current->ram.last_byte & 0x20);
    }
