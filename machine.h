...
// How long do things take to happen; fields are pulled from libspectrum via the libspectrum_timings_* functions
typedef struct machine_timings {
    libspectrum_dword processor_speed; // Processor speed
    libspectrum_word left_border, horizontal_screen, right_border; // Line timings in tstates
    libspectrum_word tstates_per_line;
    libspectrum_word interrupt_length; // Interrupt length
    libspectrum_dword tstates_per_frame; // Frame timing
} machine_timings;
...
typedef struct fuse_machine_info {
    ...
    spectrum_raminfo ram; // How do we access memory, and what's currently paged in
    spectrum_unattached_port_fn unattached_port; // What to return if we read from a port which isn't attached to anything
...
int machine_load_rom_bank_from_buffer(memory_page* bank_map, int page_num, unsigned char *buffer, size_t length, int custom);
int machine_load_rom_bank(memory_page* bank_map, int page_num, const char *filename, const char *fallback, size_t expected_length);
