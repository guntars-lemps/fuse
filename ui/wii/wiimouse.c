...
/* The below is to remember last nunchuck position when navigating the menus.
   It is used so the user needs to release nunchuck stick and then press it again to
   go to the next menu item (holding the direction will to next menu item just once,
   no matter how long the direction is being held). */
...
{
    /* do this ONLY here. wiijoystick depends on it as well,
       but mouse_update is called regardless of whether the emulation is running or not,
       ui_joystick_poll only if running.
       So we do this here and risk lagging 1 frame behind on the joystick
       FIXME: A function that does this only once depending on the current frame counter would be better
    */
    ...
    /* we don't bother with key releases here; this is only for entering and/or using the menu,
       which seems to at best disregard keyreleases and at worst react badly to them.
    */
    ...
    if (fuse_emulation_paused) {
        ...
        } else if ((wm_down & WPAD_BUTTON_B) || (wm_down & WPAD_BUTTON_1) || (wm_down & WPAD_BUTTON_HOME))
