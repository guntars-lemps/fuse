...
#define POSTMODIFIER(wiikey, speckey) \
    if (queuepos < QUEUELEN) { \
        if (post_modifier(queue + queuepos, oldevent.modifiers, event.modifiers, wiikey, speckey) == 0) { \
            queuepos++;\
        }\
    }\
...
static int post_modifier(input_event_t* event, u8 old, u8 new, u8 modifier, input_key spectrum_key)
{
...
static void *kbdthread_fn(void *arg)
{
...
int wiikeyboard_init(void)
{
    ...
    if ((LWP_MutexInit(&kbdmutex, 0) != 0) || (LWP_CreateThread(&kbdthread, kbdthread_fn, NULL, NULL, 0, 80) != 0)) {
        IOS_Close(kbd);
        return 1;
    }
