        ...
        if (wpad->exp.type == EXP_NUNCHUK) {
            ...
            if (js.mag < 0.5) {
                ...
                /* the below prevents an issue when user makes 180 deg circle with
                   the nunchuck from for example left to right while mag > 0.5,
                   in which case spectrum would get both left and right signal from joystick.
                   The below is to prevent that. */
                if (!down_held) {
                    POST_JOYRELEASE(ctrlr, INPUT_JOYSTICK_DOWN);
                }
                if (!up_held) {
