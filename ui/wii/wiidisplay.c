...
// The scaled image
static unsigned char scaled_image[4 * 3 * DISPLAY_SCREEN_HEIGHT * (size_t)(1.5 * DISPLAY_SCREEN_WIDTH)];
...
static void put_pixel(int x, int y, int colour, int mouseputpixel)
{
    if (machine_current->timex) {
        ...
        display_image[y][x] = colour;
        if (!mouseputpixel && (x >= mousex) && (x < (mousex + MOUSESIZEX)) &&
...
static int init_colours(void)
{
    ...
    for (i = 0; i < 16; i++) {
        ...
        grey.r = grey.g = grey.b = ((0.299 * colour.r) + (0.587 * colour.g) + (0.114 * colour.b)) + 0.5;
...
static int register_scalers(void)
{
    ...
    ui_error(UI_ERROR_ERROR, "Unknown display size/image size %d/%d", wiidisplay_current_size, image_scale);
    return 1;
}
...
void wiidisplay_showmouse(float x, float y)
{
    ...
    if (((mousex <= 0) || (mousey <= 0)) && ((mousenewx <= 0) || (mousenewy <= 0))) {
        return;
    }

    // if we had a mouse pos before, put mousecache at old position into picture
    ...
    // we had an old mouse and have a new one. read mouse cache from the picture and draw mouse cursor.
    for (r = 0; r < MOUSESIZEY; r++) {
        for (c = 0; c < MOUSESIZEX; c++) {
