...
int menu_select_roms_with_title(const char *title, size_t start, size_t n, int is_peripheral)
{
    ...
    gtkstock_create_ok_cancel(dialog, NULL, G_CALLBACK(roms_done), &info, DEFAULT_DESTROY, DEFAULT_DESTROY);
    ...
    for (i = 0; i < n; i++) {
        add_rom(vbox, start, i, is_peripheral);
    }
...
static void add_rom(GtkBox *parent, size_t start, gint row, int is_peripheral)
{
    ...
    g_signal_connect(G_OBJECT(change_button), "clicked", G_CALLBACK(select_new_rom), rom[row]);
