...
static void select_row(GtkTreeView *treeview, GtkTreePath *path, GtkTreeViewColumn *col, gpointer user_data);
...
static GtkWidget  *blocks; // The list of blocks
static GtkWidget  *modified_label; // (T)he label saying if the tape has been modified
...
void menu_media_tape_browse(GtkAction *gtk_action GCC_UNUSED, gpointer data GCC_UNUSED)
{
...
static GtkWidget *create_block_list(void)
{
    GtkWidget *view;
    ...
    // Fast move tape
    g_signal_connect(G_OBJECT(view), "row-activated", G_CALLBACK(select_row), model);
...
static int create_dialog(void)
{
    ...
    dialog = gtkstock_dialog_new("Fuse - Browse Tape", G_CALLBACK(delete_dialog));
...
static void select_row(GtkTreeView *treeview GCC_UNUSED,
                       GtkTreePath *path,
                       ...
                       gpointer user_data)
{
    ...
    if (current_block != -1) {
        ...
        gtk_list_store_set(GTK_LIST_STORE(model), &iter, COL_PIX, tape_marker_pixbuf, -1);
    }
    ...
    if (current_block != -1) {
        ...
        gtk_list_store_set(GTK_LIST_STORE(model), &iter, COL_PIX, NULL, -1);
    }
}
...
static gboolean delete_dialog(GtkWidget *widget, GdkEvent *event GCC_UNUSED, gpointer user_data GCC_UNUSED)
{
    dialog_created = 0;
    gtk_widget_destroy(dialog);
    return TRUE;
