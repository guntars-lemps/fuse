...
static GdkPixbuf *pixbuf_tape_inactive;
static GdkPixbuf *pixbuf_tape_active;
static GdkPixbuf *pixbuf_mdr_inactive;
static GdkPixbuf *pixbuf_mdr_active;
static GdkPixbuf *pixbuf_disk_inactive;
static GdkPixbuf *pixbuf_disk_active;
static GdkPixbuf *pixbuf_pause_inactive;
static GdkPixbuf *pixbuf_pause_active;
static GdkPixbuf *pixbuf_mouse_inactive;
...
int gtkstatusbar_create(GtkBox *parent)
{
    ...
    pixbuf_tape_inactive = gdk_pixbuf_new_from_xpm_data(gtkpixmap_tape_inactive);
    pixbuf_tape_active = gdk_pixbuf_new_from_xpm_data(gtkpixmap_tape_active);

    pixbuf_mdr_inactive = gdk_pixbuf_new_from_xpm_data(gtkpixmap_mdr_inactive);
    pixbuf_mdr_active = gdk_pixbuf_new_from_xpm_data(gtkpixmap_mdr_active);

    pixbuf_disk_inactive = gdk_pixbuf_new_from_xpm_data(gtkpixmap_disk_inactive);
    pixbuf_disk_active = gdk_pixbuf_new_from_xpm_data(gtkpixmap_disk_active);

    pixbuf_pause_inactive = gdk_pixbuf_new_from_xpm_data(gtkpixmap_pause_inactive);
    pixbuf_pause_active = gdk_pixbuf_new_from_xpm_data(gtkpixmap_pause_active);

    pixbuf_mouse_inactive = gdk_pixbuf_new_from_xpm_data(gtkpixmap_mouse_inactive);
    pixbuf_mouse_active = gdk_pixbuf_new_from_xpm_data(gtkpixmap_mouse_active);
    ...
    gtk_box_pack_end(GTK_BOX(status_bar), microdrive_status, FALSE, FALSE, 0);
...
int ui_statusbar_update(ui_statusbar_item item, ui_statusbar_state state)
{
    ...
    switch (item) {
        ...
        case UI_STATUSBAR_ITEM_MOUSE:
            which = ((state == UI_STATUSBAR_STATE_ACTIVE) ? pixbuf_mouse_active : pixbuf_mouse_inactive);
            gtk_image_set_from_pixbuf(GTK_IMAGE(mouse_status), which);
            return 0;

        case UI_STATUSBAR_ITEM_PAUSED:
            which = ((state == UI_STATUSBAR_STATE_ACTIVE) ? pixbuf_pause_active : pixbuf_pause_inactive);
        ...
        case UI_STATUSBAR_ITEM_MICRODRIVE:
            switch (state) {
                case UI_STATUSBAR_STATE_NOT_AVAILABLE:
                    gtk_widget_hide(microdrive_status);
                    break;
                case UI_STATUSBAR_STATE_ACTIVE:
                    gtk_widget_show(microdrive_status);
                    gtk_image_set_from_pixbuf(GTK_IMAGE(microdrive_status), pixbuf_mdr_active);
                    break;
                default:
                    gtk_widget_show(microdrive_status);
                    gtk_image_set_from_pixbuf(GTK_IMAGE(microdrive_status), pixbuf_mdr_inactive);
                    break;
            }
            return 0;

        case UI_STATUSBAR_ITEM_TAPE:
            which = ((state == UI_STATUSBAR_STATE_ACTIVE) ? pixbuf_tape_active : pixbuf_tape_inactive);
