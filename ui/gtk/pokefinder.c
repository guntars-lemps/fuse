...
static void gtkui_pokefinder_incremented(GtkWidget *widget, gpointer user_data GCC_UNUSED);
static void gtkui_pokefinder_decremented(GtkWidget *widget, gpointer user_data GCC_UNUSED);
...
static void possible_click(GtkTreeView *treeview, GtkTreePath *path, GtkTreeViewColumn *col, gpointer user_data);

static int dialog_created = 0;

static GtkWidget *dialog; // The dialog box itself
static GtkWidget *count_label; // The number of possible locations
static GtkWidget *location_list; // The list view of possible locations
...
void menu_machine_pokefinder(GtkAction *gtk_action GCC_UNUSED, gpointer data GCC_UNUSED)
{
    int error;
...
static GtkWidget *create_location_list(void)
{
    GtkWidget *view;
...
static int create_dialog(void)
{
    ...
    dialog = gtkstock_dialog_new("Fuse - Poke Finder", G_CALLBACK(delete_dialog));
    ...
    entry = gtk_entry_new();
    g_signal_connect(G_OBJECT(entry), "activate", G_CALLBACK(gtkui_pokefinder_search), NULL);
    ...
    accel_group = gtkstock_create_buttons(dialog, NULL, btn, ARRAY_SIZE(btn));
    gtkstock_create_close(dialog, accel_group, G_CALLBACK(gtkui_pokefinder_close), TRUE);
...
static void gtkui_pokefinder_incremented(GtkWidget *widget GCC_UNUSED, gpointer user_data GCC_UNUSED)
{
...
static void gtkui_pokefinder_decremented(GtkWidget *widget GCC_UNUSED, gpointer user_data GCC_UNUSED)
{
...
static void gtkui_pokefinder_reset(GtkWidget *widget GCC_UNUSED, gpointer user_data GCC_UNUSED)
{
    pokefinder_clear();
    update_pokefinder();
}
...
static void update_pokefinder(void)
{
        ...
        for (page = 0; page < MEMORY_PAGES_IN_16K * SPECTRUM_RAM_PAGES; page++) {
            ...
            for (offset = 0; offset < MEMORY_PAGE_SIZE; offset++) {
                if (!(pokefinder_impossible[page][offset / 8] & (1 << (offset & 7)))) {
                    ...
                    gtk_list_store_set(GTK_LIST_STORE(location_model), &iter, COL_PAGE, possible_text[0], COL_OFFSET, possible_text[1], -1);
                }
            }
        }

        /* Show widget when the GtkTreeView has been filled with data.
           Fix an empty list on GTK+ 3.10 (and maybe other versions) */
    ...
    snprintf(buffer, 256, "Possible locations: %lu", (unsigned long)pokefinder_count);
    gtk_label_set_text(GTK_LABEL(count_label), buffer);
}


static void possible_click(GtkTreeView *treeview GCC_UNUSED,
                           GtkTreePath *path,
                           ...
                           gpointer user_data GCC_UNUSED)
{
    ...
    error = debugger_breakpoint_add_address(DEBUGGER_BREAKPOINT_TYPE_WRITE,
                                            memory_source_ram,
                                            possible_page[row],
                                            possible_offset[row],
                                            0,
