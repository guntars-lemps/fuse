...
static void add_click(GtkWidget *btn,
                      const gtkstock_button *button,
                      GtkAccelGroup *accel,
                      guint key1,
                      GdkModifierType mod1,
                      guint key2,
                      GdkModifierType mod2)
{
    if (button->shortcut) {
        if (button->shortcut != GDK_KEY_VoidSymbol) {
            gtk_widget_add_accelerator(btn, "clicked", accel, button->shortcut, button->modifier, 0);
        }
    } else if (key1) {
    ...
    if (button->shortcut_alt) {
        if (button->shortcut_alt != GDK_KEY_VoidSymbol) {
            gtk_widget_add_accelerator(btn, "clicked", accel, button->shortcut_alt, button->modifier_alt, 0);
        }
    } else if (key2) {
...
GtkWidget *gtkstock_create_button(GtkWidget *widget, GtkAccelGroup *accel, const gtkstock_button *button)
{
    GtkWidget *btn;
    ...
    if (!accel) {
        accel = gtkstock_add_accel_group (widget);
    }

    if (GTK_IS_DIALOG(widget)) {
        btn = gtk_dialog_add_button(GTK_DIALOG(widget), button_label, button->response_id);
    } else {
    ...
    if (button->action) {
        if (link_object) {
            g_signal_connect_swapped(G_OBJECT(btn), "clicked", button->action, G_OBJECT(button->actiondata));
        } else {
            g_signal_connect(G_OBJECT(btn), "clicked", button->action, button->actiondata);
        }
    }

    if (button->destroy) {
        g_signal_connect_swapped(G_OBJECT(btn), "clicked", button->destroy, G_OBJECT(widget));
    }
    ...
    // Note: ok/cancel/yes/no icons are not covered by freedesktop's Icon Naming Specification 0.8.90
        ...
        icon = gtk_image_new_from_icon_name(GTK_STOCK_CANCEL, GTK_ICON_SIZE_BUTTON);
                                         ...
                                         GCallback destroy_cancel)
{
    ...
    return gtkstock_create_buttons(widget, accel, btn, ARRAY_SIZE(btn));
}
...
GtkWidget *gtkstock_dialog_new(const gchar *title, GCallback destroy)
{
    ...
    g_signal_connect(G_OBJECT(dialog), "delete-event", (destroy ? destroy : DEFAULT_DESTROY), NULL);
    if (destroy == NULL) {
        gtk_window_set_modal(GTK_WINDOW(dialog), TRUE);
    }
    gtk_window_set_transient_for(GTK_WINDOW(dialog), GTK_WINDOW(gtkui_window));
