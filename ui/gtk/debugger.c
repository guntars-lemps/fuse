...
static void toggle_display_breakpoints(GtkToggleAction* action, gpointer data);
static void toggle_display_disassembly(GtkToggleAction* action, gpointer data);
...
static void stack_activate(GtkTreeView *tree_view, GtkTreePath *path, GtkTreeViewColumn *column, gpointer user_data);
static void create_events(GtkBox *parent);
static void events_activate(GtkTreeView *tree_view, GtkTreePath *path, GtkTreeViewColumn *column, gpointer user_data);
...
static gboolean disassembly_key_press(GtkTreeView *list, GdkEventKey *event, gpointer user_data);

static gboolean disassembly_wheel_scroll(GtkTreeView *list GCC_UNUSED, GdkEvent *event, gpointer user_data);
...
static void gtkui_debugger_done_continue(GtkWidget *widget, gpointer user_data);
static void gtkui_debugger_break(GtkWidget *widget, gpointer user_data);
static gboolean delete_dialog(GtkWidget *widget, GdkEvent *event, gpointer user_data);
static void gtkui_debugger_done_close(GtkWidget *widget, gpointer user_data);

static GtkWidget *dialog; // The debugger dialog box
static GtkWidget *continue_button, *break_button; // Two of its buttons
static GtkWidget *register_display; // The register display
static GtkWidget *registers[18]; // Individual registers
static GtkWidget *memory_map; // The memory map display
static GtkWidget *memory_map_table; // The table for the memory map
static GtkWidget *map_label[MEMORY_PAGES_IN_64K][4]; // Labels in the memory map
static GtkWidget *breakpoints; // The breakpoint display
static GtkWidget *disassembly_box; // A box to hold the disassembly
static GtkWidget *disassembly; // The actual disassembly widget
static GtkWidget *stack; // The stack display
static GtkWidget *events; // The events display

static GtkListStore *breakpoints_model, *disassembly_model, *stack_model, *events_model;
...
static const char *format_8_bit(void)
{
    return (debugger_output_base == 10) ? "%3d" : "0x%02X";
}


static const char *format_16_bit(void)
{
...
static GtkCheckMenuItem *get_pane_menu_item(debugger_pane pane)
{
    ...
    switch (pane) {
        ...
        case DEBUGGER_PANE_MEMORYMAP:
            path = "/View/Memory Map";
            break;
        case DEBUGGER_PANE_BREAKPOINTS:
            path = "/View/Breakpoints";
            break;
        case DEBUGGER_PANE_DISASSEMBLY:
            path = "/View/Disassembly";
            break;
    ...
    if (!menu_item) {
        ui_error(UI_ERROR_ERROR, "couldn't get menu item '%s'", path);
        return NULL;
    }
...
static int create_dialog(void)
{
    ...
    dialog = gtkstock_dialog_new("Fuse - Debugger", G_CALLBACK(delete_dialog));
    ...
    error = create_memory_map(GTK_BOX(hbox2));
    if (error) {
        return error;
    }
...
static int create_menu_bar(GtkBox *parent, GtkAccelGroup **accel_group)
{
    ...
    gtk_action_group_add_actions(menu_action_group, menu_data, ARRAY_SIZE(menu_data), NULL);
    gtk_action_group_add_toggle_actions(menu_action_group, menu_toggles, ARRAY_SIZE(menu_toggles), NULL);
    gtk_ui_manager_insert_action_group(ui_manager_debugger, menu_action_group, 0);
    g_object_unref(menu_action_group);

    // Load the menu
    ui_menu_id = gtk_ui_manager_add_ui_from_string(ui_manager_debugger, debugger_menu, sizeof(debugger_menu), &error);
    if (error) {
        g_error_free(error);
        return 1;
    } else if (!ui_menu_id) {
...
#if GTK_CHECK_VERSION(3, 0, 0)
        gtk_grid_attach(GTK_GRID(register_display), registers[i], (i % 2), (i / 2), 1, 1);
#else
        gtk_table_attach(GTK_TABLE(register_display), registers[i], (i % 2), ((i % 2) + 1), (i / 2), ((i / 2) + 1), 0, 0, 2, 2);
#endif
...
static int create_memory_map(GtkBox *parent)
{
    ...
    gtk_table_attach(GTK_TABLE(memory_map_table), label_address, 0, 1, 0, 1, 0, 0, 2, 2);
    gtk_table_attach(GTK_TABLE(memory_map_table), label_source, 1, 2, 0, 1, 0, 0, 2, 2);
    gtk_table_attach(GTK_TABLE(memory_map_table), label_writable, 2, 3, 0, 1, 0, 0, 2, 2);
    gtk_table_attach(GTK_TABLE(memory_map_table), label_contended, 3, 4, 0, 1, 0, 0, 2, 2);
...
static void create_breakpoints(GtkBox *parent)
{
    ...
    static const gchar *const titles[] = {"ID", "Type", "Value", "Ignore", "Life", "Condition"};

    breakpoints_model = gtk_list_store_new(BREAKPOINTS_COLUMN_COUNT,
                                           G_TYPE_INT,
                                           G_TYPE_STRING,
                                           G_TYPE_STRING,
                                           G_TYPE_INT,
...
static void create_disassembly(GtkBox *parent, gtkui_font font)
{
    ...
    disassembly_model = gtk_list_store_new(DISASSEMBLY_COLUMN_COUNT, G_TYPE_STRING, G_TYPE_STRING);
    ...
    disassembly_scrollbar_adjustment = GTK_ADJUSTMENT(gtk_adjustment_new(0, 0x0000, 0x10000, 1, 20, 20));
    g_signal_connect(G_OBJECT(disassembly_scrollbar_adjustment), "value-changed", G_CALLBACK(move_disassembly), NULL);
    scrollbar = gtk_scrollbar_new(GTK_ORIENTATION_VERTICAL, disassembly_scrollbar_adjustment);
    gtk_box_pack_start(GTK_BOX(disassembly_box), scrollbar, FALSE, FALSE, 0);

    // Scrolling with keys
    g_signal_connect(GTK_TREE_VIEW(disassembly),
                     "key-press-event",
...
static void create_stack_display(GtkBox *parent, gtkui_font font)
{
    ...
    stack_model =
    gtk_list_store_new(STACK_COLUMN_COUNT, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_INT);
...
static void stack_activate(GtkTreeView *tree_view,
                           GtkTreePath *path,
...
static void events_activate(GtkTreeView *tree_view,
                            GtkTreePath *path,
                            GtkTreeViewColumn *column GCC_UNUSED,
                            gpointer user_data GCC_UNUSED)
{
...
static int create_command_entry(GtkBox *parent, GtkAccelGroup *accel_group)
{
    ...
    entry = gtk_entry_new();
    g_signal_connect(G_OBJECT(entry), "activate", G_CALLBACK(evaluate_command), NULL);
    gtk_box_pack_start(GTK_BOX(hbox), entry, TRUE, TRUE, 0);

    // The 'command evaluate' button
    eval_button = gtk_button_new_with_label("Evaluate");
    g_signal_connect_swapped(G_OBJECT(eval_button), "clicked", G_CALLBACK(evaluate_command), G_OBJECT(entry));
    gtk_box_pack_start(GTK_BOX(hbox), eval_button, FALSE, FALSE, 0);

    // Return is equivalent to clicking on 'evaluate'
    gtk_widget_add_accelerator(eval_button, "clicked", accel_group, GDK_KEY_Return, 0, 0);
...
static int create_buttons(GtkDialog *parent, GtkAccelGroup *accel_group)
{
    ...
    continue_button = gtkstock_create_button(GTK_WIDGET(parent), accel_group, &cont);
    break_button = gtkstock_create_button(GTK_WIDGET(parent), accel_group, &brk);
    gtkstock_create_close(GTK_WIDGET(parent), accel_group, G_CALLBACK(gtkui_debugger_done_close), TRUE);
...
int ui_debugger_update(void)
{
    ...
    const char *register_name[] = {"PC", "SP", "AF", "AF'", "BC", "BC'", "DE", "DE'", "HL", "HL'", "IX", "IY"};

    libspectrum_word *value_ptr[] = {&PC, &SP, &AF, &AF_, &BC, &BC_, &DE, &DE_, &HL, &HL_, &IX, &IY};
    ...
    strcpy(buffer, "  I   ");
    snprintf(&buffer[6], 76, format_8_bit(), I);
    ...
    if (capabilities & LIBSPECTRUM_MACHINE_CAPABILITY_AY) {
        ...
        snprintf(&buffer[length], (1024 - length), format_string, machine_current->ay.current_register);
    }

    if (capabilities & LIBSPECTRUM_MACHINE_CAPABILITY_128_MEMORY) {
        sprintf(format_string, "\n128Mem %s", format_8_bit());
        length = strlen(buffer);
        snprintf(&buffer[length], (1024 - length), format_string, machine_current->ram.last_byte);
    }

    if (capabilities & LIBSPECTRUM_MACHINE_CAPABILITY_PLUS3_MEMORY) {
        sprintf(format_string, "\n+3 Mem %s", format_8_bit());
        length = strlen(buffer);
        snprintf(&buffer[length], (1024 - length), format_string, machine_current->ram.last_byte2);
    }
    ...
    if ((capabilities & LIBSPECTRUM_MACHINE_CAPABILITY_TIMEX_VIDEO)  ||
        ...
        length = strlen(buffer);
        snprintf(&buffer[length], (1024 - length), format_string, scld_last_dec.byte);
    }
    ...
    if (settings_current.zxcf_active) {
        sprintf(format_string, "\n  ZXCF %s", format_8_bit());
        length = strlen(buffer);
        snprintf(&buffer[length], (1024 - length), format_string, zxcf_last_memctl());
    }
    ...
    for (i = 0, address = SP + 38; i < 20; i++, address -= 2) {
        ...
        gtk_list_store_set(stack_model,
                           &it,
                           STACK_COLUMN_ADDRESS,
                           buffer1,
                           STACK_COLUMN_VALUE_TEXT,
...
static void update_memory_map(void)
{
    ...
    for (block = 0; block < MEMORY_PAGES_IN_64K; block++) {
        ...
        if ((page->source != source) || (page->page_num != page_num) || (page->offset != offset) ||
            ...
            snprintf(buffer, 40, "%s %d", memory_source_description(page->source), page->page_num);
...
#if GTK_CHECK_VERSION(3, 0, 0)
                gtk_grid_attach(GTK_GRID(memory_map_table), row_labels[i], i, (row + 1), 1, 1);
#else
                gtk_table_attach(GTK_TABLE(memory_map_table), row_labels[i], i, (i + 1), (row + 1), (row + 2), 0, 0, 2, 2);
#endif
        ...
        switch (bp->type) {
            ...
            case DEBUGGER_BREAKPOINT_TYPE_WRITE:
                if (bp->value.address.source == memory_source_any) {
                    ...
                    snprintf(format_string, sizeof(format_string), "%%s:%s:%s", format_16_bit(), format_16_bit());
                    snprintf(buffer,
                             sizeof(buffer),
                             format_string,
            ...
            case DEBUGGER_BREAKPOINT_TYPE_PORT_WRITE:
                snprintf(format_string, sizeof(format_string), "%s:%s", format_16_bit(), format_16_bit());
                snprintf(buffer, sizeof(buffer), format_string, bp->value.port.mask, bp->value.port.port);
                break;
            ...
            case DEBUGGER_BREAKPOINT_TYPE_EVENT:
                snprintf(buffer, sizeof(buffer), "%s:%s", bp->value.event.type, bp->value.event.detail);
                break;

        }

        gtk_list_store_append(breakpoints_model, &it);
        gtk_list_store_set(breakpoints_model,
                           &it,
                           BREAKPOINTS_COLUMN_ID,
                           bp->id,
                           BREAKPOINTS_COLUMN_TYPE,
...
static void update_disassembly(void)
{
    ...
    for (i = 0, address = disassembly_top; i < 20; i++) {
        ...
        gtk_list_store_set(disassembly_model,
                           &it,
                           DISASSEMBLY_COLUMN_ADDRESS,
                           buffer1,
                           DISASSEMBLY_COLUMN_INSTRUCTION,
...
static void add_event(gpointer data, gpointer user_data GCC_UNUSED)
{
    ...
    if (ptr->type != event_type_null) {
        ...
        gtk_list_store_set(events_model,
                           &it,
                           EVENTS_COLUMN_TIME,
                           ptr->tstates,
                           EVENTS_COLUMN_TYPE,
...
int ui_debugger_disassemble(libspectrum_word address)
{
    ...
    g_signal_handlers_block_by_func(G_OBJECT(disassembly_scrollbar_adjustment),
                                    G_CALLBACK(move_disassembly),
                                    NULL);

    // Note: GtkAdjustment can not cope with "upper bound - page_size" value and higher
    gtk_adjustment_set_value(disassembly_scrollbar_adjustment, address);

    // Enable events for scrollbar
    g_signal_handlers_unblock_by_func(G_OBJECT(disassembly_scrollbar_adjustment), G_CALLBACK(move_disassembly), NULL);
...
static void move_disassembly(GtkAdjustment *adjustment, gpointer user_data GCC_UNUSED)
{
    ...
    if ((value > disassembly_top) && ((value - disassembly_top) <= 1)) {
        ...
        /* disassembly_top - 1 <= value < disassembly_top => 'up' button pressed
           ...
           Unfortunately, it's not trivial to determine where disassembly should now start,
           as we have variable length instructions of unbounded length
           (multiple DD and FD prefixes on one instruction are possible).

           In general, we want the _longest_ opcode which produces the
           current top in second place (consider something like LD A,nn:
           we're not interested if nn happens to represent a one-byte opcode),
           so look back a reasonable length (say, 8 bytes) and see what we find.

           In some cases (eg if we're currently pointing to a data byte of a multi-byte opcode),
           it will be impossible to get the current top second. In this case, just move back a byte.
...
static void gtkui_debugger_done_step(GtkWidget *widget GCC_UNUSED, gpointer user_data GCC_UNUSED)
{
    debugger_step();
}


static void gtkui_debugger_done_continue(GtkWidget *widget GCC_UNUSED, gpointer user_data GCC_UNUSED)
{
    debugger_run();
}


static void gtkui_debugger_break(GtkWidget *widget GCC_UNUSED, gpointer user_data GCC_UNUSED)
{
...
static gboolean delete_dialog(GtkWidget *widget, GdkEvent *event GCC_UNUSED, gpointer user_data)
{
