...
static void create_binary_dialog(struct binary_info *info, const char *title)
{
    ...
    g_signal_connect(G_OBJECT(button), "clicked", info->change_filename, info);

    label_start = gtk_label_new("Start");

    info->start_widget = gtk_entry_new();
    g_signal_connect(G_OBJECT(info->start_widget), "activate", info->activate_data, info);
    ...
    g_signal_connect(G_OBJECT(info->length_widget), "activate", info->activate_data, info);
    ...
    gtk_grid_attach(GTK_GRID(table), info->filename_widget, 1, 0, 1, 1);
    ...
    gtk_table_attach(GTK_TABLE(table), label_filename, 0, 1, 0, 1, GTK_FILL, GTK_FILL, 3, 3);
    gtk_table_attach(GTK_TABLE(table), info->filename_widget, 1, 2, 0, 1, GTK_FILL, GTK_FILL, 3, 3);
    ...
    gtk_table_attach(GTK_TABLE(table), info->start_widget, 1, 3, 1, 2, GTK_FILL, GTK_FILL, 3, 3);
    gtk_table_attach(GTK_TABLE(table), label_length, 0, 1, 2, 3, GTK_FILL, GTK_FILL, 3, 3);
    gtk_table_attach(GTK_TABLE(table), info->length_widget, 1, 3, 2, 3, GTK_FILL, GTK_FILL, 3, 3);
    ...
    // Command buttons
    gtkstock_create_ok_cancel(info->dialog, NULL, info->activate_data, info, DEFAULT_DESTROY, DEFAULT_DESTROY);
}


void menu_file_loadbinarydata(GtkAction *gtk_action GCC_UNUSED, gpointer data GCC_UNUSED)
{
...
void menu_file_savebinarydata(GtkAction *gtk_action GCC_UNUSED, gpointer data GCC_UNUSED)
{
    struct binary_info info;
