...
/* A copy of every pixel on the screen, replaceable by plotting directly into
   rgb_image below */
libspectrum_word gtkdisplay_image[2 * DISPLAY_SCREEN_HEIGHT][DISPLAY_SCREEN_WIDTH];
...
static guchar rgb_image[4 * 2 * (DISPLAY_SCREEN_HEIGHT + 4) * (DISPLAY_SCREEN_WIDTH  + 3)];
static const gint rgb_pitch = (DISPLAY_SCREEN_WIDTH + 3) * 4;

// The scaled image
static guchar scaled_image[3 * DISPLAY_SCREEN_HEIGHT * 6 * DISPLAY_SCREEN_WIDTH];
...
#if !GTK_CHECK_VERSION(3, 0, 0)
static gint gtkdisplay_expose(GtkWidget *widget, GdkEvent *event, gpointer data);
#else
static gboolean gtkdisplay_draw(GtkWidget *widget, cairo_t *cr, gpointer user_data);
#endif // #if !GTK_CHECK_VERSION(3, 0, 0)

static gint drawing_area_resize_callback(GtkWidget *widget, GdkEvent *event, gpointer data);
...
int uidisplay_init(int width, int height)
{
    ...
    g_signal_connect(G_OBJECT(gtkui_drawing_area), "expose_event", G_CALLBACK(gtkdisplay_expose), NULL);

    colour_format = FORMAT_x8b8g8r8;

    g_signal_connect(G_OBJECT(gtkui_drawing_area), "configure_event", G_CALLBACK(drawing_area_resize_callback), NULL);

#else

    g_signal_connect(G_OBJECT(gtkui_drawing_area), "draw", G_CALLBACK(gtkdisplay_draw), NULL);

    colour_format = FORMAT_x8r8g8b8;

    g_signal_connect(G_OBJECT(gtkui_window), "configure_event", G_CALLBACK(drawing_area_resize_callback), NULL);

#endif // #if !GTK_CHECK_VERSION(3, 0, 0)

    error = init_colours(colour_format);
    if (error) {
        return error;
    }
...
static int drawing_area_resize(int width, int height, int force_scaler)
{
    ...
    surface = cairo_image_surface_create_for_data(scaled_image,
                                                  CAIRO_FORMAT_RGB24,
                                                  (scale * image_width),
                                                  (scale * image_height),
                                                  scaled_pitch);
...
static void register_scalers(int force_scaler)
{
    ...
    scaler = scaler_is_supported(current_scaler) ? current_scaler : SCALER_NORMAL;
    ...
    if (force_scaler && (drawing_area_scale != scaling_factor)) {
        ...
        switch (gtkdisplay_current_size) {
            case 1:
                scaler = machine_current->timex ? SCALER_HALF : SCALER_NORMAL;
                break;
            case 2:
                scaler = machine_current->timex ? SCALER_NORMAL : SCALER_DOUBLESIZE;
                break;
            case 3: scaler = machine_current->timex ? SCALER_TIMEX1_5X : SCALER_TRIPLESIZE;
                break;
        }
    }
...
#if GTK_CHECK_VERSION(3, 0, 0)
    if (display_updated) {
        gdk_window_process_updates(gtk_widget_get_window(gtkui_drawing_area), FALSE);
        display_updated = 0;
    }
#endif // #if GTK_CHECK_VERSION(3, 0, 0)
...
void uidisplay_area(int x, int y, int w, int h)
{
    ...
    for (yy = y; yy < (y + h); yy++) {
        ...
        libspectrum_dword *rgb;
        libspectrum_word *display;
...
static gint gtkdisplay_expose(GtkWidget *widget GCC_UNUSED, GdkEvent *event, gpointer data GCC_UNUSED)
{
    gtkdisplay_area(event->expose.area.x, event->expose.area.y, event->expose.area.width, event->expose.area.height);
    return TRUE;
}
...
static gint drawing_area_resize_callback(GtkWidget *widget GCC_UNUSED, GdkEvent *event, gpointer data GCC_UNUSED)
{
...
static gint drawing_area_resize_callback(GtkWidget *widget GCC_UNUSED, GdkEvent *event, gpointer data GCC_UNUSED)
{
    drawing_area_resize(event->configure.width, (event->configure.height - extra_height), 1);
...
void gtkdisplay_update_geometry(void)
{
    ...
    if (settings_current.aspect_hint) {
        ...
        geometry.min_aspect = geometry.max_aspect = (scale * DISPLAY_ASPECT_WIDTH) /
    ...
    gtk_window_set_geometry_hints(GTK_WINDOW(gtkui_window), geometry_widget, &geometry, hints);
}
...
static void gtkdisplay_load_gfx_mode(void)
{
    ...
    gtk_window_resize(GTK_WINDOW(gtkui_window), (scale * image_width), (scale * image_height) + extra_height);
