...
/* Debugger update function. The dialog box is created every time it is displayed,
...
int widget_debugger_draw(void *data)
{
    ...
    widget_printstring(LC(10), (LR(15) - 4), 6, "\022S\021ingle step  \022C\021ontinue  Co\022m\021mand");
...
static void display_registers(void)
{
    ...
    if (capabilities & LIBSPECTRUM_MACHINE_CAPABILITY_AY) {
        show_register1(LC(37), LR(4), "AY", machine_current->ay.current_register);
    }

    if (capabilities & LIBSPECTRUM_MACHINE_CAPABILITY_128_MEMORY) {
        show_register1(LC(6), LR(5), "128Mem", machine_current->ram.last_byte);
    }

    if (capabilities & LIBSPECTRUM_MACHINE_CAPABILITY_128_MEMORY) {
        show_register1(LC(15), LR(5), "+3Mem", machine_current->ram.last_byte2);
    }
    ...
    for (block = 0; block < MEMORY_PAGES_IN_64K; block++) {
        ...
        if ((page->source != source) ||
            ...
            snprintf(pbuf, sizeof(pbuf), "%s %d", memory_source_description(memory_map_read[block].source),
...
static void display_bytes(void)
{
    ...
    for (y = 0; y < 8; ++y) {
        ...
        for (x = 0; x < 8; ++x) {
            sprintf((pbuf + (x * 4)), format_8_bit(), readbyte_internal(addr + x));
            if (x < 7) {
                strcat(pbuf, "  ");
            }
        }
...
static void display_text(void)
{
    ...
    for (y = 0; y < 8; ++y) {
        ...
        for (x = 0; x < 32; ++x) {
            widget_printchar_fixed((LC(x + 8) / 8), (LR(y) / 8), 7, readbyte_internal(addr + x));
        }
    }
}
...
static void display_breakpts(void)
{
    ...
    for (ptr = debugger_breakpoints; (i < 8) && ptr; ptr = ptr->next, ++i) {
        ...
        widget_printstring(LC(6), LR(i), 7, debugger_breakpoint_type_abbr[bp->type]);
        ...
        switch (bp->type) {
            ...
            case DEBUGGER_BREAKPOINT_TYPE_WRITE:
                if (bp->value.address.source == memory_source_any) {
                    ...
                    snprintf(fmt, sizeof(fmt), "%%s:%s:%s", format_16_bit(), format_16_bit());
                    snprintf(pbuf,
                             sizeof(pbuf),
                             fmt,
