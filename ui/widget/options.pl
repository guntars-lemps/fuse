...
static void widget_combo_click(const char *title, const char * const *options, char **current, int def)
{
    int error, i;
    widget_select_t sel;
...
static int option_enumerate_combo(const char * const *options, char *value, int def)
{
    int i;
    if (value != NULL) {
        for (i = 0; options[i] != NULL; i++) {
...
int widget_options_print_entry(int left_edge, int width, int number, const char *prefix, int value, const char *suffix);
int widget_options_print_combo(int left_edge,
                               int width,
                               int number,
                               const char *prefix,
                               const char * const *options,
...
static int widget_options_print_label(int left_edge,
                                      int width,
                                      int number,
                                      const char *string);
static int widget_options_print_data(int left_edge,
                                     int menu_width,
                                     int number,
                                     const char *string,
                                     int tcolor);
...
static int widget_options_show_all(widget_option_entry *options, settings_info *show)
{
...
static int widget_options_print_label(int left_edge, int width, int number, const char *string)
{
    ...
    buffer[l] = '\\0';
    w = widget_printstring(((left_edge * 8) + 8), ((number * 8) + 24), WIDGET_COLOUR_FOREGROUND, buffer) - 1;
...
static int widget_calculate_option_width(widget_option_entry *menu)
{
    ...
    for (ptr = &menu[1]; ptr->text; ptr++) {
        ...
        /* If this is a number format, leave room for 4 digits as well as the text */
        if (ptr->suffix) {
            total_width += widget_stringwidth(ptr->suffix) + (4 * 8);
        }
        if (ptr->options) {
            int combo_width = 0;
...
int widget_$_->{name}_draw(void *data GCC_UNUSED)
{
    ...
    if (!widget_$_->{name}_running) {
        /* we want to copy settings, only when start up */
        highlight_line = 0;
...
static void widget_$widget->{value}_click(void)
{
    ...
    snprintf(text_data.text, 40, "%d", widget_options_settings.$widget->{value});
...
static void widget_$widget->{value}_click(void)
{
    widget_combo_click("$title",
                       widget_$widget->{value}_combo,
...
CODE
	...
	if( $widget->{type} eq "Checkbox" ) {
	    ...
	    print << "CODE";
static void widget_option_$widget->{value}_draw(int left_edge,
                                                int width,
                                                struct widget_option_entry *menu,
                                                settings_info *show)
{
...
static void widget_option_$widget->{value}_draw(int left_edge,
                                                ...
                                                settings_info *show)
{
    widget_options_print_entry(left_edge, width, menu->index, menu->text, show->$widget->{value}, menu->suffix);
}
...
static void widget_option_$widget->{value}_draw(int left_edge,
                                                ...
                                                settings_info *show)
{
    widget_options_print_combo(left_edge,
                               width,
                               menu->index,
                               menu->text,
                               menu->options,
...
void widget_$_->{name}_keyhandler(input_key key)
{
    ...
    switch (key) \{
        ...
        case INPUT_JOYSTICK_RIGHT:
            ...
            options_$_->{name}\[highlight_line + 1\].draw(menu_left_edge_x,
                                                          menu_width,
                                                          options_$_->{name} + highlight_line + 1,
    ...
    if (cursor_pressed) \{
        ...
        options_$_->{name}\[old_highlight_line + 1\].draw(menu_left_edge_x,
                                                          menu_width,
                                                          options_$_->{name} + old_highlight_line + 1,
                                                          &widget_options_settings);
        options_$_->{name}\[highlight_line + 1\].draw(menu_left_edge_x,
                                                      menu_width,
                                                      options_$_->{name} + highlight_line + 1,
