...
int widget_pokefinder_draw(void *data)
{
    ...
    widget_printstring(16, 88, WIDGET_COLOUR_FOREGROUND, "\x0AI\x01nc'd \x0A" "D\x01" "ec'd \x0AS\x01" "earch");
...
static void display_possible(void)
{
    ...
    if (FEW_ENOUGH()) {
        ...
        for (i = 0; i < pokefinder_count; i++) {
            ...
            snprintf(buf, sizeof(buf), "%2d:%04X", possible_page[i], possible_offset[i]);
    ...
    switch (key) {
        ...
        case INPUT_KEY_b: // Add breakpoint
            if (FEW_ENOUGH()) {
                ...
                if (debugger_breakpoint_add_address(DEBUGGER_BREAKPOINT_TYPE_WRITE,
                                                    memory_source_ram,
                                                    possible_page[selected],
                                                    possible_offset[selected],
                                                    0,
                                                    DEBUGGER_BREAKPOINT_LIFE_PERMANENT,
                                                    NULL)) {
                    widget_printstring(16, 80, WIDGET_COLOUR_FOREGROUND, "Breakpoint failed");
                } else {
                    widget_printstring(16, 80, WIDGET_COLOUR_FOREGROUND, "Breakpoint added");
                }
