...
struct widget_dirent **widget_filenames; // Filenames in the current directory
size_t widget_numfiles; // (T)he number of files in the current directory
...
/* The number of the filename in the top-left corner of the current display,
   that of the filename which the `cursor' is on,
   and that which it will be on after this keypress */
static size_t top_left_file, current_file, new_current_file;

static char *widget_get_filename(const char *title, int saving);

static int widget_add_filename(int *allocated, int *number, struct widget_dirent ***namelist, const char *name);
static void widget_scan(char *dir);
static int widget_select_file(const char *name);
static int widget_scan_compare(const widget_dirent **a, const widget_dirent **b);

#if !defined AMIGA && !defined __MORPHOS__
static char* widget_getcwd(void);
#endif // ifndef AMIGA
static int widget_print_all_filenames(struct widget_dirent **filenames,
                                      int n,
                                      int top_left,
                                      int current,
                                      const char *dir);
static int widget_print_filename(struct widget_dirent *filename, int position, int inverted);
...
static int widget_add_filename(int *allocated,
                               int *number,
...
char *amiga_asl(char *title, BOOL is_saving)
{
    ...
    if (AslBase = OpenLibrary("asl.library", 0)) {
        filereq = AllocAslRequestTags(ASL_FileRequest,
                                      ...
                                      "#?.(sna|z80|szx|sp|snp|zxs|tap|tzx|csw|rzx|dsk|trd|scl|"
                                          "mdr|dck|hdf|rom|psg|scr|mlt|png|gz|bz2)",
                                      ASLFR_DoPatterns,
                                      TRUE,
                                      TAG_DONE);
    ...
    if (AslBase = IExec->OpenLibrary("asl.library", 52)) {
        if (IAsl = (struct AslIFace *)IExec->GetInterface(AslBase, "main", 1, NULL)) {
            filereq = IAsl->AllocAslRequestTags(ASL_FileRequest,
                                                ...
                                                is_saving,
                                                ASLFR_InitialPattern,
                                                "#?.(sna|z80|szx|sp|snp|zxs|tap|tzx|csw|rzx|dsk|trd|scl|mdr|dck|hdf|"
                                                    "rom|psg|scr|mlt|png|gz|bz2)",
                                                ASLFR_DoPatterns,
                                                TRUE,
                                                TAG_DONE);
...
static int widget_scandir(const char *dir, struct widget_dirent ***namelist, int (*select_fn)(const char*))
{
...
static void widget_scan(char *dir)
{
    ...
    qsort(widget_filenames,
          widget_numfiles,
          sizeof(struct widget_dirent*),
...
static int widget_scan_compare(const struct widget_dirent **a, const struct widget_dirent **b)
{
    int isdir1 = S_ISDIR((*a)->mode), isdir2 = S_ISDIR((*b)->mode);
...
#endif // !defined AMIGA && !defined __MORPHOS__
...
static int widget_filesel_draw(void *data)
{
    ...
    widget_print_all_filenames(widget_filenames, widget_numfiles, top_left_file, current_file, directory);
...
static char* widget_getcwd(void)
{
    ...
    do {
        ...
        if (errno == ERANGE) {
            ptr = directory;
            directory_length *= 2;
            directory = (char*)realloc(directory, (directory_length * sizeof(char)));
            if (directory == NULL) {
                free(ptr);
                return NULL;
            }
                                      ...
                                      const char *dir)
{
    ...
    if (widget_stringwidth(dir) > 223) {
        ...
        while (widget_stringwidth(dir) > (223 - prefix)) {
            dir++;
        }
    ...
    if (top_left) {
        widget_up_arrow(1, 5, WIDGET_COLOUR_FOREGROUND);
    }

    // Print the filenames, mostly normally, but with the currently selected file inverted
    ...
    if (is_saving) {
        widget_printstring(12, (22 * 8), WIDGET_COLOUR_FOREGROUND, "\012RETURN\001 = select");
        widget_printstring_right(244, (22 * 8), WIDGET_COLOUR_FOREGROUND, "\012TAB\001 = enter name");
    }
...
#define MAX_SUFFIX_WIDTH 56
    ...
    background = inverted ? WIDGET_COLOUR_HIGHLIGHT : WIDGET_COLOUR_BACKGROUND;
    ...
    if (dot) {
        ...
        while ((suffix_width = (dot && !dir) ? widget_stringwidth(suffix) : 0) > 110) {
            suffix_truncated = 1;
            suffix[strlen(suffix) - 1] = '\0';
        }
    }

    while ((width = widget_stringwidth(buffer)) >= (FILENAME_WIDTH - dir - (dot ? (truncated + suffix_width) : 0))) {
        truncated = 2;
    ...
    if (dot) {
        widget_printstring((x + width + 2 + truncated), y, (foreground ^ 2), suffix);
    }
    if (suffix_truncated) {
...
static int widget_filesel_chdir(void)
{
    ...
    ptr = fn;
    fn = realloc(fn, ((strlen(fn) + 1 + strlen(widget_filenames[current_file]->name) + 1) * sizeof(char)));
    if (fn == NULL) {
        free(ptr);
        widget_end_widget(WIDGET_FINISHED_CANCEL);
        return 1;
...
void widget_filesel_keyhandler(input_key key)
{
    ...
    switch (key) {
        ...
        case INPUT_KEY_Resize: // Fake keypress used on window resize
            ...
            widget_print_all_filenames(widget_filenames, widget_numfiles, top_left_file, current_file);
            break;
#endif
        ...
        case INPUT_JOYSTICK_DOWN:
            if ((current_file + 2) < widget_numfiles) {
                new_current_file += 2;
            }
            break;
        ...
        case INPUT_JOYSTICK_RIGHT:
            if (current_file < (widget_numfiles - 1)) {
                new_current_file++;
            }
            break;

        case INPUT_KEY_Page_Up:
            new_current_file = (current_file > ENTRIES_PER_SCREEN) ? (current_file - ENTRIES_PER_SCREEN) : 0;
            break;
        ...
        case INPUT_KEY_Tab:
            if (is_saving) {
                ...
                text_data.text[0] = 0;
                if (widget_do_text(&text_data) || !widget_text_text || !*widget_text_text) {
                    break;
                }
    ...
    if (new_current_file != current_file) {
        ...
        /* If we've got off the top or bottom of the currently displayed file list,
           then reset the top-left corner and display the whole thing */
        if (new_current_file < top_left_file) {

            top_left_file = new_current_file & ~1;
            widget_print_all_filenames(widget_filenames, widget_numfiles, top_left_file, new_current_file, dirtitle);
            ...
            // Otherwise, print the current file uninverted and the  new current file inverted

            widget_print_filename(widget_filenames[current_file], (current_file - top_left_file), 0);

            widget_print_filename(widget_filenames[new_current_file], (new_current_file - top_left_file), 1);
