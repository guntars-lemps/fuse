...
/* The widgets actually available.
   Make sure the order here matches the order defined in enum widget_type (widget.h) */
...
static int widget_read_font(const char *filename)
{
    ...
    while (i < file.length) {
        ...
        if ((page != 0) || ((i + 3 + width) > file.length) || ((left >= 0) && ((left + width) > 8))) {
            ui_error(UI_ERROR_ERROR, "font contains invalid character");
            utils_close_file(&file);
            return 1;
        }
...
static const widget_font_character *widget_char(int pp)
{
    if ((pp < 0) || (pp >= 256)) {
...
int widget_printstring(int x, int y, int col, const char *s)
{
    ...
    while ((x < (256 + DISPLAY_BORDER_ASPECT_WIDTH)) && ((c = *(libspectrum_byte *)s++) != 0)) {
        if ((col == WIDGET_COLOUR_DISABLED) && (c < 26)) {
            continue;
        }
...
void widget_printchar_fixed(int x, int y, int col, int c)
{
    ...
    if (c < 128) {
        ...
        if (c & 1) {
            widget_rectangle((x + 4), y, 4, 4, col);
        }
        if (c & 2) {
            widget_rectangle(x, y, 4, 4, col);
        }
        if (c & 4) {
            widget_rectangle((x + 4), (y + 4), 4, 4, col);
        }
        if (c & 8) {
            widget_rectangle(x, (y + 4), 4, 4, col);
        }
        return;
    } else if (c < 165) {
        inverse = 1;
...
void widget_display_rasters(int y, int h)
{
    ...
    uidisplay_area(0, (scale * (DISPLAY_BORDER_HEIGHT + y)), (scale * DISPLAY_ASPECT_WIDTH), (scale * h));
    uidisplay_frame_end();
}
...
int widget_do(widget_type which, void *data)
{
    ...
    if (ui_widget_level >= 0) {
        ...
        if (!widget_return[ui_widget_level].finished) {
            widget_keyhandler = widget_data[widget_return[ui_widget_level].type].keyhandler;
            widget_data[widget_return[ui_widget_level].type].draw(widget_return[ui_widget_level].data);
        }
    } else {
...
int widget_calculate_menu_width(widget_menu_entry *menu)
{
    ...
    for (ptr = &menu[1]; ptr->text; ptr++) {
        ...
        if (ptr->detail) {
            total_width += widget_stringwidth(ptr->detail()) + (2 * 8);
        }
...
int widget_dialog_with_border(int x, int y, int width, int height)
{
    ...
    widget_draw_rectangle_solid(((8 * x) + 1), ((8 * y) + 1), ((8 * width) - 2), ((8 * height) - 2), paper);
...
{
    uidisplay_putpixel((x + DISPLAY_BORDER_ASPECT_WIDTH), (y + DISPLAY_BORDER_HEIGHT), colour);
}
...
/* Tape browser update function.
   The dialog box is created every time it is displayed, so no need to do anything here */
