...
int split_message(const char *message, char ***lines, size_t *count, size_t line_length)
{
    ...
    // Setup so we'll allocate the first line as soon as we get the first word
    *lines = NULL;
    *count = 0;
    ...
    while (*ptr) {
        ...
        /* message now points to a word of length (ptr-message);
           if that's longer than an entire line (most likely filenames),
           just take the last bit */
        while (widget_substringwidth(message, ptr - message) >= line_length) {
            message++;
        }

        // Check we've got room for the word, plus some prefixing space
        if ((position + widget_substringwidth(message, (ptr - message)) + 4) >= line_length) {
