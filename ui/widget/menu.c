...
static void print_items(void)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        colour = menu[i + 1].inactive ? WIDGET_COLOUR_DISABLED : WIDGET_COLOUR_FOREGROUND;

        if (i == highlight_line) {
            widget_rectangle(menu_left_edge_x, height, ((width * 8) - 2), (1 * 8), WIDGET_COLOUR_HIGHLIGHT);
        } else {
            widget_rectangle(menu_left_edge_x, height, ((width * 8) - 2), (1 * 8), WIDGET_COLOUR_BACKGROUND);
        }

        widget_printstring((menu_left_edge_x + 8), height, colour, buffer);

        if (menu[i + 1].submenu) {
            widget_draw_submenu_arrow(DISPLAY_BORDER_ASPECT_WIDTH + menu_left_edge_x + (width * 8) - 9,
                                      (i * 8) + 49,
                                      colour);
        }
...
void menu_options_general(int action)
{
    widget_do_general();
}


void menu_options_media(int action)
{
    widget_do_media();
}


void menu_options_peripherals_general(int action)
{
    widget_do_peripherals_general();
}


void menu_options_peripherals_disk(int action)
{
    widget_do_peripherals_disk();
}


void menu_options_sound(int action)
{
    widget_do_sound();
}


void menu_options_rzx(int action)
{
    widget_do_rzx();
}


void menu_options_movie(int action)
{
    widget_do_movie();
}


void menu_options_diskoptions(int action)
{
...
void menu_options_joysticks_select(int action)
{
    ...
    for (i = 0; (i < JOYSTICK_TYPE_COUNT) && (i < MAX_JOYSTICK_TYPES); i++) {
        ...
        snprintf((char *)joystick_names[i], 100, "\012%s\011 %s", shortcut, joystick_name[i]);
...
void menu_machine_reset(int action)
{
    ...
    if (widget_do_query(message) || !widget_query.confirm) {
        return;
    }
...
void menu_machine_select(int action)
{
    ...
    for (i = 0; i < machine_count; i++) {
        ...
        snprintf(options[i], 40, "%s", libspectrum_machine_name(machine_types[i]->machine));
    ...
    if (machine_current->machine != new_machine) {
...
static int set_active(struct widget_menu_entry *menu, const char *path, int active)
{
    if (*path == '/') {
        path++;
    }
    ...
    for (; menu->text; menu++) {
        ...
        if (*q == '/') {
            return set_active(menu->submenu, q, active);
        }
