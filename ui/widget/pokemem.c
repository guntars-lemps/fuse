...
int widget_pokemem_print_trainer(unsigned int left_edge,
                                 unsigned int width,
                                 int number,
                                 int disabled,
                                 int checked,
                                 const char *string);
void widget_pokemem_update_line(unsigned int left_edge,
                                unsigned int width,
                                int index);
...
int widget_pokemem_draw(void *data GCC_UNUSED)
{
    ...
    widget_printstring(((menu_left_edge_x * 8) + 2), 16, WIDGET_COLOUR_TITLE, pokemem_title);

    widget_pokemem_print_list(menu_left_edge_x, menu_width);

    widget_printstring(((menu_left_edge_x * 8) + 8), ((page_size + 4) * 8), WIDGET_COLOUR_FOREGROUND, "\x0A" "A\x01" "dd");
...
void widget_pokemem_print_list(unsigned int left_edge, unsigned int width)
{
    ...
    if (store && pokemem_count) {
        ...
        for (i = top_index; (i < pokemem_count) && (i < page_limit); i++) {
            ...
            widget_pokemem_print_trainer(left_edge, width, (i - top_index), trainer->disabled, entry->checked, buf);
        }
                                 ...
                                 const char *string)
{
    ...
    buffer[l] = '\0';
    w = widget_printstring(((left_edge * 8) + 9), ((number * 8) + 24), WIDGET_COLOUR_FOREGROUND, buffer) - 1;
...
void widget_pokemem_update_line(unsigned int left_edge, unsigned int width, int index)
{
    char buf[32];
    entry_t *entry;
    trainer_t *trainer;
    ...
    snprintf(buf, sizeof(buf), "%s", trainer->name);
    widget_pokemem_print_trainer(left_edge, width, (index - top_index), trainer->disabled, entry->checked, buf);
    ...
    // Last row in page
    if (((top_index + page_size) < pokemem_count) && ((index - top_index) == (page_size - 1))) {
        widget_down_arrow(left_edge, (page_size + 2), WIDGET_COLOUR_FOREGROUND);
    }
}
...
void widget_pokemem_keyhandler(input_key key)
{
    ...
    switch (key) {
        ...
        case INPUT_KEY_a: // Add poke
            if (!widget_pokemem_add_custom_poke()) {
    ...
    if (store && (new_selected != selected)) {
        if (new_selected < top_index) {
            ...
            // Otherwise, print the current trainer uninverted and the new current trainer inverted
            highlight_line = new_selected - top_index;
            if (selected >= 0) {
                widget_pokemem_update_line(menu_left_edge_x, menu_width, selected);
            }
