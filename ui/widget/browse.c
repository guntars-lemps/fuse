...
static void add_block_description(libspectrum_tape_block *block, void *user_data)
{
    ...
    buffer = malloc(MAX_BLOCK_DESC);
    if (!buffer) {
        return;
    }
    libspectrum_tape_block_description(buffer, MAX_BLOCK_DESC, block);
    if (strlen(data_detail)) {
        strcpy(buffer, data_detail);
    }
...
static void show_blocks(void)
{
    ...
    for (i = 0, ptr = g_slist_nth(blocks, top_line); (i < 18) && ptr; i++, ptr = ptr->next) {
