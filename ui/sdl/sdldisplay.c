...
/* This is a rule of thumb for the maximum number of rects that can be updated each frame.
   If more are generated we just update the whole screen */
...
static int sdldisplay_allocate_colours(int numColours, Uint32 *colour_values, Uint32 *bw_values);
...
static int sdl_convert_icon(SDL_Surface *source, SDL_Surface **icon, int red)
{
    ...
    (scaler_get_proc16(SCALER_DOUBLESIZE))((libspectrum_byte*)icon[0]->pixels,
                                           icon[0]->pitch,
...
int uidisplay_init(int width, int height)
{
    ...
    if (settings_current.sdl_fullscreen_mode && strcmp(settings_current.sdl_fullscreen_mode, "list") == 0) {
    ...
    // Check if there are any modes available, or if our resolution is restricted at all
    if (no_modes) {
...
static void sdldisplay_find_best_fullscreen_scaler(void)
{
    ...
    if (settings_current.full_screen) {
        ...
        while ((i < SCALER_NUM) &&
            ...
            while (!scaler_is_supported(i)) {
                i++;
            }
            scaler_select_scaler(i++);
            sdldisplay_current_size = scaler_get_scaling_factor(current_scaler);
            /* if we failed to find a suitable size scaler, just use normal
               (what the user had originally may be too big) */
...
static int sdldisplay_load_gfx_mode(void)
{
    ...
    sdldisplay_gc = SDL_SetVideoMode(((settings_current.full_screen && fullscreen_width) ?
                                         fullscreen_width : (image_width * sdldisplay_current_size)),
                                     ((settings_current.full_screen && fullscreen_width) ?
    ...
    settings_current.full_screen = !!(sdldisplay_gc->flags & (SDL_FULLSCREEN | SDL_NOFRAME));
    ...
    fullscreen_x_off = (sdldisplay_gc->w - (image_width * sdldisplay_current_size)) * (sdldisplay_is_full_screen  / 2);
    fullscreen_y_off = (sdldisplay_gc->h - (image_height * sdldisplay_current_size)) * (sdldisplay_is_full_screen / 2);
...
void uidisplay_frame_save(void)
{
    ...
    saved = SDL_ConvertSurface(tmp_screen, tmp_screen->format, SDL_SWSURFACE);
}
...
static void sdl_blit_icon(SDL_Surface **icon, SDL_Rect *r, Uint32 tmp_screen_pitch, Uint32 dstPitch)
{
    ...
    // Extend the dirty region by 1 pixel for scalers that "smear" the screen, e.g. 2xSAI
    ...
    scaler_proc16((libspectrum_byte*)tmp_screen->pixels + (x + 1) * tmp_screen->format->BytesPerPixel + (y + 1) * tmp_screen_pitch,
                  tmp_screen_pitch,
                  (libspectrum_byte*)sdldisplay_gc->pixels + (dst_x * sdldisplay_gc->format->BytesPerPixel) + (dst_y * dstPitch),
                  dstPitch,
                  w,
                  dst_h);
...
void uidisplay_putpixel(int x, int y, int colour)
{
    ...
    if (machine_current->timex) {
        x <<= 1;
        y <<= 1;
        dest_base = dest = (libspectrum_word*)((libspectrum_byte*)tmp_screen->pixels +
                                               (x + 1) * tmp_screen->format->BytesPerPixel +
                                               (y + 1) * tmp_screen->pitch);

        *(dest++) = palette_colour;
        *(dest++) = palette_colour;
        dest = (libspectrum_word*)((libspectrum_byte*)dest_base + tmp_screen->pitch);
        *(dest++) = palette_colour;
        *(dest++) = palette_colour;
    } else {
        dest = (libspectrum_word*)((libspectrum_byte*)tmp_screen->pixels +
                                   (x + 1) * tmp_screen->format->BytesPerPixel +
                                   (y + 1) * tmp_screen->pitch);
...
// Print the 8 pixels in `data' using ink colour `ink' and paper colour `paper' to the screen at ((8*x) , y)
void uidisplay_plot8(int x, int y, libspectrum_byte data, libspectrum_byte ink, libspectrum_byte paper)
{
    ...
    if (machine_current->timex) {
        ...
        dest_base = (libspectrum_word*)((libspectrum_byte*)tmp_screen->pixels +
                                        (x + 1) * tmp_screen->format->BytesPerPixel +
                                        (y + 1) * tmp_screen->pitch);
        ...
        for (i = 0; i < 2; i++) {
            ...
            *dest = (data & 0x01) ? palette_ink : palette_paper;
            ...
            dest_base = (libspectrum_word*)((libspectrum_byte*)dest_base + tmp_screen->pitch);
        }
    } else {
        x <<= 3;
...
// Print the 16 pixels in `data' using ink colour `ink' and paper colour `paper' to the screen at ((16*x) , y)
...
void uidisplay_frame_end(void)
{
    ...
    /* We check for a switch to fullscreen here to give systems with a
       windowed-only UI a chance to free menu etc.
       resources before  the switch to fullscreen (e.g. Mac OS X) */
    if ((sdldisplay_is_full_screen != settings_current.full_screen) && uidisplay_hotswap_gfx_mode()) {
    ...
    if (SDL_MUSTLOCK(sdldisplay_gc)) {
        SDL_LockSurface(sdldisplay_gc);
    }
    ...
    for (r = updated_rects; r != last_rect; r++) {
        ...
        scaler_proc16((libspectrum_byte*)tmp_screen->pixels +
                      (r->x + 1) * tmp_screen->format->BytesPerPixel + (r->y + 1) * tmp_screen_pitch,
                      tmp_screen_pitch,
                      (libspectrum_byte*)sdldisplay_gc->pixels +
                      (dst_x * sdldisplay_gc->format->BytesPerPixel) + (dst_y * dstPitch),
                      dstPitch,
                      r->w,
                      dst_h);
    ...
    if (SDL_MUSTLOCK(sdldisplay_gc)) {
        SDL_UnlockSurface(sdldisplay_gc);
    }
...
int uidisplay_end(void)
{
    ...
    for (i = 0; i < 2; i++) {
        if (red_cassette[i]) {
            SDL_FreeSurface(red_cassette[i]);
            red_cassette[i] = NULL;
        }
        if (green_cassette[i]) {
            SDL_FreeSurface(green_cassette[i]);
            green_cassette[i] = NULL;
        }
        if (red_mdr[i]) {
            SDL_FreeSurface(red_mdr[i]);
            red_mdr[i] = NULL;
        }
        if (green_mdr[i]) {
            SDL_FreeSurface(green_mdr[i]);
            green_mdr[i] = NULL;
        }
        if (red_disk[i]) {
            SDL_FreeSurface(red_disk[i]);
            red_disk[i] = NULL;
        }
        if (green_disk[i]) {
            SDL_FreeSurface(green_disk[i]);
            green_disk[i] = NULL;
        }
    }
    return 0;
