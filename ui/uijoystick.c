...
static int init_stick(int which, const char *const device, const char *const calibration)
{
    switch (JSInit(&jsd[which], device, calibration, JSFlagNonBlocking)) {

        case JSSuccess:
            if (JSLoadCalibrationUNIX(&jsd[which]) && (errno != ENOENT)) {
                ui_error(UI_ERROR_ERROR, "failed to read calibration for joystick %i: %s", (which + 1), strerror(errno));
                break;
            }

            if ((jsd[which].total_axises < 2) || (jsd[which].total_buttons < 1)) {
                ui_error(UI_ERROR_ERROR, "sorry, joystick %i (%s) is inadequate!", (which + 1), device);
                break;
            }
            return 0;

        case JSBadValue:
            ui_error(UI_ERROR_ERROR, "failed to initialise joystick %i: %s", (which + 1), "invalid parameter/value");
            break;
        ...
        case JSNoAccess:
            ...
            /*
            ui_error (UI_ERROR_ERROR, "failed to initialise joystick %i: %s", (which + 1), "cannot access device");
            */
            break;

        case JSNoBuffers:
            ui_error(UI_ERROR_ERROR, "failed to initialise joystick %i: %s", (which + 1), "not enough memory");
            break;
...
static int open_joystick(int which, const char *device, const char *calibration)
{
    char path[PATH_MAX];

    // If we were given an explicit device to use for this joystick, try only that
    if (device && device[0]) {
        return init_stick(which, device, calibration);
    }
...
int ui_joystick_init(void)
{
    ...
    cfgdir = compat_get_config_path();
    if (!cfgdir) {
        return 1;
    }

    // Default calibration file is ~/.joystick
    calibration = libspectrum_new(char, (strlen(cfgdir) + strlen(JSDefaultCalibration) + 2));
...
static void poll_joystick(int which)
{
    ...
    if (buttons > NUM_JOY_BUTTONS) {
        // We support 'only' NUM_JOY_BUTTONS (15 as defined in ui/uijoystick.h) fire buttons
