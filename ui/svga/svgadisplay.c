...
static const ptrdiff_t scaled_pitch = 3 * DISPLAY_SCREEN_WIDTH * sizeof(libspectrum_word);

// The line buffer for svga_putpixel ...
static libspectrum_byte line_buff[3 * 4 * DISPLAY_SCREEN_WIDTH];
...
void set_mode(int i, int n, vga_modeinfo *inf)
{
    if ((inf->colors >= 16) && !(inf->flags & IS_MODEX) &&
        (inf->width >= (256 * (i + 1))) && (inf->height >= (192 * (i + 1)))) {
        modes[i].n = n;
            ...
            if ((inf->colors >= 16) && !(inf->flags & IS_MODEX) &&
                ...
                for (j = 0; j < 3; j++) {
                    ...
                    if ((exact == 0) && (inf->width == w) && (inf->height == h) &&
                        ((modes[j].n == -1) ||
                        (((modes[j].width != w) || (modes[j].height != h) || (modes[j].depth != 16)) &&
                        ((inf->colors == 65536) || (inf->colors > modes[j].colors))))) {
                        set_mode(j, i, inf);
                    }
                    ...
                    if ((exact == 1) && (inf->width >= w) && (inf->height >= h) &&
                        ...
                        ((modes[j].n == -1) ||
                        (((modes[j].width > inf->width) || (modes[j].height > inf->height) || (modes[j].depth != 16)) &&
                        ((inf->colors == 65536) || (inf->colors > modes[j].colors))))) {
                        set_mode(j, i, inf);
                    }
...
int svgadisplay_init(void)
{
    ...
    if (settings_current.svga_modes && (strcmp(settings_current.svga_modes, "list") == 0)) {
        fprintf(stderr, "=====================================================================\n"
        ...
        for (j = 0; j <= vga_lastmodenumber(); j++) {
            if (vga_hasmode(j)) {
                ...
                if ((inf->colors >= 16) && !(inf->flags & IS_MODEX) && (inf->width >= 256) && (inf->height >= 192)) {
                    fprintf(stderr,
                            "% 4d  % 5d% 5d    %s   % 4d%%% 4d%%",
                            j,
                            inf->width,
    ...
    if (settings_current.svga_modes) {
        ...
        if ((n0 > 0) && vga_hasmode(n0)) {
            set_mode(0, n0, vga_getmodeinfo(n0));
        }
        if ((n1 > 0) && vga_hasmode(n1)) {
            set_mode(1, n1, vga_getmodeinfo(n1));
        }
        if ((n2 > 0) && vga_hasmode(n2)) {
            set_mode(2, n2, vga_getmodeinfo(n2));
        }
    }
...
static void svgadisplay_putpixel_15(int x, int y, libspectrum_word *colour)
{
    ...
    *(libspectrum_word *)line_buff_ptr = (c & 0x1f) + ((c >> 1) & 0xffe0);
    line_buff_ptr += 2;
}
...
static int svgadisplay_allocate_colours4(void)
{
    ...
    if (settings_current.bw_tv) {
        for (i = 0; i < 16; i++) { // grey
            red = green = blue = (((rgb_for_4[i * 3] * 4822) / 255) +
...
static int svgadisplay_allocate_colours8(void)
{
    ...
    for (r = 0; r < 4; r++) { // rgb232 => 128
        for (g = 0; g < 8; g++) {
            for (b = 0; b < 4; b++) {
                if (settings_current.bw_tv) {
                     red = green = blue = (((r * 4822) / 3) + ((g * 9467) / 7) + ((b * 1839) / 3)) >> 8;
                } else {
                    red = r * 63 / 3;
                    green = g * 63 / 7;
                    blue = b * 63 / 3;
...
static void register_scalers(void)
{
    ...
    if (scaler_is_supported(current_scaler) && ((svgadisplay_current_size * 4) == f)) {
...
int uidisplay_hotswap_gfx_mode(void)
{
    ...
    if ((svgadisplay_current_size * 4) != (image_scale * (machine_current->timex ? 2 : 1))) {
...
static void svgadisplay_update_rect_noscale(int x, int y, int w, int h)
{
    ...
    if (xclip || yclip) {
        ...
        if ((x + w) > (scaled_image_w - xclip)) {
            w = scaled_image_w - xclip - x;
        }
        if ((y + h) > (scaled_image_h - yclip)) {
            h = scaled_image_h - yclip - y;
        }
    }
...
void uidisplay_putpixel(int x, int y, int colour)
{
    libspectrum_word pc = ((svgadisplay_depth == 4) ? colour :
                           (settings_current.bw_tv ? pal_grey[colour] : pal_colour[colour]));
...
/* Print the 8 pixels in `data' using ink colour `ink' and paper colour `paper'
   to the screen at ((8*x) , y) */
void uidisplay_plot8(int x, int y, libspectrum_byte data, libspectrum_byte ink, libspectrum_byte paper)
{
    libspectrum_word *dest;
    libspectrum_word pi = ((svgadisplay_depth == 4) ? ink : (settings_current.bw_tv ? pal_grey[ink] : pal_colour[ink]));
    libspectrum_word pp = ((svgadisplay_depth == 4) ? paper : (settings_current.bw_tv ? pal_grey[paper] : pal_colour[paper]));
...
/* Print the 16 pixels in `data' using ink colour `ink' and paper colour `paper'
   to the screen at ((16*x) , y) */
...
{
    /* FIXME: Save current framebuffer state as the widget UI wants to scribble in here */
}


void uidisplay_frame_restore(void)
{
    /* FIXME: Restore saved framebuffer state as the widget UI wants to draw a new menu */
}
