...
static LRESULT WINAPI fuse_window_proc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch (msg) {
        ...
        case WM_ACTIVATE:
            if ((LOWORD(wParam) == WA_ACTIVE) || (LOWORD(wParam) == WA_CLICKACTIVE)) {
                ...
                // We'll call DefWindowProc to get keyboard focus when debugger window is open and inactive
            }
            break;

        case WM_USER_EXIT_PROCESS_MESSAGES:
            /* Odd case when message loop is overridden by a modal dialog.
               This should not be caught here, so we delay this notification */
            exit_process_messages++;
            return 0;

        case WM_ERASEBKGND:
            // Improves speed and avoid flickering when main window is invalidated by another window
            return TRUE;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    HACK: MinGW-w64 based toolchains neither feature _argc nor _argv.
    The 32 bit incarnation only defines __MINGW32__.
    This leads to build breakage due to missing declarations.
    Luckily MinGW-w64 based toolchains define __MINGW64_VERSION_foo macros inside _mingw.h,
    which is included from all system headers. Thus we abuse that to detect them.
    */
...
int ui_init(int *argc, char ***argv)
{
    ...
    if (settings_current.statusbar) {
        h_ofs += (statr.bottom - statr.top);
    }

    MoveWindow(fuse_hWnd,
               wr.left,
               wr.top,
...
int ui_error_specific(ui_error_level severity, const char *message)
{
    ...
    switch (severity) {
        ...
        case UI_ERROR_ERROR:
            ...
            break;
        default:
            MessageBox(fuse_hWnd, message, "Fuse - (Unknown Error Level)",  (MB_ICONINFORMATION | MB_OK));
            break;
    }
    return 0;
}
...
{
    // FIXME: choosing spectrum SE crashes Fuse sound_frame () at sound.c:477
...
static int set_active(HMENU menu, const char *path, int active)
{
    ...
    for (i = 0; i < menu_count; i++) {
        ...
        if (GetMenuString(menu, i, menu_text, 255, MF_BYPOSITION) == 0) {
            continue;
        }
        ...
        if (*q == '/') {
            return set_active(GetSubMenu(menu, i), q, active);
        }
...
ui_confirm_joystick_t ui_confirm_joystick(libspectrum_joystick libspectrum_type, int inputs)
{
    ...
    _sntprintf(title, ARRAY_SIZE(title), _T("Fuse - Configure %s Joystick"), libspectrum_joystick_name(libspectrum_type));
...
int win32ui_get_monospaced_font(HFONT *font)
{
    if (!monospaced_font) {
        ...
        *font = CreateFont(font_height, 0, 0, 0, 400, FALSE, FALSE, FALSE, 0, 400, 2, 1, 1, TEXT("Courier New"));
        if (*font == NULL) {
...
static void selector_dialog_build(HWND hwndDlg, win32ui_select_info *items)
{
    ...
    if (caption_width > window_width) {
        window_width = caption_width;
    }
    ...
    for (i  =0; i < items->length; i++) {
        ...
        CreateWindow(TEXT("BUTTON"),
                     ...
                     or.bottom,
                     hwndDlg,
                     (HMENU)(LONG_PTR)(IDC_SELECT_OFFSET + i),
                     fuse_hInstance,
                     0);
        SendDlgItemMessage(hwndDlg, (IDC_SELECT_OFFSET + i), WM_SETFONT, (WPARAM)h_ms_font, FALSE);

        // check the radiobutton corresponding to current label
        if (i == items->selected) {
            SendDlgItemMessage(hwndDlg, (IDC_SELECT_OFFSET + i), BM_SETCHECK, BST_CHECKED, 0);
        }
    ...
    CreateWindow(TEXT("BUTTON"),
                 ...
                 fuse_hInstance,
                 0);
    SendDlgItemMessage(hwndDlg, IDOK, WM_SETFONT, (WPARAM)h_ms_font, FALSE);
    ...
    CreateWindow(TEXT("BUTTON"),
                 ...
                 fuse_hInstance,
                 0);
    SendDlgItemMessage(hwndDlg, IDCANCEL, WM_SETFONT, (WPARAM)h_ms_font, FALSE);
    ...
    SetWindowPos(hwndDlg, NULL, 0, 0, window_width, window_height, (SWP_NOMOVE | SWP_NOZORDER | SWP_NOACTIVATE));
}


static INT_PTR CALLBACK selector_dialog_proc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    int i;
    HWND next_item = NULL;
        ...
        case WM_COMMAND:
            ...
            switch (LOWORD(wParam)) {
                case IDOK: {
                    ...
                    i = 0;
                    while ((next_item = GetNextDlgGroupItem(hwndDlg, next_item, FALSE)) != NULL) {
                        if (SendDlgItemMessage(hwndDlg, (IDC_SELECT_OFFSET + i), BM_GETCHECK, 0, 0) == BST_CHECKED) {
                            EndDialog(hwndDlg, i);
                            return TRUE;
                        }
                        i++;
...
{
    /*
    selector_dialog will display a modal dialog, with a list of grouped radiobuttons, OK and Cancel buttons.
    The radiobuttons' labels, their count, current selection and dialog title are provided via win32ui_select_info.
    The function returns an int corresponding to the selected radiobutton
    */

    // FIXME: fix accelerators for this window

    return DialogBoxParam(fuse_hInstance,
                          MAKEINTRESOURCE(IDD_SELECT_DIALOG),
                          fuse_hWnd,
                          selector_dialog_proc,
                          (LPARAM)items);
}
...
// Handler for the main window's WM_PAINT notification. The handler is an equivalent of GTK's expose_event
static int win32ui_window_paint(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
    blit();
    return 0;
...
void win32ui_fuse_resize(int width, int height)
{
    ...
    SystemParametersInfo(SPI_GETWORKAREA, 0, &or, 0);
    if ((wr.left + width + w_ofs) > or.right) {
        wr.left = or.right - width - w_ofs;
    }
    if ((wr.top + height + h_ofs) > or.bottom) {
        wr.top = or.bottom - height - h_ofs;
    }
    if (wr.left < or.left) {
        wr.left = or.left;
    }
    ...
    MoveWindow(fuse_hWnd, wr.left, wr.top, (width + w_ofs), (height + h_ofs), TRUE);
}


/*
win32ui_process_messages() is an equivalent of gtk's gtk_main(),
it processes messages until it receives WM_USER_EXIT_PROCESS_MESSAGES message, which should be sent using:
PostMessage(fuse_hWnd, WM_USER_EXIT_PROCESS_MESSAGES, 0, 0);
(equivalent of gtk_main_quit());
With process_queue_once = 1 it checks for messages pending for fuse window, processes them and exists.
With process_queue_once = 0 it processes the messages until it receives WM_USER_EXIT_PROCESS_MESSAGES message.
*/
    ...
    while (1) {
        while (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
            ...
            for (i = 0; processMsg && (i < ARRAY_SIZE(hModelessDlgs)); i++) {
                if (IsDialogMessage(hModelessDlgs[i], &msg)) {
                    processMsg = FALSE;
                }
            }
            ...
            if (processMsg) {
                if (!TranslateAccelerator(fuse_hWnd, hAccels, &msg)) {
                    if ((LOWORD(msg.message) == WM_QUIT) || (LOWORD(msg.message) == WM_USER_EXIT_PROCESS_MESSAGES)) {
                        return;
                    }
