...
INT_PTR CALLBACK win32ui_pokemem_proc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam);
LRESULT CALLBACK listview_proc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam);
...
void menu_machine_pokememory(int action GCC_UNUSED)
{
    ...
    DialogBox(fuse_hInstance, MAKEINTRESOURCE(IDD_POKEMEM), fuse_hWnd, (DLGPROC)win32ui_pokemem_proc);
                ...
                switch (nmhdr->code) {
                    ...
                    case LVN_ITEMCHANGED:
                        ...
                        if ((lpnmitem->uChanged & LVIF_STATE) && (lpnmitem->iItem >= 0)) {
                            ...
                            if ((new_state != old_state) && (new_state == 2) && !trainer->active && trainer->ask_value) {
                                ListView_SetItemState(nmhdr->hwndFrom, lpnmitem->iItem, LVIS_SELECTED, LVIS_SELECTED);
        ...
        case WM_LBUTTONDOWN:
            ...
            if ((result >= 0) && itemclicked.iSubItem) {
                ...
                if (!trainer->active && trainer->ask_value) {
                    ListView_SetItemState(hWnd, itemclicked.iItem, LVIS_SELECTED, LVIS_SELECTED);
        ...
        case WM_NOTIFY:
            ...
            switch (nmhdr->code) {
                ...
                case LVN_ENDLABELEDIT:
                    ...
                    lvi.pszText = (dispinfo->item.cchTextMax > 1) ? dispinfo->item.pszText : (LPTSTR)TEXT("0");
                    ...
                    // Update listview
                    SendMessage(hWnd, LVM_SETITEMTEXT, (WPARAM)lvi.iItem, (LPARAM)&lvi);
...
LRESULT CALLBACK edit_proc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch (msg) {
        ...
        case WM_KILLFOCUS:
            ...
            SendMessage(GetParent(hWnd), WM_NOTIFY, (WPARAM)IDC_PM_LIST, (LPARAM)&lvDispinfo);
...
void move_custom_edit(HWND hwnd_c_edit, HWND hwnd_parent)
{
    ...
    if (item_rect.top >= header_rect.bottom) {
        ...
        SetWindowPos(hwnd_c_edit, NULL, item_rect.left, item_rect.top, width, height, (SWP_NOZORDER | SWP_SHOWWINDOW));
    } else {
        // Hide custom edit when scrolls above header
        SetWindowPos(hwnd_c_edit, NULL, 0, 0, 0, 0, (SWP_NOMOVE | SWP_NOZORDER | SWP_NOSIZE | SWP_HIDEWINDOW));
    }
}
...
void pokemem_update_list(void)
{
    ...
    // Get count of items
    items = SendDlgItemMessage(fuse_hPMWnd, IDC_PM_LIST, LVM_GETITEMCOUNT, 0, 0);
...
void pokemem_add_custom_poke(void)
{
    ...
    // Parse bank
    length = SendDlgItemMessage(fuse_hPMWnd, IDC_PM_BANK_EDIT, WM_GETTEXT, (WPARAM)buffer_size, (LPARAM)buffer);
    ...
    // Parse address
    length = SendDlgItemMessage(fuse_hPMWnd, IDC_PM_ADDR_EDIT, WM_GETTEXT, (WPARAM)buffer_size, (LPARAM)buffer);

    // TODO: accept hex address
    a = length ? _ttol(buffer) : 0;

    if (!length || (a < 0) || (a > 65535)) {
        ui_error(UI_ERROR_ERROR, "Invalid address: use a number from 0 to 65535");
    ...
    // Parse value
    length = SendDlgItemMessage(fuse_hPMWnd, IDC_PM_VALUE_EDIT, WM_GETTEXT, (WPARAM)buffer_size, (LPARAM)buffer);
    ...
    if (!trainer->active && !trainer->disabled) {
        ...
        lvi.stateMask = LVIS_STATEIMAGEMASK;
        index = SendDlgItemMessage(fuse_hPMWnd, IDC_PM_LIST, LVM_GETITEMCOUNT, 0, 0) - 1;
        SendDlgItemMessage(fuse_hPMWnd, IDC_PM_LIST, LVM_SETITEMSTATE, (WPARAM)index, (LPARAM)&lvi);
    }
