...
{
    // No action needed; joysticks already handled by the Window messages sent by mmsystem
}
...
void win32joystick_buttonevent(int which_joystick, int button_down, unsigned int wParam)
{
    ...
    up to 8 milliseconds in analog joysticks (digital joysticks just a few clock cycles)
    */
    if (wParam & JOY_BUTTON1CHG) {
        button = INPUT_JOYSTICK_FIRE_1;
    } else if (wParam & JOY_BUTTON2CHG) {
        button = INPUT_JOYSTICK_FIRE_2;
    } else if (wParam & JOY_BUTTON3CHG) {
        button = INPUT_JOYSTICK_FIRE_3;
    }
    else if (wParam & JOY_BUTTON4CHG) {
        button = INPUT_JOYSTICK_FIRE_4;
    }
    else {
        return; // Fuse for Windows supports up to 4 joystick buttons
    }

    event.types.joystick.which = which_joystick;
    event.type = button_down ? INPUT_EVENT_JOYSTICK_PRESS : INPUT_EVENT_JOYSTICK_RELEASE;
...
void menu_options_joysticks_select(int action)
{
    ...
    DialogBoxParam(fuse_hInstance, MAKEINTRESOURCE(IDD_JOYSTICKS), fuse_hWnd, dialog_proc, (LPARAM)&info);

    fuse_emulation_unpause();
}


static INT_PTR CALLBACK dialog_proc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    switch (uMsg) {
...
static void dialog_init(HWND hwndDlg, struct joystick_info *info)
{
    ...
    for (i = 0; i < NUM_JOY_BUTTONS; i++) {
        ...
        info->button[i].label = GetDlgItem(hwndDlg, (IDC_JOYSTICKS_BUTTON_BUTTON1 + i));
        info->button[i].static_label = GetDlgItem(hwndDlg, (IDC_JOYSTICKS_STATIC_BUTTON1 + i));
        info->button[i].frame = GetDlgItem(hwndDlg, (IDC_JOYSTICKS_GROUP_BUTTON1 + i));
        if (info->button[i].setting) {
            create_fire_button_selector(info->button[i].name, &(info->button[i]), hwndDlg);
        } else {
            // disable the button configuration part of the dialog
            SendMessage(info->button[i].label, WM_SETTEXT, 0, (LPARAM)TEXT("N/A"));
            SendMessage(info->button[i].static_label, WM_SETTEXT, 0, (LPARAM)TEXT("N/A"));
            EnableWindow(info->button[i].label, FALSE);
            EnableWindow(info->button[i].static_label, FALSE);
            EnableWindow(GetDlgItem(hwndDlg, (IDC_JOYSTICKS_GROUP_BUTTON1 + i)), FALSE);
        }
    }
}
...
static void setup_info(struct joystick_info *info, int action)
{
    ...
    switch (action) {
        ...
        case 1:
            ...
            for (i = 0; i < NUM_JOY_BUTTONS; i++) {
                _sntprintf(info->button[i].name, 80, "Button %lu", ((unsigned long)i + 1));
            }
            break;
...
static void create_joystick_type_selector(struct joystick_info *info, HWND hwndDlg)
{
    ...
    for (i = 0; i < JOYSTICK_TYPE_COUNT; i++) {
        ...
        rect.top = (i * 10) + 10;
        rect.right = 5 + 45;
        rect.bottom = (i * 10) + 10 + 10;
        MapDialogRect(hwndDlg, &rect);
        info->radio[i] = CreateWindowEx(0,
                                        WC_BUTTON,
                                        joystick_name[i],
...
static void create_fire_button_selector(const TCHAR *title, struct button_info *info, HWND hwndDlg)
{
...
static void show_key_selection_popoup(HWND hwndDlg, LPARAM lParam)
{
    ...
    info = (struct button_info *)GetWindowLongPtr((HWND)lParam, GWLP_USERDATA);
    // create a popup right over the button that has been clicked
    GetWindowRect((HWND)lParam, &rect);
    hpopup = GetSubMenu(LoadMenu(fuse_hInstance, MAKEINTRESOURCE(IDR_JOYSTICKS_POPUP)), 0);
    // popup returns the key value
    menu_id = TrackPopupMenu(hpopup,
