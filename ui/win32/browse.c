...
void menu_media_tape_browse(int action)
{
    ...
    if (!dialog_created) {
        dialog = CreateDialog(fuse_hInstance, MAKEINTRESOURCE(IDD_BROWSE), fuse_hWnd, dialog_proc);
        if (dialog == NULL) {
...
static void dialog_init(HWND hwndDlg)
{
    ...
    DWORD lv_ext_style;
    lv_ext_style = SendDlgItemMessage(hwndDlg, IDC_BROWSE_LV, LVM_GETEXTENDEDLISTVIEWSTYLE, 0, 0);
    lv_ext_style |= LVS_EX_FULLROWSELECT;
    SendDlgItemMessage(hwndDlg, IDC_BROWSE_LV, LVM_SETEXTENDEDLISTVIEWSTYLE, 0, lv_ext_style);
    ...
    for (i = 0; i < 3; i++) {
        ...
        SendDlgItemMessage(hwndDlg, IDC_BROWSE_LV, LVM_INSERTCOLUMN, i, (LPARAM)&lvc);
    }
    ...
    icon_tape_marker = LoadImage(fuse_hInstance, "win32bmp_tape_marker", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);
    icon_tape_marker_mask = LoadImage(fuse_hInstance, "win32bmp_tape_marker_mask", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);
    GetObject(icon_tape_marker, sizeof(bmp), &bmp);

    // FIXME: destroy the list later
    himl = ImageList_Create(bmp.bmWidth, bmp.bmHeight, (ILC_COLOR | ILC_MASK), 1, 0);

    ImageList_Add(himl, icon_tape_marker, icon_tape_marker_mask);

    SendDlgItemMessage(hwndDlg, IDC_BROWSE_LV, LVM_SETIMAGELIST, LVSIL_SMALL, (LPARAM)himl);
...
int ui_tape_browser_update(ui_tape_browser_update_type change GCC_UNUSED, libspectrum_tape_block *block GCC_UNUSED)
{
    ...
    if (tape_modified) {
        SendDlgItemMessage(dialog, IDC_BROWSE_MODIFIED, WM_SETTEXT, 0, (LPARAM)TEXT("Tape modified"));
    } else {
        SendDlgItemMessage(dialog, IDC_BROWSE_MODIFIED, WM_SETTEXT, 0, (LPARAM)TEXT("Tape not modified"));
    }
...
static void add_block_details(libspectrum_tape_block *block, void *user_data)
{
    ...
    lvi.iImage = -1;
    lvi.iItem = SendDlgItemMessage(dialog, IDC_BROWSE_LV, LVM_GETITEMCOUNT, 0, 0);
    for (i = 0; i < 3; i++) {
        lvi.iSubItem = i;
        lvi.pszText = details[i];
        if (i == 0) {
            SendDlgItemMessage(dialog, IDC_BROWSE_LV, LVM_INSERTITEM, 0, (LPARAM)&lvi);
        } else {
            SendDlgItemMessage(dialog, IDC_BROWSE_LV, LVM_SETITEM, 0, (LPARAM)&lvi);
        }
    }
}
...
static void select_row(LPNMITEMACTIVATE lpnmitem)
{
    ...
    size_t i, count;
    count = SendDlgItemMessage(dialog, IDC_BROWSE_LV, LVM_GETITEMCOUNT, 0, 0);
    LVITEM li;
    li.mask = LVIF_IMAGE;
    li.iSubItem = 0;
