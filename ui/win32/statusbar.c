...
void win32statusbar_create(HWND hWnd)
{
    ...
    icon_tape_inactive = LoadImage(fuse_hInstance, "win32bmp_tape_inactive", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);
    icon_tape_active = LoadImage(fuse_hInstance, "win32bmp_tape_active", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);

    icon_mdr_inactive = LoadImage(fuse_hInstance, "win32bmp_mdr_inactive", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);
    icon_mdr_active = LoadImage(fuse_hInstance, "win32bmp_mdr_active", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);

    icon_disk_inactive = LoadImage(fuse_hInstance, "win32bmp_disk_inactive", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);
    icon_disk_active = LoadImage(fuse_hInstance, "win32bmp_disk_active", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);

    icon_pause_inactive = LoadImage(fuse_hInstance, "win32bmp_pause_inactive", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);
    icon_pause_active = LoadImage(fuse_hInstance, "win32bmp_pause_active", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);

    icon_mouse_inactive = LoadImage(fuse_hInstance, "win32bmp_mouse_inactive", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);
    icon_mouse_active = LoadImage(fuse_hInstance, "win32bmp_mouse_active", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);
    ...
    fuse_hStatusWindow = CreateWindowEx(0,
                                        STATUSCLASSNAME,
                                        NULL,
                                        dwStyle,
                                        0,
...
int ui_statusbar_update_speed(float speed)
{
    ...
    SendMessage(fuse_hStatusWindow, SB_SETTEXT, (WPARAM)2, (LPARAM)buffer);
...
void win32statusbar_redraw(HWND hWnd, LPARAM lParam)
{
    ...
    for (i = 0; i < 5; i++) {
            ...
            case UI_STATUSBAR_ITEM_DISK:
                switch (icons_status[UI_STATUSBAR_ITEM_DISK]) {
                    ...
                    case UI_STATUSBAR_STATE_ACTIVE:
                        src_bmp = icon_disk_active;
                        break;
                    default:
                        src_bmp = icon_disk_inactive;
                        break;
                }
                break;
            ...
            case UI_STATUSBAR_ITEM_MICRODRIVE:
                switch (icons_status[UI_STATUSBAR_ITEM_MICRODRIVE]) {
                    case UI_STATUSBAR_STATE_NOT_AVAILABLE:
                        ...
                        break;
                    case UI_STATUSBAR_STATE_ACTIVE:
                        src_bmp = icon_mdr_active;
                        break;
                    default:
                        src_bmp = icon_mdr_inactive;
                        break;
                }
                break;
        ...
        if (src_bmp != NULL) {
            ...
            BitBlt(dest_dc,
                   rc_item.left + new_icons_part_width,
                   rc_item.top + (icons_part_height - bmp.bmHeight) - (2 * icons_part_margin),
                   bmp.bmWidth,
                   bmp.bmHeight,
                   mask_dc,
                   0,
