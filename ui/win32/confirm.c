...
int win32ui_confirm(const char *string)
{
    ...
    result = MessageBox(fuse_hWnd, string, "Fuse - Confirm", (MB_OKCANCEL | MB_ICONQUESTION));
    if (result == IDOK) {
        confirm = 1;
    }
...
ui_confirm_save_t ui_confirm_save_specific(const char *message)
{
    ...
    result = MessageBox(fuse_hWnd, message, "Fuse - Confirm", (MB_YESNOCANCEL | MB_ICONQUESTION));
    switch (result) {
        case IDYES:
