...
static int option_enumerate_combo(const char * const *options, char *value, int count, int def)
{
    int i;
    if (value != NULL) {
        for (i = 0; i < count; i++) {
...
print << "CODE";
static void menu_options_$_->{name}_init(HWND hwndDlg)
{
    char buffer[80];
    int i;
...
CODE
    ...
    SendDlgItemMessage(hwndDlg,
                       IDC_${optname}_${idcname},
                       BM_SETCHECK,
                       settings_current.$widget->{value} ? BST_CHECKED : BST_UNCHECKED,
                       0);
...
CODE
	    ...
	    my $idcname = uc( "$widget->{value}" );
        print << "CODE";
    SendDlgItemMessage(hwndDlg, IDC_${optname}_${idcname}, EM_LIMITTEXT, $widget->{data1}, 0);
    /* FIXME This is asuming SendDlgItemMessage is not UNICODE */
    snprintf(buffer, 80, "%d", settings_current.$widget->{value});
    SendDlgItemMessage(hwndDlg, IDC_${optname}_${idcname}, WM_SETTEXT, 0, (LPARAM)buffer);
...
CODE
    ...
    for (i = 0; i < $_->{name}_$widget->{value}_combo_count; i++) {
        ...
        SendDlgItemMessage(hwndDlg,
                           ...
                           CB_ADDSTRING,
                           0,
                           (LPARAM)$_->{name}_$widget->{value}_combo[i]);
    }
    SendDlgItemMessage(hwndDlg,
                       IDC_${optname}_${idcname},
                       CB_SETCURSEL,
                       (LPARAM)$combo_default{$widget->{value}},
                       0);
    if (settings_current.$widget->{value} != NULL) {
        for (i = 0; i < $_->{name}_$widget->{value}_combo_count; i++) {
            if (!strcmp(settings_current.$widget->{value}, $_->{name}_$widget->{value}_combo[i])) {
                SendDlgItemMessage(hwndDlg, IDC_${optname}_${idcname}, CB_SETCURSEL, i, 0);
            }
        }
    }
CODE
...
static void menu_options_$_->{name}_done(HWND hwndDlg)
{
    char buffer[80];
...
CODE
	...
	if( $type eq "Checkbox" ) {
	    ...
	    print << "CODE";
    settings_current.$widget->{value} = IsDlgButtonChecked(hwndDlg, IDC_${optname}_${idcname});
...
CODE
    ...
    settings_current.$widget->{value} =
        utils_safe_strdup($_->{name}_$widget->{value}_combo[SendDlgItemMessage(hwndDlg,
                                                                               IDC_${optname}_${idcname},
                                                                               CB_GETCURSEL,
                                                                               0,
                                                                               0)]);
...
static BOOL CALLBACK menu_options_$_->{name}_proc(HWND hwndDlg, UINT msg, WPARAM wParam GCC_UNUSED, LPARAM lParam GCC_UNUSED)
{
    switch (msg) {
        case WM_INITDIALOG:
            /* FIXME: save the handle returned by LoadIcon() in win32ui.c */
            SendMessage(hwndDlg, WM_SETICON, ICON_SMALL, (LPARAM)LoadIcon( fuse_hInstance, "win32_icon"));
...
void menu_options_$_->{name}(int action GCC_UNUSED)
{
    fuse_emulation_pause();

    DialogBox(fuse_hInstance, MAKEINTRESOURCE(IDD_$optname), fuse_hWnd, (DLGPROC)menu_options_$_->{name}_proc);
