...
static INT_PTR CALLBACK memorybrowser_proc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam);

static LRESULT CALLBACK memory_listview_proc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam);
...
static void update_display(HWND hwndDlg, libspectrum_word base)
{
    ...
    for (i = 0; i < memorysb_page_rows; i++) {
        ...
        // append the item
        lvi.iItem = SendDlgItemMessage(hwndDlg, IDC_MEM_LV, LVM_GETITEMCOUNT, 0, 0);
        lvi.iSubItem = 0;
        lvi.pszText = text[0];
        SendDlgItemMessage(hwndDlg, IDC_MEM_LV, LVM_INSERTITEM, 0, (LPARAM)&lvi);
        lvi.iSubItem = 1;
        lvi.pszText = text[1];
        SendDlgItemMessage(hwndDlg, IDC_MEM_LV, LVM_SETITEM, 0, (LPARAM)&lvi);
        lvi.iSubItem = 2;
        lvi.pszText = text[2];
        SendDlgItemMessage(hwndDlg, IDC_MEM_LV, LVM_SETITEM, 0, (LPARAM)&lvi);
    }
...
static int scroller(HWND hwndDlg, WPARAM scroll_command)
{
    ...
    ListView_SetItemState(GetDlgItem(hwndDlg, IDC_MEM_LV), selected, LVIS_SELECTED, LVIS_SELECTED);
...
void menu_machine_memorybrowser(int action GCC_UNUSED)
{
    ...
    DialogBox(fuse_hInstance, MAKEINTRESOURCE(IDD_MEM), fuse_hWnd, (DLGPROC)memorybrowser_proc);
...
static INT_PTR CALLBACK memorybrowser_proc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam GCC_UNUSED)
{
    switch (uMsg) {
        case WM_INITDIALOG:
    ...
    switch (msg) {
        ...
        case WM_KEYDOWN:
            ...
            if (scroll_notify != 0xffff) {
                SendMessage(GetParent(hWnd), WM_VSCROLL, MAKEWPARAM(scroll_notify, 0), (LPARAM)NULL);
                return 0;
            }
            break;
...
static void memorybrowser_init(HWND hwndDlg)
{
    ...
    SetWindowLongPtr(hwnd_list, GWLP_WNDPROC, (LONG_PTR)(WNDPROC)memory_listview_proc);

    // set extended listview style to select full row, when an item is selected
    DWORD lv_ext_style;
    lv_ext_style = SendDlgItemMessage(hwndDlg, IDC_MEM_LV, LVM_GETEXTENDEDLISTVIEWSTYLE, 0, 0);
    lv_ext_style |= LVS_EX_FULLROWSELECT;
    lv_ext_style |= LVS_EX_DOUBLEBUFFER;
    SendDlgItemMessage(hwndDlg, IDC_MEM_LV, LVM_SETEXTENDEDLISTVIEWSTYLE, 0, lv_ext_style);
    ...
    for (i = 0; i < 3; i++) {
        ...
        SendDlgItemMessage(hwndDlg, IDC_MEM_LV, LVM_INSERTCOLUMN, i, (LPARAM)&lvc);
    }

    // set font of the listview to monospaced one
    SendDlgItemMessage(hwndDlg, IDC_MEM_LV , WM_SETFONT, (WPARAM)font, FALSE);

    // Recalculate visible rows, Visual Styles could change rows height
    memorysb_page_rows = SendDlgItemMessage(hwndDlg, IDC_MEM_LV, LVM_GETCOUNTPERPAGE, 0, 0);
