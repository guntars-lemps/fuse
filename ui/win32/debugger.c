...
static INT_PTR CALLBACK win32ui_debugger_proc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam);

static LRESULT CALLBACK disassembly_listview_proc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam);
...
static const TCHAR *format_8_bit(void)
{
    return (debugger_output_base == 10) ? TEXT("%3d") : TEXT("0x%02X");
}


static const TCHAR *format_16_bit(void)
{
...
static int hide_hidden_panes(void)
{
    ...
    for (i = DEBUGGER_PANE_BEGIN; i < DEBUGGER_PANE_END; i++) {
        ...
        checkitem = get_pane_menu_item(i);
        if (!checkitem) {
            return 1;
        }
...
static UINT get_pane_menu_item(debugger_pane pane)
{
    ...
    switch (pane) {
        case DEBUGGER_PANE_REGISTERS:
            menu_item_id = IDM_DBG_REG;
            break;
        case DEBUGGER_PANE_MEMORYMAP:
            menu_item_id = IDM_DBG_MEMMAP;
            break;
        case DEBUGGER_PANE_BREAKPOINTS:
            menu_item_id = IDM_DBG_BPS;
            break;
        case DEBUGGER_PANE_DISASSEMBLY:
            menu_item_id = IDM_DBG_DIS;
            break;
        case DEBUGGER_PANE_STACK:
            menu_item_id = IDM_DBG_STACK;
            break;
        case DEBUGGER_PANE_EVENTS:
            menu_item_id = IDM_DBG_EVENTS;
            break;

        case DEBUGGER_PANE_END:
            break;
    }
...
{
    /*
    Instead of get_pane() and then hiding or showing the widget as it is done in gtk ui,
    we need to show/hide multiple widgets, hence this combined function.

    show parameter needs to be SW_SHOW to reveal the pane, or SW_HIDE to hide it.
    */
...
int ui_debugger_deactivate(int interruptable)
{
    ...
    if (dialog_created) {
        EnableWindow(GetDlgItem(fuse_hDBGWnd, IDC_DBG_BTN_CONT), (!interruptable ? TRUE : FALSE));
        EnableWindow(GetDlgItem(fuse_hDBGWnd, IDC_DBG_BTN_BREAK), (interruptable ? TRUE : FALSE));
    }
    return 0;
}
...
static int create_dialog(void)
{
    ...
    error = win32ui_get_monospaced_font(&font);
    if (error) {
        return error;
    }

    fuse_hDBGWnd = CreateDialog(fuse_hInstance, MAKEINTRESOURCE(IDD_DBG), fuse_hWnd, win32ui_debugger_proc);
    ...
    error = create_breakpoints();
    if (error)  {
        return error;
    }
    error = create_disassembly(font);
    if (error) {
        return error;
    }
    error = create_stack_display(font);
    if (error) {
        return error;
    }
    error = create_events();
    ...
    for (i = DEBUGGER_PANE_BEGIN; i < DEBUGGER_PANE_END; i++) {
        ...
        check_item = get_pane_menu_item(i);
        if (!check_item) {
            break;
        }
...
static void toggle_display(debugger_pane pane, UINT menu_item_id)
{
    ...
    // Windows doesn't automatically checks/unchecks the menus when they're clicked
    if (mii.fState && MFS_CHECKED) {
...
static int create_breakpoints(void)
{
    ...
    LPCTSTR breakpoint_titles[] = {_T("ID"), _T("Type"), _T("Value"), _T("Ignore"), _T("Life"), _T("Condition")};
    // set extended listview style to select full row, when an item is selected
    DWORD lv_ext_style;
    lv_ext_style = SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_BPS, LVM_GETEXTENDEDLISTVIEWSTYLE, 0, 0);
    lv_ext_style |= LVS_EX_FULLROWSELECT;
    SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_BPS, LVM_SETEXTENDEDLISTVIEWSTYLE, 0, lv_ext_style);
    ...
    for (i = 0; i < 6; i++) {
        ...
        SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_BPS, LVM_INSERTCOLUMN, i, (LPARAM)&lvc);
    }
    return 0;
}
...
static int create_disassembly(HFONT font)
{
    ...
    SetWindowLongPtr(hwnd_list, GWLP_WNDPROC, (LONG_PTR)(WNDPROC)disassembly_listview_proc);

    // set extended listview style to select full row, when an item is selected
    DWORD lv_ext_style;
    lv_ext_style = SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_PC, LVM_GETEXTENDEDLISTVIEWSTYLE, 0, 0);
    lv_ext_style |= LVS_EX_FULLROWSELECT;
    lv_ext_style |= LVS_EX_DOUBLEBUFFER;
    SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_PC, LVM_SETEXTENDEDLISTVIEWSTYLE, 0, lv_ext_style);
    ...
    for (i = 0; i < 2; i++) {
        ...
        SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_PC, LVM_INSERTCOLUMN, i, (LPARAM)&lvc);
    }
    ...
    disassembly_page = SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_PC, LVM_GETCOUNTPERPAGE, 0, 0);
    ...
    SetScrollInfo(GetDlgItem(fuse_hDBGWnd, IDC_DBG_SB_PC), SB_CTL, &si, TRUE);
...
static int create_stack_display(HFONT font)
{
    ...
    // set extended listview style to select full row, when an item is selected
    DWORD lv_ext_style;
    lv_ext_style = SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_STACK, LVM_GETEXTENDEDLISTVIEWSTYLE, 0, 0);
    lv_ext_style |= LVS_EX_FULLROWSELECT;
    SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_STACK, LVM_SETEXTENDEDLISTVIEWSTYLE, 0, lv_ext_style);
    ...
    for (i = 0; i < 2; i++) {
        ...
        SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_STACK, LVM_INSERTCOLUMN, i, (LPARAM)&lvc);
    }
...
static void stack_click(LPNMITEMACTIVATE lpnmitem)
{
    ...
    li.pszText = buffer;
    li.cchTextMax = 255;
    retval = SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_STACK, LVM_GETITEMTEXT, row, (LPARAM)&li);
    if (!retval) {
...
static int create_events(void)
{
    ...
    // set extended listview style to select full row, when an item is selected
    DWORD lv_ext_style;
    lv_ext_style = SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_EVENTS, LVM_GETEXTENDEDLISTVIEWSTYLE, 0, 0);
    lv_ext_style |= LVS_EX_FULLROWSELECT;
    SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_EVENTS, LVM_SETEXTENDEDLISTVIEWSTYLE, 0, lv_ext_style);
    ...
    for (i = 0; i < 2; i++) {
        ...
        SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_EVENTS, LVM_INSERTCOLUMN, i, (LPARAM)&lvc);
    }
...
static void events_click(LPNMITEMACTIVATE lpnmitem)
{
    ...
    li.pszText = buffer;
    li.cchTextMax = 255;
    got_text = SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_EVENTS, LVM_GETITEMTEXT, row, (LPARAM)&li);
    if (!got_text) {
        ui_error(UI_ERROR_ERROR, "couldn't get text for row %d", row);
        return;
    }
...
int ui_debugger_update(void)
{
    ...
    const char *register_name[] = {TEXT("PC"), TEXT("SP"), TEXT("AF"), TEXT("AF'"),
                                   TEXT("BC"), TEXT("BC'"), TEXT("DE"), TEXT("DE'"),
                                   TEXT("HL"), TEXT("HL'"), TEXT("IX"), TEXT("IY")};
    ...
    for (i = 0; i < 12; i++) {
        ...
        SendDlgItemMessage(fuse_hDBGWnd, (IDC_DBG_REG_PC + i), WM_SETTEXT, (WPARAM)0, (LPARAM)buffer);
    }

    _tcscpy(buffer, TEXT("  I   "));
    _sntprintf(&buffer[6], 76, format_8_bit(), I);
    SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_REG_I, WM_SETTEXT, (WPARAM)0, (LPARAM)buffer);

    _tcscpy(buffer, TEXT("  R   "));
    _sntprintf(&buffer[6], 80, format_8_bit(), (R & 0x7f) | (R7 & 0x80));
    SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_REG_R, WM_SETTEXT, (WPARAM)0, (LPARAM)buffer);

    _sntprintf(buffer, 80, TEXT("Halted %d"), z80.halted);
    SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_REG_HALTED, WM_SETTEXT, (WPARAM)0, (LPARAM)buffer);
    _sntprintf(buffer, 80, TEXT("T-states %5d"), tstates);
    SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_REG_T_STATES, WM_SETTEXT, (WPARAM)0, (LPARAM)buffer);
    _sntprintf(buffer, 80, TEXT("  IM %d\r\nIFF1 %d\r\nIFF2 %d"), IM, IFF1, IFF2);
    SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_REG_IM, WM_SETTEXT, (WPARAM)0, (LPARAM)buffer);

    _tcscpy(buffer, TEXT("SZ5H3PNC\r\n"));
    for (i = 0; i < 8; i++) {
        buffer[i + 10] = (F & (0x80 >> i)) ? '1' : '0';
    }
    buffer[18] = '\0';
    SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_REG_FLAGS, WM_SETTEXT, (WPARAM)0, (LPARAM)buffer);
    ...
    if (capabilities & LIBSPECTRUM_MACHINE_CAPABILITY_AY) {
        ...
        _sntprintf(&buffer[length], (1024 - length), format_string, machine_current->ay.current_register);
    }

    if (capabilities & LIBSPECTRUM_MACHINE_CAPABILITY_128_MEMORY) {
        _stprintf(format_string, "\r\n128Mem %s", format_8_bit());
        length = _tcslen(buffer);
        _sntprintf(&buffer[length], (1024 - length), format_string, machine_current->ram.last_byte);
    }

    if (capabilities & LIBSPECTRUM_MACHINE_CAPABILITY_PLUS3_MEMORY) {
        _stprintf(format_string, "\r\n+3 Mem %s", format_8_bit());
        length = _tcslen(buffer);
        _sntprintf(&buffer[length], (1024 - length), format_string, machine_current->ram.last_byte2);
    }
    ...
    if ((capabilities & LIBSPECTRUM_MACHINE_CAPABILITY_TIMEX_VIDEO) ||
        ...
        length = _tcslen(buffer);
        _sntprintf(&buffer[length], (1024 - length), format_string, scld_last_dec.byte);
    }
    ...
    if (settings_current.zxcf_active) {
        _stprintf(format_string, "\r\n  ZXCF %s", format_8_bit());
        length = _tcslen(buffer);
        _sntprintf(&buffer[length], (1024 - length), format_string, zxcf_last_memctl());
    }

    SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_REG_ULA, WM_SETTEXT, (WPARAM)0, (LPARAM)buffer);
    ...
    SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_STACK, LVM_DELETEALLITEMS, 0, 0);
    ...
    for (i = 0, address = (SP + 38); i < 20; i++, address -= 2) {
        ...
        libspectrum_word contents = readbyte_internal(address) + (0x100 * readbyte_internal(address + 1));
        ...
        SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_STACK, LVM_INSERTITEM, 0, (LPARAM)&lvi);
        lvi.iSubItem = 1;
        lvi.pszText = disassembly_text[1];
        SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_STACK, LVM_SETITEM, 0, (LPARAM)&lvi);
    }
...
static void update_memory_map(void)
{
    ...
    for (block = 0; (block < MEMORY_PAGES_IN_64K) && (row < 8); block++) {
        ...
        if ((page->source != source) || (page->page_num != page_num) || (page->offset != offset) ||
            (page->writable != writable) || (page->contended != contended)) {

            _sntprintf(buffer, 40, format_16_bit(), ((unsigned)block * MEMORY_PAGE_SIZE));
            SendDlgItemMessage(fuse_hDBGWnd, (IDC_DBG_MAP11 + (row * 4)), WM_SETTEXT, (WPARAM)0, (LPARAM)buffer);

            // FIXME: memory_source_description is not unicode
            _snprintf(buffer, 40, TEXT("%s %d"), memory_source_description(page->source), page->page_num);

            SendDlgItemMessage(fuse_hDBGWnd, (IDC_DBG_MAP11 + (row * 4) + 1), WM_SETTEXT, (WPARAM)0, (LPARAM)buffer);

            SendDlgItemMessage(fuse_hDBGWnd,
                               IDC_DBG_MAP11 + (row * 4) + 2,
                               WM_SETTEXT,
                               (WPARAM)0,
                               (LPARAM)(page->writable ? TEXT("Y") : TEXT("N")));

            SendDlgItemMessage(fuse_hDBGWnd,
                               IDC_DBG_MAP11 + (row * 4) + 3,
                               WM_SETTEXT,
                               (WPARAM)0,
                               (LPARAM)(page->contended ? TEXT("Y") : TEXT("N")));
            row++;
...
static void update_breakpoints(void)
{
    ...
    TCHAR buffer[1024],
    *breakpoint_text[6] = {&buffer[0], &buffer[40], &buffer[80], &buffer[120], &buffer[160], &buffer[200]};
    GSList *ptr;
    ...
    SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_BPS, LVM_DELETEALLITEMS, 0, 0);
    ...
    for (ptr = debugger_breakpoints; ptr; ptr = ptr->next) {
        ...
        _sntprintf(breakpoint_text[1], 40, "%s", debugger_breakpoint_type_text[bp->type]);
        ...
        switch (bp->type) {
            ...
            case DEBUGGER_BREAKPOINT_TYPE_WRITE:
                if (bp->value.address.source == memory_source_any) {
                    _sntprintf(breakpoint_text[2], 40, format_16_bit(), bp->value.address.offset);
                } else {
                    snprintf(format_string, 1024, "%%s:%s:%s", format_16_bit(), format_16_bit());
                    snprintf(breakpoint_text[2],
                             40,
                             format_string,
            ...
            case DEBUGGER_BREAKPOINT_TYPE_PORT_WRITE:
                ...
                _sntprintf(breakpoint_text[2], 40, format_string, bp->value.port.mask, bp->value.port.port);
                break;
            ...
            case DEBUGGER_BREAKPOINT_TYPE_EVENT:
                _sntprintf(breakpoint_text[2], 40, "%s:%s", bp->value.event.type, bp->value.event.detail);

        }

        _sntprintf(breakpoint_text[3], 40, "%lu", (unsigned long)bp->ignore);
        _sntprintf(breakpoint_text[4], 40, "%s", debugger_breakpoint_life_text[bp->life]);
        if (bp->condition) {
        ...
        lvi.iItem = SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_BPS, LVM_GETITEMCOUNT, 0, 0);
        ...
        for (i = 0; i < 6; i++) {
            ...
            if (i == 0) {
                SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_BPS, LVM_INSERTITEM, 0, (LPARAM)&lvi);
            } else {
                SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_BPS, LVM_SETITEM, 0, (LPARAM)&lvi);
            }
        }
    }
}
...
static void update_disassembly(void)
{
    ...
    SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_PC, LVM_DELETEALLITEMS, 0, 0);
    ...
    for (i = 0, address = disassembly_top; i < disassembly_page; i++) {
        ...
        lvi.iSubItem = 0;
        lvi.pszText = disassembly_text[0];
        SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_PC, LVM_INSERTITEM, 0, (LPARAM)&lvi);
        lvi.iSubItem = 1;
        lvi.pszText = disassembly_text[1];
        SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_PC, LVM_SETITEM, 0, (LPARAM)&lvi);
    }
...
{
    // clear the listview
    SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_EVENTS, LVM_DELETEALLITEMS, 0, 0);
...
static void add_event(gpointer data, gpointer user_data GCC_UNUSED)
{
    ...
    // append the item
    lvi.iItem = SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_EVENTS, LVM_GETITEMCOUNT, 0, 0);
    lvi.iSubItem = 0;
    lvi.pszText = event_text[0];
    SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_EVENTS, LVM_INSERTITEM, 0, (LPARAM)&lvi);
    lvi.iSubItem = 1;
    lvi.pszText = event_text[1];
    SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_LV_EVENTS, LVM_SETITEM, 0, (LPARAM)&lvi);
}
...
{
    /*
    Note: the scroll bar can not cope with "upper bound - page_size" value and higher.
    PC register, key and wheel scrolling are fine with that */
    SCROLLINFO si;
    si.cbSize = sizeof(si);
    si.fMask = SIF_POS;
...
static int move_disassembly(WPARAM scroll_command)
{
    ...
    switch (LOWORD(scroll_command)) {
        case SB_BOTTOM:
            ...
            address = debugger_search_instruction(disassembly_min, -disassembly_page);
            break;
        case SB_TOP:
        ...
        case SB_LINEUP:
            ...
            break;
        case SB_PAGEUP:
            address = debugger_search_instruction(disassembly_top, -disassembly_page);
            break;
        case SB_PAGEDOWN:
           address = debugger_search_instruction(disassembly_top, disassembly_page);
           break;
    ...
    // Get selected row
    cursor_row = ListView_GetNextItem(GetDlgItem(fuse_hDBGWnd, IDC_DBG_LV_PC), -1, LVNI_SELECTED);
...
static void evaluate_command(void)
{
    ...
    buffer_size = SendDlgItemMessage(fuse_hDBGWnd, IDC_DBG_ED_EVAL, WM_GETTEXTLENGTH, (WPARAM)0, (LPARAM)0);
    ...
    if (SendDlgItemMessage(fuse_hDBGWnd,
                           IDC_DBG_ED_EVAL,
                           WM_GETTEXT,
                           (WPARAM)(buffer_size + 1),
                           (LPARAM)buffer) != buffer_size) {
        ui_error(UI_ERROR_ERROR, "Couldn't get the content of the Evaluate text box");
        return;
    }
...
static INT_PTR CALLBACK win32ui_debugger_proc(HWND hWnd GCC_UNUSED, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch (msg) {
        case WM_COMMAND:
...
static LRESULT CALLBACK disassembly_key_press(HWND hWnd, WPARAM wParam)
{
    ...
    if (initial_top != disassembly_top) {
        // Mark selected row
        if (cursor_row >= 0) {
            ListView_SetItemState(hWnd, cursor_row, (LVIS_FOCUSED | LVIS_SELECTED), (LVIS_FOCUSED | LVIS_SELECTED));
        }
        return TRUE;
    }
    return FALSE;
...
static LRESULT CALLBACK disassembly_wheel_scroll(HWND hWnd, WPARAM wParam)
{
    libspectrum_word address;
    int cursor_row;
    short delta;
