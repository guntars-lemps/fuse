...
static INT_PTR CALLBACK win32ui_pokefinder_proc(HWND hWnd GCC_UNUSED, UINT msg, WPARAM wParam, LPARAM lParam)
{
    int height;
...
void move_button(int button, int dlg_height)
{
    ...
    SetWindowPos(ctrl_hWnd, 0, x, y, 0, 0, (SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE));
}
...
static void update_pokefinder(void)
{
        ...
        for (page = 0; page < (MEMORY_PAGES_IN_16K * SPECTRUM_RAM_PAGES); page++) {
            ...
            for (offset = 0; offset < MEMORY_PAGE_SIZE; offset++) {
                if (!((pokefinder_impossible[page][offset / 8] & 1) << (offset & 7))) {
                    ...
                    SendDlgItemMessage(fuse_hPFWnd, IDC_PF_LIST, LVM_SETITEMCOUNT, i, 0);

                    // add the item
                    lvi.iItem = i;
                    lvi.iSubItem = 0;
                    lvi.pszText = possible_text[0];
                    SendDlgItemMessage(fuse_hPFWnd, IDC_PF_LIST, LVM_INSERTITEM, 0, (LPARAM)&lvi);
                    lvi.iSubItem = 1;
                    lvi.pszText = possible_text[1];
                    SendDlgItemMessage(fuse_hPFWnd, IDC_PF_LIST, LVM_SETITEM, 0, (LPARAM)&lvi);
    ...
    SetWindowPos(fuse_hPFWnd, NULL, 0, 0, rcx, rcy, (SWP_NOMOVE | SWP_NOZORDER | SWP_NOACTIVATE));

    // print possible locations
    _sntprintf(buffer, 256, "Possible locations: %lu", (unsigned long)pokefinder_count);
...
void menu_machine_pokefinder(int action GCC_UNUSED)
{
    ...
    if (fuse_hPFWnd == NULL) {
        ...
        DWORD lv_ext_style;
        lv_ext_style = SendDlgItemMessage(fuse_hPFWnd, IDC_PF_LIST, LVM_GETEXTENDEDLISTVIEWSTYLE, 0, 0);
        lv_ext_style |= LVS_EX_FULLROWSELECT;
        SendDlgItemMessage(fuse_hPFWnd, IDC_PF_LIST, LVM_SETEXTENDEDLISTVIEWSTYLE, 0, lv_ext_style);
        ...
        SendDlgItemMessage(fuse_hPFWnd, IDC_PF_LIST, LVM_INSERTCOLUMN, 0, (LPARAM)&lvc);
        lvc.mask |= LVCF_SUBITEM;
        lvc.cx = cx;
        lvc.pszText = (LPTSTR)TEXT("Offset");
        SendDlgItemMessage(fuse_hPFWnd, IDC_PF_LIST, LVM_INSERTCOLUMN, 1, (LPARAM)&lvc);
...
static void win32ui_pokefinder_search(void)
{
    ...
    buffer_size = SendDlgItemMessage(fuse_hPFWnd, IDC_PF_EDIT, WM_GETTEXTLENGTH, (WPARAM)0, (LPARAM)0);
    ...
    if (SendDlgItemMessage(fuse_hPFWnd, IDC_PF_EDIT, WM_GETTEXT, (WPARAM)(buffer_size + 1), (LPARAM)buffer) != buffer_size) {
