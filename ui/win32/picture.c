...
{
    if (!hDialogPicture) {
        hDialogPicture = CreateDialog(fuse_hInstance, MAKEINTRESOURCE(IDD_PICTURE), fuse_hWnd, (DLGPROC)picture_wnd_proc);
       ...
       BITMAPINFO picture_BMI;
        ...
        picture_BMP = CreateDIBSection(dc, &picture_BMI, DIB_RGB_COLORS, &picture, NULL, 0);
...
static LRESULT WINAPI picture_wnd_proc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch (msg) {
        case WM_PAINT:
            ...
            BitBlt(dest_dc, 0, 0, DISPLAY_ASPECT_WIDTH, DISPLAY_SCREEN_HEIGHT, pic_dc, 0, 0, SRCCOPY);
...
static void draw_screen(libspectrum_byte *screen, int border)
{
    ...
    for (y = 0; y < DISPLAY_BORDER_HEIGHT; y++) {
        for (x = 0; x < DISPLAY_ASPECT_WIDTH; x++) {
            *(libspectrum_dword*)(picture + (y * pitch) + (4 * x)) = win32display_colours[border];
            *(libspectrum_dword*)(picture + ((y + DISPLAY_BORDER_HEIGHT + DISPLAY_HEIGHT) * pitch) + (4 * x)) =
    ...
    for (y = 0; y < DISPLAY_HEIGHT; y++) {
        for (x = 0; x < DISPLAY_BORDER_ASPECT_WIDTH; x++) {
            *(libspectrum_dword*)(picture + ((y + DISPLAY_BORDER_HEIGHT) * pitch) + (4 * x)) = win32display_colours[border];
        ...
        for (x = 0; x < DISPLAY_WIDTH_COLS; x++) {
            ...
            for (i = 0; i < 8; i++) {
                libspectrum_dword pix = win32display_colours[ (data & 0x80) ? ink : paper];

                // rearrange pixel components
                pix = (pix & 0x0000ff00) | ((pix & 0x000000ff) << 16) | ((pix & 0x00ff0000) >> 16);

                *(libspectrum_dword*)(picture + ((y + DISPLAY_BORDER_HEIGHT) * pitch) +
                                      (4 * ((8 * x) + DISPLAY_BORDER_ASPECT_WIDTH + i))) = pix;
                data <<= 1;
            }
        }
    }
