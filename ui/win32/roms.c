...
static INT_PTR CALLBACK roms_proc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam);
static void roms_init(HWND hwndDlg, LPARAM lParam);

/* FIXME:
use GWL_USERDATA whenever more suitable, and use MapDialogRect whenever drawing the interface at runtime
...
int menu_select_roms_with_title(const char *title, size_t start, size_t n, int is_peripheral)
{
    ...
    DialogBoxParam(fuse_hInstance, MAKEINTRESOURCE(IDD_ROMS), fuse_hWnd, (DLGPROC)roms_proc, (LPARAM)&info);
...
static void roms_init(HWND hwndDlg, LPARAM lParam)
{
    ...
    rect.left = 25;
    rect.top = (info->n * 30) + 5;
    rect.right = 25 + 50;
    rect.bottom = (info->n * 30) + 5 + 14;
    ...
    */
    ...
    rect.right += (window_rect.right - window_rect.left) - (client_rect.right - client_rect.left);
    rect.bottom += (window_rect.bottom - window_rect.top) - (client_rect.bottom - client_rect.top);
...
static void add_rom(HWND hwndDlg, size_t start, size_t row, int is_peripheral)
{
    ...
    // create an edit
    setting = settings_get_rom_setting(&settings_current, (start + row), is_peripheral);

    rect.left = 5;
    rect.top = (row * 30) + 10;
    rect.right = 5 + 110;
    rect.bottom = (row * 30) + 10 + 14;
    MapDialogRect(hwndDlg, &rect);
...
static void roms_done(HWND hwndDlg, LONG_PTR lParam)
{
    ...
    for (i = 0; i < info->n; i++) {
        ...
        setting = settings_get_rom_setting(&settings_current, (info->start + i), info->is_peripheral);
