...
static XImage *image = NULL; /* (T)he image structure to draw the Speccy's screen on */
...
static libspectrum_word rgb_image[2 * (DISPLAY_SCREEN_HEIGHT + 4)][2 * (DISPLAY_SCREEN_WIDTH  + 3)];
static const int rgb_pitch = 2 * (DISPLAY_SCREEN_WIDTH + 3);

// A scaled copy of the image displayed on the Spectrum's screen
static libspectrum_word scaled_image[3 * DISPLAY_SCREEN_HEIGHT][3 * DISPLAY_SCREEN_WIDTH];
static const ptrdiff_t scaled_pitch = 3 * DISPLAY_SCREEN_WIDTH * 2;

// A scaled copy of the image displayed on the Spectrum's screen
static libspectrum_word rgb_image_backup[2 * (DISPLAY_SCREEN_HEIGHT + 4)][2 * (DISPLAY_SCREEN_WIDTH  + 3)];
...
static int xdisplay_find_visual(void)
{
    ...
    vis = XGetVisualInfo(display, VisualScreenMask, &visual_tmpl, &nvis);
    if (vis != NULL) {
        for (i = 0; i < nvis; i++) {
            /*
            Save the visual index and its depth, if this is the first truecolor visual,
            or a visual that is 'preferred' over the previous 'best' visual.
            */
            if (((sel_v_depth == -1) && (vis[i].depth >= 4)) ||
                ((vis[i].depth > sel_v_depth) && (vis[i].depth <= 16)) ||
                ((vis[i].depth <= 8) && (vis[i].depth == sel_v_depth) && (vis[i].class != sel_v_class) &&
...
static void xdisplay_putpixel_4(int x, int y, libspectrum_word *colour)
{
    XPutPixel(image, x, y, *colour);
}


static void xdisplay_putpixel_8(int x, int y, libspectrum_word *colour)
{
    unsigned long c = *colour;

    c = (((c & 0x1f) * 3) / 31) + (((((c >> 5) & 0x3f) * 7) / 63) << 2) + ((((c >> 11) * 3) / 31) << 5);
    XPutPixel(image, x, y, colours[c]);
}


static void xdisplay_putpixel_15(int x, int y, libspectrum_word *colour)
{
    unsigned long c = *colour;
...
static void xdisplay_putpixel_16(int x, int y, libspectrum_word *colour)
{
    XPutPixel(image, x, y, *colour);
}


static void xdisplay_putpixel_24(int x, int y, libspectrum_word *colour)
{
    unsigned long c = *colour;

    c = ((((c & 0x1f) * 255) / 31) << bShift) + (((((c >> 5) & 0x3f) * 255) / 63) << gShift) +
        ((((c >> 11) * 255) / 31) << rShift);
    XPutPixel(image, x, y, c);
}


static int xdisplay_alloc_colour(Colormap *map, XColor *colour)
{
    for (;;) {
        ...
        fprintf(stderr,
                "%s: XAllocColor failed (%04x %04x %04x)\n",
                fuse_progname,
                colour->red,
                colour->green,
                colour->blue);
...
static int xdisplay_allocate_colours4(void)
{
    ...
    if (settings_current.bw_tv) {
        for (i = 0; i < 16; i++) { // grey
            c.red = c.green = c.blue = ((rgb_for_4[i * 3] * 19595) / 255) +
...
static int xdisplay_allocate_colours8(void)
{
    ...
    for (r = 0; r < 4; r++) { // rgb232 => 128
        for (g = 0; g < 8; g++) {
            for (b = 0; b < 4; b++) {
                if (settings_current.bw_tv) {
                    c.red = c.green = c.blue = ((r * 19595) / 3) + ((g * 38469) / 7) + ((b *  7471) / 3);
                } else {
                    c.red = (r * 65535) / 3;
                    c.green = (g * 65535) / 7;
                    c.blue = (b * 65535) / 3;
...
static int try_shm(void)
{
    ...
    */
    ...
    // Now flag the chunk for deletion; this will take effect when everything has detached from it
    shmctl(id, IPC_RMID, NULL);
...
static int get_shm_id(const int size)
{
    ...
    do {
        ...
        // If the chunk didn't already exist, try and create one for our use
        if (id == -1) {
        ...
        if (shmctl(id, IPC_STAT, &shm) != -1) {
            ...
            if (shm.shm_nattch) {
                ...
                // If we couldn't remove that chunk, try another key. If we could, just try again
...
static void register_scalers(void)
{
    ...
    if (current_scaler != SCALER_NUM) {
        f = 4.0 * scaler_get_scaling_factor(current_scaler) * (machine_current->timex ? 2 : 1);
    }
    if (scaler_is_supported(current_scaler) && ((xdisplay_current_size * 4) == f)) {
...
static void xdisplay_update_rect_noscale(int x, int y, int w, int h)
{
    int yy, xx;
...
static void xdisplay_update_rect_scale(int x, int y, int w, int h)
{
    int yy = y, xx = x;

    y = (y * image_scale) >> 2;
    x = (x * image_scale) >> 2;
    scaler_proc16((libspectrum_byte *)&(rgb_image[yy + 2][xx + 1]),
...
void xdisplay_area(int x, int y, int w, int h)
{
...
static void xdisplay_setup_rgb_putpixel(void)
{
...
{
    libspectrum_word pc = settings_current.bw_tv ? pal_grey[colour] : pal_colour[colour];
...
void uidisplay_plot8(int x, int y, libspectrum_byte data, libspectrum_byte ink, libspectrum_byte paper)
{
    ...
    libspectrum_word pi = settings_current.bw_tv ? pal_grey[ink] : pal_colour[ink];
    libspectrum_word pp = settings_current.bw_tv ? pal_grey[paper] : pal_colour[paper];
