...
int xui_screenNum; // (W)hich screen are we using on our X server?
...
int ui_init(int *argc, char ***argv)
{
    ...
    if ((display = XOpenDisplay(displayName)) == NULL) {
        fprintf(stderr,"%s: cannot connect to X server %s\n", fuse_progname, XDisplayName(displayName));
        return 1;
    }
    ...
    xui_mainWindow = XCreateSimpleWindow(display,
                                         RootWindow(display, xui_screenNum),
                                         0,
                                         0,
                                         DISPLAY_ASPECT_WIDTH,
                                         DISPLAY_SCREEN_HEIGHT,
                                         0,
                                         BlackPixel(display, xui_screenNum),
                                         WhitePixel(display, xui_screenNum));
    ...
    XSetWMProperties(display, xui_mainWindow, &windowName, &iconName, *argv, *argc, sizeHints, wmHints, classHint);
    ...
    XChangeWindowAttributes(display, xui_mainWindow, windowFlags, &windowAttributes);
    ...
    Pixmap mask = XCreatePixmap(display, RootWindow(display, xui_screenNum), 1, 1, 1);
    ...
    while (XEventsQueued(display, QueuedAlready)) {
        ...
        switch (event.type) {
            case ConfigureNotify:
                xdisplay_configure_notify(event.xconfigure.width, event.xconfigure.height);
                break;
            case Expose:
                xdisplay_area(event.xexpose.x, event.xexpose.y, event.xexpose.width, event.xexpose.height);
                break;
            case ButtonPress:
            ...
            case KeyRelease:
                ...
                break;
            case ClientMessage:
                if ((event.xclient.format == 32) && (event.xclient.data.l[0] == delete_window_atom)) {
