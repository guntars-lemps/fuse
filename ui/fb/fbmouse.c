...
int fbmouse_init(void)
{
    ...
    // Try to select plain PS/2 (just in case it's not already selected). Ignore any error.
    ...
    /* Try to set the mouse mode. We prefer ExPS/2 but will accept ImPS/2.
       ...
       buttons 2 and 3, and so that the wheel is available should it be wanted somewhere.
    */
...
static void mouse_update_gpm(void)
{
    ...
    for (;;) {
        ...
        if (event.dx | event.dy) {
            ui_mouse_motion(event.dx, event.dy);
        }
        ...
        if (db) {
            ...
            if (db & GPM_B_LEFT) {
                ui_mouse_button(1, event.buttons & GPM_B_LEFT);
            }
            if (db & GPM_B_RIGHT) {
                ui_mouse_button(3, event.buttons & GPM_B_RIGHT);
            }
        }
    }
}
...
#endif
{
    ...
    btn_state = btn_new;
    if (btn_changed & 1) {
        ui_mouse_button(1, btn_new & 1);
    }
    if (btn_changed & 2) {
        ui_mouse_button(3, btn_new & 2);
    }

    if (xoff || yoff) {
        ui_mouse_motion(xoff, -yoff);
    ]
}
