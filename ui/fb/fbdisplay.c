...
libspectrum_word fbdisplay_image[2 * DISPLAY_SCREEN_HEIGHT][DISPLAY_SCREEN_WIDTH];
...
int fbdisplay_init(void)
{
    ...
    if (ioctl(fb_fd, FBIOGET_FSCREENINFO, &fixed) || ioctl(fb_fd, FBIOGET_VSCREENINFO, &orig_display)) {
    ...
    for (i = 0; i < 16; i++) {
        ...
        c = ((i & 1) ? (v * 0.114) : 0.0) + ((i & 2) ? (v * 0.299) : 0.0) + ((i & 4) ? (v * 0.587) : 0.0) + 0.5;
...
static int fb_set_mode(void)
{
    ...
    const fuse_fb_mode_t *fb_modes = (settings_current.doublescan_mode == 0) ? fb_modes_singlescan :
    ...
    switch (fb_resolution) {
        case FB_RES(640, 480):
            for (y = start; y < start + height; y++) {
                ...
                if (hires) {
                    for (i = 0, point = gm + (y * display.xres_virtual) + x; i < width; i++, point++) {
                        *point = colours[fbdisplay_image[y][x+i]];
                    }
                } else {

                    for (i = 0, point = gm + (2 * y * display.xres_virtual) + (x * 2); i < width; i++, point += 2) {
        ...
        case FB_RES(640, 240):
            ...
            for (y = start; y < start + height; y++) {
                ...
                if (hires) {
                    ...
                    for (i = 0, point = gm + (y * display.xres_virtual) + (x * 2); i < width; i++, point += 2) {
