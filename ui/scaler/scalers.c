...
#define HQ_YUVDIFF(y1,u1,v1,y2,u2,v2) ((ABS(y1 - y2) > HQ_trY) || (ABS(u1 - u2) > HQ_trU) || (ABS(v1 - v2) > HQ_trV))
...
#define MOVE_B_TO_A(A,B) w[A] = w[B]; y[A] = y[B]; u[A] = u[B]; v[A] = v[B];
...
int scaler_select_bitformat(libspectrum_dword BitFormat)
{
    switch (BitFormat) {
        ...
        /* FIXME(?): there is an assumption here that our colour fields
           ...
           and (*) xxxx|xyyy|yyzz|zzz0 for the 555 mode,
           where (*) is the least significant bit on LSB machines and the most significant bit on MSB machines.
           This is currently (April 2003) OK as the only user interface to use this code is SDL,
           which hides all variation in SDL_MapRGB(3), but be very careful (especially about endianness)
           if we ever use the "interpolating" scalers from another user interface */
...
/* The assumption here is that the colour fields are laid out in memory as
   (LSB) red|green|blue|padding (MSB).
   We wish to access these as 32-bit entities,
   so make sure we get our masks the right way round. */
...
static inline int GetResult(libspectrum_dword A, libspectrum_dword B, libspectrum_dword C, libspectrum_dword D)
{
    const int ac = (A == C);
    const int bc = (B == C);
    const int x1 = ac;
...
static inline libspectrum_dword INTERPOLATE(libspectrum_dword A, libspectrum_dword B)
{
    if (A != B) {
...
static inline libspectrum_dword Q_INTERPOLATE(libspectrum_dword A,
                                              libspectrum_dword B,
                                              libspectrum_dword C,
                                              libspectrum_dword D)
{
    register libspectrum_dword x = ((A & qcolorMask) >> 2) +
                                   ...
                                   ((C & qcolorMask) >> 2) +
                                   ((D & qcolorMask) >> 2);
    register libspectrum_dword y = (A & qlowpixelMask) + (B & qlowpixelMask) + (C & qlowpixelMask) + (D & qlowpixelMask);
...
static inline libspectrum_dword HQ_INTERPOLATE_3(libspectrum_dword A, libspectrum_dword B)
{
    return ((((A & greenMask) * 7 + (B & greenMask)) & green8_Mask) +
            (((A & redblueMask) * 7 + (B & redblueMask)) & redblue8_Mask)) >> 3;
}


static inline libspectrum_dword HQ_INTERPOLATE_4(libspectrum_dword A, libspectrum_dword B, libspectrum_dword C)
{
    return ((((A & greenMask) * 2 + ((B & greenMask) + (C & greenMask)) * 7) & green16_Mask) +
            (((A & redblueMask) * 2 + ((B & redblueMask) + (C & redblueMask)) * 7) & redblue16_Mask)) >> 4;
}


static inline libspectrum_dword HQ_INTERPOLATE_6(libspectrum_dword A, libspectrum_dword B, libspectrum_dword C)
{
    return ((((A & greenMask) * 5 + (B & greenMask) * 2 + (C & greenMask)) & green8_Mask) +
            (((A & redblueMask) * 5 + (B & redblueMask) * 2 + (C & redblueMask)) & redblue8_Mask)) >> 3;
}


static inline libspectrum_dword HQ_INTERPOLATE_7(libspectrum_dword A, libspectrum_dword B, libspectrum_dword C)
{
    return ((((A & greenMask) * 6 + (B & greenMask) + (C & greenMask)) & green8_Mask) +
            (((A & redblueMask) * 6 + (B & redblueMask) + (C & redblueMask)) & redblue8_Mask)) >> 3;
}


static inline libspectrum_dword HQ_INTERPOLATE_9(libspectrum_dword A, libspectrum_dword B, libspectrum_dword C)
{
    return ((((A & greenMask) * 2 + ((B & greenMask) + (C & greenMask)) * 3) & green8_Mask) +
            (((A & redblueMask) * 2 + ((B & redblueMask) + (C & redblueMask)) * 3) & redblue8_Mask)) >> 3;
}


static inline libspectrum_dword HQ_INTERPOLATE_10(libspectrum_dword A, libspectrum_dword B, libspectrum_dword C)
{
    return ((((A & greenMask) * 14 + (B & greenMask) + (C & greenMask)) & green16_Mask) +
            (((A & redblueMask) * 14 + (B & redblueMask) + (C & redblueMask)) & redblue16_Mask)) >> 4;
}
                                 ...
                                 int height)
{
    ...
    while (height--) {
        ...
        for (i = 0; i < width; ++i) {
            ...
            libspectrum_dword colorA0, colorA1, colorA2, colorA3, colorB0, colorB1, colorB2, colorB3, colorS1, colorS2;
                            ...
                            int height)
{
    ...
    while (height--) {
        ...
        for (i = 0; i < width; ++i) {
            ...
            libspectrum_dword colorC, colorD, colorE, colorF, colorG, colorH, colorI;
            libspectrum_dword colorJ, colorK, colorL, colorM, colorN, colorO;
                               ...
                               int height)
{
    ...
    while (height--) {
        ...
        for (i = 0,
             s = (const scaler_data_type*)srcPtr,
                            ...
                            int height)
{
    ...
    */
    ...
    libspectrum_byte r1, g1, b1;
    libspectrum_byte r2, g2, b2;
    libspectrum_byte r3, g3, b3;
    ...
    */
    for (j = height; j; j--) {
        ...
        p++;
#endif
        u1 = (RGB_TO_U(r2, g2, b2) + (2 * RGB_TO_U(r0, g0, b0)) + RGB_TO_U(r1, g1, b1)) >> 2;
        v1 = (RGB_TO_V(r2, g2, b2) + (2 * RGB_TO_V(r0, g0, b0)) + RGB_TO_V(r1, g1, b1)) >> 2;
        for (i = width; i; i -= 2) {
#if SCALER_DATA_SIZE == 2
            // 1.a. RGB => RGB
            r2 = R_TO_R(*p);
            ...
            u2 = (RGB_TO_U(r1, g1, b1) + (2 * RGB_TO_U(r2, g2, b2)) + RGB_TO_U(r3, g3, b3)) >> 2;
            v2 = (RGB_TO_V(r1, g1, b1) + (2 * RGB_TO_V(r2, g2, b2)) + RGB_TO_V(r3, g3, b3)) >> 2;
            // 3.a. YUV => RGB
        ...
        for (i = width; i; i--) {
            ...
            *q = rx + (gx << 8) + (bx << 16);
            ...
            if (settings_current.pal_tv2x) {
                *(q + nextlineDst) = ((((*q & redblueMask) * 7) >> 3) & redblueMask) |
    ...
    for (j = height; j; j--) {
        ...
        for (i = width; i; i--) {
            ...
            *(q + nextlineDst) = *q;
            ...
            if (settings_current.pal_tv2x) {
                *(q + (nextlineDst << 1)) = ((((*q & redblueMask) * 7) >> 3) & redblueMask) |
                                            ((((*q & greenMask) * 7) >> 3) & greenMask);
            } else {
                *(q + (nextlineDst << 1)) = *q;
            }
                            ...
                            int height)
{
    ...
    scaler_data_type *q, *q1, *qN, *qN1;
    scaler_data_type *q0 = (scaler_data_type *)dstPtr;
    ...
    */
    for (j = 0; j < height; j++) {
        ...
        for (i = 0; i < width; i++) {
            pattern = 0;
            if (HQ_YUVDIFF(y[5], u[5], v[5], y[1], u[1], v[1])) {
                pattern |= 0x01;
            }
            if (HQ_YUVDIFF(y[5], u[5], v[5], y[2], u[2], v[2])) {
                pattern |= 0x02;
            }
            if (HQ_YUVDIFF(y[5], u[5], v[5], y[3], u[3], v[3])) {
                pattern |= 0x04;
            }
            if (HQ_YUVDIFF(y[5], u[5], v[5], y[4], u[4], v[4])) {
                pattern |= 0x08;
            }
            if (HQ_YUVDIFF(y[5], u[5], v[5], y[6], u[6], v[6])) {
                pattern |= 0x10;
            }
            if (HQ_YUVDIFF(y[5], u[5], v[5], y[7], u[7], v[7])) {
                pattern |= 0x20;
            }
            if (HQ_YUVDIFF(y[5], u[5], v[5], y[8], u[8], v[8])) {
                pattern |= 0x40;
            }
            if (HQ_YUVDIFF(y[5], u[5], v[5], y[9], u[9], v[9])) {
                pattern |= 0x80;
            }
