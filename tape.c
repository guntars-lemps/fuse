...
static const char * const stop_event_detail_string = "stop";
...
static void tape_event_record_sample(libspectrum_dword last_tstates, int type, void *user_data);
...
static int tape_init(void *context)
{
    ...
    play_event = debugger_event_register(debugger_type_string, play_event_detail_string);
    stop_event = debugger_event_register(debugger_type_string, stop_event_detail_string);

    debugger_system_variable_register(debugger_type_string, microphone_variable_name, get_microphone, NULL);

    tape_edge_event = event_register(next_edge, "Tape edge");
    tape_mic_off_event = event_register(tape_stop_mic_off, "Tape stop MIC off");
    record_event = event_register(tape_event_record_sample, "Tape sample record");

    tape_modified = 0;

    // Don't call tape_stop() here as the UI hasn't been initialised yet, so we can't update the statusbar
    tape_playing = 0;
    tape_microphone = 0;
...
void tape_register_startup(void)
{
    ...
    startup_manager_register(STARTUP_MANAGER_MODULE_TAPE,
                             dependencies,
...
int tape_open(const char *filename, int autoload)
{
    ...
    error = tape_read_buffer(file.buffer, file.length, LIBSPECTRUM_ID_UNKNOWN, filename, autoload);
    if (error) {
...
int tape_read_buffer(unsigned char *buffer, size_t length, libspectrum_id_t type,  const char *filename, int autoload)
{
    int error;
...
static int does_tape_load_with_code(void)
{
    ...
    for (block = libspectrum_tape_iterator_init(&iterator, tape); block; block = libspectrum_tape_iterator_next(&iterator)) {
        ...
        if ((block_type != LIBSPECTRUM_TAPE_BLOCK_ROM) && (block_type != LIBSPECTRUM_TAPE_BLOCK_DATA_BLOCK)) {
            continue;
        }

        // For this to be a CODE block, the block must have the right length,
        // it must have the header flag set and it must indicate a CODE block
        block_length = libspectrum_tape_block_data_length(block);
        data = libspectrum_tape_block_data(block);
        needs_code = (block_length == 19) && (data[0] == 0x00) && (data[1] == 0x03);
...
int tape_select_block(size_t n)
{
    ...
    error = tape_select_block_no_update(n);
    if (error) {
        return error;
    }
...
int tape_write(const char* filename)
{
    ...
    // Work out what sort of file we want from the filename; default to .tzx if we couldn't guess
    error = libspectrum_identify_file_with_class(&type, &class, filename, NULL, 0);
    if (error) {
        return error;
    }
...
// returns 0 if a block was loaded (even if it had an tape loading error or equivalent)
// or non-zero if there was an error at the emulator level, or tape traps are not active
int tape_load_trap(void)
{
    ...
    if (!settings_current.tape_traps || tape_playing || rzx_playback || rzx_recording) {
        return 2;
    }
    ...
    // If this block isn't a ROM loader, start the block playing.
    // After that, return with `error' so that we actually do whichever instruction it was that caused the trap to hit
    ...
    // We don't properly handle the case of partial loading, so don't run the traps in that situation
    ...
    // Peek at the next block. If it's a ROM block, move along, initialise the block, and return
...
// Append to the current tape file in memory; returns 0 if a block was saved
// or non-zero if there was an error at the emulator level, or tape traps are not active
int tape_save_trap(void)
{
    ...
    if (!settings_current.tape_traps || tape_recording || rzx_playback || rzx_recording) {
        return 2;
    }
...
static int tape_play(int autoplay)
{
    ...
    // Once the tape has started, the phantom typist has done its job so cancel any pending actions
    phantom_typist_deactivate();
...
int tape_stop(void)
{
    if (tape_playing) {
        ...
        // Turn off any lingering MIC level in a second
        // (some loaders like Alkatraz seem to check the MIC level soon after loading is finished,
        // presumably as a copy protection check
        event_add(tstates + machine_current->timings.tstates_per_frame, tape_mic_off_event);
    }

    if (stop_event != -1) {
        debugger_event(stop_event);
    }
...
void tape_record_start(void)
{
    ...
    rec_state.tape_buffer = libspectrum_new(libspectrum_byte, rec_state.tape_buffer_size);
    rec_state.tape_buffer_used = 0;

    // start scheduling events that record into a buffer that we start allocating here
...
static int write_rec_buffer(libspectrum_byte *tape_buffer, libspectrum_dword tape_buffer_used, int last_level_count)
{
...
void tape_event_record_sample(libspectrum_dword last_tstates, int type, void *user_data)
{
    if (rec_state.last_level != (ula_tape_level())) {
        ...
        if ((rec_state.tape_buffer_used + 5) >= rec_state.tape_buffer_size) {
            ...
            rec_state.tape_buffer = libspectrum_renew(libspectrum_byte,
...
int tape_record_stop(void)
{
    ...
    // stop scheduling events and turn buffer into a block and pop into the current tape
...
void tape_next_edge(libspectrum_dword last_tstates, int from_acceleration)
{
    ...
    libspec_error = libspectrum_tape_get_next_edge(&edge_tstates, &flags, tape);
    if (libspec_error != LIBSPECTRUM_ERROR_NONE) {
        return;
    }

    // Invert the microphone state
    if (edge_tstates || !(flags & LIBSPECTRUM_TAPE_FLAGS_NO_EDGE) ||
        (flags & (LIBSPECTRUM_TAPE_FLAGS_STOP | LIBSPECTRUM_TAPE_FLAGS_LEVEL_LOW | LIBSPECTRUM_TAPE_FLAGS_LEVEL_HIGH))) {
    ...
    // If we've been requested to stop the tape, do so and then return without stacking another edge
    if ((flags & LIBSPECTRUM_TAPE_FLAGS_STOP) ||
         ((flags & LIBSPECTRUM_TAPE_FLAGS_STOP48) &&
         (!(libspectrum_machine_capabilities(machine_current->machine) & LIBSPECTRUM_MACHINE_CAPABILITY_128_MEMORY)))) {
        tape_stop();
        return;
    }
    ...
    if (flags & LIBSPECTRUM_TAPE_FLAGS_BLOCK) {
        ...
        // If the tape was started automatically, tape traps are active and the new block is a ROM loader,
        // stop the tape and return without putting another event into the queue
        block = libspectrum_tape_current_block(tape);
    ...
    // should occur 'edge_tstates' after the last edge, not after the current time
    // (these will be slightly different as we only process events between instructions).
...
int tape_foreach(void (*function)(libspectrum_tape_block *block, void *user_data), void *user_data)
{
    libspectrum_tape_block *block;
    libspectrum_tape_iterator iterator;
...
int tape_block_details(char *buffer, size_t length, libspectrum_tape_block *block)
{
    libspectrum_byte *data;
    const char *type;
    ...
    switch (libspectrum_tape_block_type(block)) {
        ...
        case LIBSPECTRUM_TAPE_BLOCK_DATA_BLOCK:
            // See if this looks like a standard Spectrum header and if so display some extra data
...
normal:
            snprintf(buffer, length, "%lu bytes", (unsigned long)libspectrum_tape_block_data_length(block));
            break;
        ...
        case LIBSPECTRUM_TAPE_BLOCK_PURE_TONE:
            snprintf(buffer, length, "%lu tstates", (unsigned long)libspectrum_tape_block_pulse_length(block));
            break;

        case LIBSPECTRUM_TAPE_BLOCK_PULSES:
            snprintf(buffer, length, "%lu pulses", (unsigned long)libspectrum_tape_block_count(block));
            break;
        ...
        case LIBSPECTRUM_TAPE_BLOCK_PAUSE:
            snprintf(buffer, length, "%lu ms", (unsigned long)libspectrum_tape_block_pause(block));
            break;
        ...
        case LIBSPECTRUM_TAPE_BLOCK_LOOP_START:
            snprintf(buffer, length, "%lu iterations", (unsigned long)libspectrum_tape_block_count(block));
            break;

        case LIBSPECTRUM_TAPE_BLOCK_SELECT:
            snprintf(buffer, length, "%lu options", (unsigned long)libspectrum_tape_block_count(block));
            break;
