        ...
        case 1:
            switch (ctx->type) {
                case UTILS_AUXILIARY_LIB:
                    strncpy(ctx->path, "PROGDIR:lib/", PATH_MAX);
                    return 1;
                case UTILS_AUXILIARY_ROM:
                    strncpy(ctx->path, "PROGDIR:roms/", PATH_MAX);
                    return 1;
                case UTILS_AUXILIARY_WIDGET:
                    strncpy(ctx->path, "PROGDIR:ui/widget/", PATH_MAX);
                    return 1;
                case UTILS_AUXILIARY_GTK:
                    strncpy(ctx->path, "PROGDIR:ui/gtk/", PATH_MAX);
                    return 1;
                default:
            ...
            if (compat_is_absolute_path(fuse_progname)) {
                ...
                if (!getcwd(buffer, len)) {
                    ui_error(UI_ERROR_ERROR, "error getting current working directory: %s", strerror(errno));
                    return 0;
                }
