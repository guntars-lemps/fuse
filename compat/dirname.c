...
static char *__memrchr(char *s, int c, size_t n)
{
    for (s += n - 1; n; n--, s--) {
        if (*s == c) {
            return s;
...
char *dirname(char *path)
{
    ...
    if (last_slash != NULL) {
        ...
        /* This assignment is ill-designed but the XPG specs require to return a string containing "."
           in any case no directory part is found and so a static and constant string is required.  */
        path = (char *)dot;
    }
